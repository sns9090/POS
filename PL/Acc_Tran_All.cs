...
namespace POS
{
    public partial class Acc_Tran_All : BaseForm
    {
       // DataTable dtg;
        DataTable dthdr, dtdtl, dttrtyps;
        string a_ref, a_type,a_condi;
        ...
        public Acc_Tran_All(string atype,string aref,string condi)
        {
            InitializeComponent();
        ...
        */
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView1.CurrentCell == dataGridView1[0, dataGridView1.CurrentRow.Index])
            {
                try
                {
                    SqlDataAdapter da = new SqlDataAdapter("select top 1 a_key a ,a_name b from accounts where a_active=1 and accontrol=0 and a_key = '" + dataGridView1.CurrentRow.Cells[0].Value + "' and a_brno='" + BL.CLS_Session.brno + "'", con);
           ...
           // dtg = dataGridView1.DataSource;
            ...
            try
            {
                ...
                */
                ...
                // MessageBox.Show(tr);
                dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('01','02','03','04','14','05','15','06','16','07','17','21','31','32')");
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("يرجى اختيار نوع القيد");
                    return;
                }
                ...
                {
                    daml.SqlCon_Open();
                    int exexcuted = daml.Insert_Update_Delete_retrn_int("INSERT INTO acc_hdr(a_brno,a_type,a_ref,a_mdate, a_hdate,a_text,a_amt,a_entries,usrid,jhsrc) VALUES('" + BL.CLS_Session.brno + "','" + comboBox1.SelectedValue + "'," + ref_max + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "'," + txt_amt.Text + "," + (dataGridView1.RowCount - 1) + ",'" + txt_user.Text + "','Acc1')", false);
                    daml.SqlCon_Close();
                    if (exexcuted > 0)
                    {
    ...
    */
                    if (!row.IsNewRow && row.Cells[0].Value != null)
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,  a_camt,a_folio,a_mdate,a_hdate,jddbcr) VALUES(@a0,@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10)", con))
                        {
                           ...
                           // cmd.Parameters.AddWithValue("@a5", (string.IsNullOrEmpty(row.Cells[3].Value.ToString())? 0 : Convert.ToDouble(row.Cells[3].Value)));
                            ...
                            cmd.Parameters.AddWithValue("@a10", (Convert.ToDouble(row.Cells[3].Value)>0 ? "C" : "D"));
        ...
        private void Fill_Data(string atype, string aref,string acondi)
        {
            ...
            try
            {
              //  string mdate, hdate;
               // mdate = txt_mdate.Text.Replace("-", "").Substring(4, 4) + txt_mdate.Text.Replace("-", "").Substring(2, 2) + txt_mdate.Text.Replace("-", "").Substring(0, 2);
              //  hdate = txt_hdate.Text.Replace("-", "").Substring(4, 4) + txt_hdate.Text.Replace("-", "").Substring(2, 2) + txt_hdate.Text.Replace("-", "").Substring(0, 2);
//string[] stringArray = { "04", "05", "14", "15" };
