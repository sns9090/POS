...
namespace POS.Pos
{
    public partial class Resturant_sales : Form
    {
        SqlDataAdapter dacr1, dacr, dacr2;
        DataTable dtn, hdTable, dtTable, dtcards;
        DataSet1 ds;
       ...
       // StringBuilder input = new StringBuilder();
        ...
        private void Resturant_sales_Load(object sender, EventArgs e)
        {
            ...
            if (!BL.CLS_Session.port_no.Equals("99") && BL.CLS_Session.use_cd)
            {
                if (SerialPort.GetPortNames().ToList().Contains("COM" + BL.CLS_Session.port_no))
                {
            ...
            dtcards = daml.SELECT_QUIRY_only_retrn_dt("select * from pos_cards where card_id>4 and inactive=0");
            cmb_app.DataSource = dtcards;
            cmb_app.DisplayMember = BL.CLS_Session.lang.Equals("E") ? "card_edesc" : "card_desc";
            cmb_app.ValueMember = "card_id";
            cmb_app.SelectedIndex = -1;
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (textBox1.Text != "")
            {
                ...
                if (remain > 0)
                {
                    ...
                    label5.Text = remain.ToString()+"   " + "المتبقي عالزبون";
            ...
            else
            {
              ...
              //  label5.Text = label1.Text + "   " + "المتبقي عالزبون";
                label5.Text = (Convert.ToDouble(label1.Text) - Convert.ToDouble(string.IsNullOrEmpty(txt_desc.Text) ? "0" : txt_desc.Text)).ToString() + "   " + "المتبقي عالزبون";
        ...
        private void Save_Only(object sender, EventArgs e)
        {
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            hdTable.Rows.Add(BL.CLS_Session.brno, BL.CLS_Session.slno, refmax + 1, BL.CLS_Session.ctrno, (chk_agel.Checked && !string.IsNullOrEmpty(txt_custno.Text)) ? 3 : typeno
                        , DateTime.Now, Convert.ToDouble(label1.Text)
                        , Convert.ToDouble(label9.Text), (chk_iscard.Checked ? 0 : Convert.ToDouble(textBox1.Text)), Convert.ToDouble(textBox2.Text), lblsalid.Text, rqmax + 1
                        , string.IsNullOrEmpty(txt_custno.Text) ? 0 : Convert.ToInt32(txt_custno.Text), Convert.ToDouble(string.IsNullOrEmpty(txt_desc.Text) ? "0" : txt_desc.Text)
                        , (Convert.ToDouble(label1.Text) - Convert.ToDouble(string.IsNullOrEmpty(txt_desc.Text) ? "0" : txt_desc.Text))
                        , DateTime.Now, 0, string.IsNullOrEmpty(BL.CLS_Session.tax_no) ? 0 : Math.Round(((Convert.ToDouble(label1.Text) - Convert.ToDouble(string.IsNullOrEmpty(txt_desc.Text) ? "0" : txt_desc.Text)) * Convert.ToDouble(BL.CLS_Session.tax_per) / (100 + Convert.ToDouble(BL.CLS_Session.tax_per))), 4)
                        , Convert.ToDouble(string.IsNullOrEmpty(txt_dscper.Text) ? "0" : txt_dscper.Text), (chk_iscard.Checked && cmb_app.SelectedIndex==-1 ? "1" :cmb_app.SelectedIndex!=-1 ? cmb_app.SelectedValue : "0")
                        , (chk_iscard.Checked ? textBox1.Text : "0"), 0, 0, 0, txt_custnam.Text, txt_temp.Text);


            ////using (SqlCommand cmd1 = new SqlCommand("INSERT INTO pos_hdr(brno,slno,ref,contr,type,date,total,count,payed,total_cost,saleman,req_no,cust_no,discount,net_total,tax_amt,dscper,card_type,card_amt,note,taxfree_amt,mobile) VALUES(@br,@sl,@rf,@ctr,@a0,@a1,@a2,@a3,@a4,@a5,@a7,@a8,@a9,@a11,@a12,@a13,@a14,@a15,@a16,@a17,@a18,@a19)", con2))
            ////    {
            ////        cmd1.Parameters.AddWithValue("@br", BL.CLS_Session.brno);
            ////        cmd1.Parameters.AddWithValue("@sl", BL.CLS_Session.slno);
            ...
            {/*
                ...
                */
                if (!row.IsNewRow && row.Cells[0].Value != null || !row.IsNewRow && row.Cells[4].Value != null || !row.IsNewRow && row.Cells[5].Value != null)
                {
                    ...
                    dtTable.Rows.Add(BL.CLS_Session.brno, BL.CLS_Session.slno, refmax + 1, BL.CLS_Session.ctrno, (chk_agel.Checked && !string.IsNullOrEmpty(txt_custno.Text)) ? 3 : typeno
                        , row.Cells[0].Value, row.Cells[2].Value.ToString().Length > 100 ? row.Cells[2].Value.ToString().Substring(0, 100) : row.Cells[2].Value, row.Cells[3].Value, row.Cells[5].Value, row.Cells[4].Value, row.Cells[6].Value, row.Cells[7].Value != null? row.Cells[7].Value:0
                                      , row.Cells[1].Value, srno, 1, 0, 1, string.IsNullOrEmpty(BL.CLS_Session.tax_no) ? 0 : Math.Round((Convert.ToDouble(row.Cells[8].Value) * Convert.ToDouble(BL.CLS_Session.tax_per) / (100 + Convert.ToDouble(BL.CLS_Session.tax_per))), 4),0,0);
           ...
           // MessageBox.Show("ok");
            using (SqlCommand cmd = new SqlCommand("Save_Pos_Hdr_Dtl"))
            {
                ...
                cmd.Parameters.AddWithValue("@ref", refmax);
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try {
                if(dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[4] && Convert.ToDouble(dataGridView1.CurrentRow.Cells[5].Value)<=0)
        ...
        private void txt_desc_Leave(object sender, EventArgs e)
        {
            ...
            txt_dscper.Text =Convert.ToDouble(label1.Text)>0? (100 * (Convert.ToDouble(string.IsNullOrEmpty(txt_desc.Text) ? "0" : txt_desc.Text)) / Convert.ToDouble(label1.Text)).ToString(): "0";
        ...
        private void cmb_app_KeyDown(object sender, KeyEventArgs e)
        {
