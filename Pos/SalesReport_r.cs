...
namespace POS.Pos
{
    public partial class Sales_Report_r : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlDataAdapter da4 = new SqlDataAdapter("select round(sum(case when type in  (1,4,5) then total  when type= 0 then -total end),2),round(sum(case when type in  (1,4,5) then total_cost  when type= 0 then -total_cost end),2),round(sum(case when type in  (1,4,5) then discount  when type=0 then -discount end),2),round(sum(case when type in  (1,4,5) then net_total  when type= 0 then -net_total end ),2),round(sum(case when type in  (1,4,5) then tax_amt  when type =0 then -tax_amt end),2) tax ,round(sum(((case when type in  (1,4,5) then net_total  when type= 0 then -net_total end) - (case when type in  (1,4,5) then tax_amt  when type= 0 then -tax_amt end))),2) net1,round(sum(case when type in  (1,4,5) then net_total-card_amt  when type= 0 then -net_total end),2)cash ,round(isnull(sum(case when type in  (1,4,5) and card_type in(1,2,3) then card_amt  else 0 end),0),2) card,round(isnull(sum(case when type in  (1,4,5) and card_type in(4) then card_amt  else 0 end),0),2) othr from " + condcncl + " where  brno='" + BL.CLS_Session.brno + "' and date between'" + datval.convert_to_yyyy_MMddwith_dash(xxx) + " " + maskedTextBox3.Text + "' and '" + datval.convert_to_yyyy_MMddwith_dash(zzz) + " " + maskedTextBox4.Text + "' " + conds + condt + condam + cndtyp + condcst + "", con3);
