...
namespace POS.Pos
{
    public partial class SalesReport_tm : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
               ...
               // string condam = string.IsNullOrEmpty(txt_salman.Text) ? " " : " and saleman=" + txt_salman.Text + " ";
                ...
                string condnotpay = chk_notpayd.Checked ? " and b.rtameen=0 " : " ";
                string xxx;
                ...
                SqlDataAdapter da3 = new SqlDataAdapter(@"select a.contr ,a.ref,FORMAT ([date], 'dd-MM-yyyy hh:mm:ss tt') [date],[count] ,round( net_total,2)[net_total],c.cu_name,isnull(b.dtameen,0) tamin,[saleman],(case when b.dtameen=b.rtameen then 1 else 0 end) dpayed,a.[type] at

                                                              FROM pos_hdr  a  join pos_hdr_r b on  a.[brno]=b.[dbrno] and a.[slno]=b.[dslno] and a.[ref]=b.[dref] and a.[contr] = b.[dcontr] 
                                                             left join customers c on a.[brno]=c.cu_brno and a.cust_no=c.cu_no where b.dtameen>0 and  a.brno='" + BL.CLS_Session.brno + "' and a.date between'" + datval.convert_to_yyyy_MMddwith_dash(xxx) + " " + maskedTextBox3.Text + "' and '" + datval.convert_to_yyyy_MMddwith_dash(zzz) + " " + maskedTextBox4.Text + "' " + conds + conddr + condcst + condnotpay +"", con3);
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    using (SqlCommand cmd1 = new SqlCommand("update pos_hdr_r set rtameen=@a3 where dref=" + row.Cells[1].Value + " and dcontr=" + row.Cells[0].Value + " and dbrno='" + BL.CLS_Session.brno + "'", con3))
                    {

                        cmd1.Parameters.AddWithValue("@a3", string.IsNullOrEmpty(row.Cells[8].Value.ToString()) ? 0 : Convert.ToDouble(row.Cells[6].Value));
                    ...
                    DataTable dtb = daml.SELECT_QUIRY_only_retrn_dt("SELECT * FROM pos_tamin_sdad where sref=" + row.Cells[1].Value + " and scontr=" + row.Cells[0].Value + " and sbrno='" + BL.CLS_Session.brno + "'");
                    if (dtb.Rows.Count == 0)
                    {
                        using (SqlCommand cmd1 = new SqlCommand("INSERT INTO pos_tamin_sdad(sbrno,sslno,sref,scontr,stype,sdate,spayed,snote,smobile,stameen) VALUES(@br,@sl,@rf,@ctr,@a0,@a1,@a2,@a3,@a4,@a5)", con3))
                        {
                            cmd1.Parameters.AddWithValue("@br", BL.CLS_Session.brno);
                            cmd1.Parameters.AddWithValue("@sl", BL.CLS_Session.slno);
                            cmd1.Parameters.AddWithValue("@rf", row.Cells[1].Value);
                            cmd1.Parameters.AddWithValue("@ctr", row.Cells[0].Value);
                            cmd1.Parameters.AddWithValue("@a0", row.Cells[9].Value);
                            ...
                            cmd1.Parameters.AddWithValue("@a2", string.IsNullOrEmpty(row.Cells[8].Value.ToString()) ? 0 : Convert.ToDouble(row.Cells[6].Value));
                            cmd1.Parameters.AddWithValue("@a3", "");
                            cmd1.Parameters.AddWithValue("@a4", "");
                            cmd1.Parameters.AddWithValue("@a5", row.Cells[6].Value);
