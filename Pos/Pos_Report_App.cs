...
namespace POS.Pos
{
    public partial class Pos_Report_App : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                */
                string condst = cmb_salctr.SelectedIndex != -1 ? " and c.slno = '" + cmb_salctr.SelectedValue.ToString() + "' " : " and c.brno + c.slno in(" + sl + ") ";
                string cond = checkBox1.Checked ? "c.date" : "c.date";
                string condgrp = cmb_grp.SelectedIndex == -1 ? "" : " and s.item_group='" + cmb_grp.SelectedValue + "' ";
                string condsgrp = cmb_sgrp.SelectedIndex == -1 ? "" : " and s.sgroup='" + cmb_sgrp.SelectedValue + "' ";
                string condsgrp1 = cmb_sgrp.SelectedIndex == -1 ? "" : " and s.sgroup='" + cmb_grp.SelectedValue.ToString() + cmb_sgrp.SelectedValue.ToString() + "' ";
              //  string condp = rad_posted.Checked ? " and posted=1 " : (rad_notpostd.Checked ? " and posted=0 " : " ");
                string condbr = chk_allbr.Checked ? "" : " and c.brno='" + BL.CLS_Session.brno + "' ";
              //  string condf = txt_fno.Text != "" ? txt_fno.Text : "1";
               ...
               // string conslctr = cmb_salctr.SelectedIndex != -1 ? " and slcenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " ";
                string condinvtype = cmb_type.SelectedIndex != -1 ? " and c.type = '" + cmb_type.SelectedValue.ToString() + "' " : " ";
              ...
              //
                ...
                {//  qstr = "Select a.item_group br_no,b.group_name br_name, nettotal=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),tax=round(sum(case when c.invtype in ('04','05') then c.QTY else - c.QTY end),2),salcost = round(sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end),2) ,win=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1))-sum((invdsvl/(nettotal+invdsvl))*iif(1=1,((c.QTY*(c.price-(c.price*c.discpc/100)))-c.offer_amt),((c.QTY*(c.price-Floor(c.price*c.discpc/100)))-c.offer_amt))*(case when h.invtype in ('04','05') then 1 else -1 end))-sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end)  ,2),winp='' from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condgrp + condsgrp + condst + "  group By  a.item_group,b.group_name order By a.item_group";
                    qstr = "  Select cast(g.card_id as nvarchar) br_no,g.card_desc br_name,sal= isnull(round(sum((case when b.type in (1,3,4,5) then 1 else -1 end)*(((b.qty*(b.price-(b.price*b.discpc/100))))-offr_amt))-sum((case when b.type in (1,3,4,5) then 1 else -1 end)*(((b.qty*(b.price-(b.price*b.discpc/100))))-offr_amt)*c.dscper/100),2),0)  "
                    ...
                    // + "+sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.tax_amt))-sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*(b.price-(b.price*b.discpc/100))*(a.invdsvl/(a.nettotal+a.invdsvl)))),2),0)"
                     ...
                     + ",per = 0.00  from pos_cards g with (NOLOCK) inner Join pos_hdr c with (NOLOCK) inner Join pos_dtl b with (NOLOCK) "
                     + "on  c.brno = b.brno and c.contr = b.contr and c.ref = b.ref and c.slno=b.slno and " + cond + " between '" + datval.convert_to_yyyyDDdd(xxx) + " " + (BL.CLS_Session.posatrtday.ToString().Length == 1 ? "0" + BL.CLS_Session.posatrtday.ToString() : BL.CLS_Session.posatrtday.ToString()) + ":00:00.000' and '" + datval.convert_to_yyyyDDdd(zzz) + " " + (BL.CLS_Session.posatrtday.ToString().Length == 1 ? "0" + BL.CLS_Session.posatrtday.ToString() : BL.CLS_Session.posatrtday.ToString()) + ":00:00.000' " + condbr + condinvtype + condst + " on g.card_id=c.card_type "
                     + "  where 1=1 " + condgrp + condsgrp + " and 0=0 group by g.card_id,g.card_desc order by g.card_id ";
                }
                else
                {//  qstr = "Select a.sgroup br_no,b.group_name br_name, nettotal=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),tax=round(sum(case when c.invtype in ('04','05') then c.QTY else - c.QTY end),2),salcost = round(sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end),2) ,win=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1))-sum((invdsvl/(nettotal+invdsvl))*iif(1=1,((c.QTY*(c.price-(c.price*c.discpc/100)))-c.offer_amt),((c.QTY*(c.price-Floor(c.price*c.discpc/100)))-c.offer_amt))*(case when h.invtype in ('04','05') then 1 else -1 end))-sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end)  ,2),winp='' from  items a join groups b on a.sgroup = b.group_id and b.group_pid is not null join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condgrp + condsgrp + condst + "  group By  a.sgroup,b.group_name order By a.sgroup";
                    ////qstr = "  Select g.card_id br_no,g.card_desc br_name,nettotal= isnull(round(sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*(b.price-(b.price*b.discpc/100)))-(case when a.with_tax=1 then b.tax_amt else 0 end)) "
                    //// + "+sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.tax_amt))-sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*(b.price-(b.price*b.discpc/100))*(a.invdsvl/(a.nettotal+a.invdsvl)))),2),0)"
                    //// + ",tax=isnull(round(sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*b.pkqty)),2),0) "
                    //// + ",salcost = 0 ,win = 0.00 ,winp ='' from groups g with (NOLOCK) left Join items s with (NOLOCK) inner Join pu_hdr a with (NOLOCK) inner Join pu_dtl b with (NOLOCK) "
                    //// + "on  a.branch = b.branch and a.invtype = b.invtype and a.ref = b.ref and a.pucenter=b.pucenter and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condst + " on s.item_no=b.itemno on g.card_id=s.sgroup"
                    //// + "  where 1=1 " + condgrp + condsgrp + " and g.group_pid is not null group by g.card_id,g.card_desc";
                    ...
                    qstr = "  Select cast(g.card_id as nvarchar) br_no,g.card_desc br_name,sal= isnull(round(sum((case when b.type in (1,3,4,5) then 1 else -1 end)*(((b.qty*(b.price-(b.price*b.discpc/100))))-offr_amt))-sum((case when b.type in (1,3,4,5) then 1 else -1 end)*(((b.qty*(b.price-(b.price*b.discpc/100))))-offr_amt)*c.dscper/100),2),0)  "
                     ...
                     + "on  c.brno = b.brno and c.contr = b.contr and c.ref = b.ref and c.slno=b.slno and " + cond + " between '" + datval.convert_to_yyyyDDdd(xxx) + " " + (BL.CLS_Session.posatrtday.ToString().Length == 1 ? "0" + BL.CLS_Session.posatrtday.ToString() : BL.CLS_Session.posatrtday.ToString()) + ":00:00.000' and '" + datval.convert_to_yyyyDDdd(zzz) + " " + (BL.CLS_Session.posatrtday.ToString().Length == 1 ? "0" + BL.CLS_Session.posatrtday.ToString() : BL.CLS_Session.posatrtday.ToString()) + ":00:00.000' " + condbr + condinvtype + condst + " on s.item_no=b.itemno on g.card_id=s.item_group"
                     + "  where 1=1 " + condgrp  + " and g.group_pid is null group by g.card_id,g.card_desc order by g.card_id  ";

                    //qstr = "  Select g.card_id br_no,g.card_desc br_name,sal= isnull(round(sum((case when b.type in (1,3,4,5) then 1 else -1 end)*(((b.qty*(b.price-(b.price*b.discpc/100))))-offr_amt))-sum((case when b.type in (1,3,4,5) then 1 else -1 end)*(((b.qty*(b.price-(b.price*b.discpc/100))))-offr_amt)*c.dscper/100),2),0)  "
                    //    // + ",tax=round(sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*b.pkqty)),2) "
                    //    // + "+sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.tax_amt))-sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*(b.price-(b.price*b.discpc/100))*(a.invdsvl/(a.nettotal+a.invdsvl)))),2),0)"
                    //+ ",qty=isnull(round(sum((case when b.type in (1,3,4,5) then 1 else -1 end)*(b.qty*b.pkqty)),2),0) "
                    //+ ",per = 0.00  from groups g with (NOLOCK) left Join items s with (NOLOCK) inner Join pos_hdr c with (NOLOCK) inner Join pos_dtl b with (NOLOCK) "
                    //+ "on  c.brno = b.brno and c.contr = b.contr and c.ref = b.ref and c.slno=b.slno and " + cond + " between '" + datval.convert_to_yyyyDDdd(xxx) + " " + (BL.CLS_Session.posatrtday.ToString().Length == 1 ? "0" + BL.CLS_Session.posatrtday.ToString() : BL.CLS_Session.posatrtday.ToString()) + ":00:00.000' and '" + datval.convert_to_yyyyDDdd(zzz) + " " + (BL.CLS_Session.posatrtday.ToString().Length == 1 ? "0" + BL.CLS_Session.posatrtday.ToString() : BL.CLS_Session.posatrtday.ToString()) + ":00:00.000' " + condbr + condinvtype + condst + " on s.item_no=b.itemno on g.card_id=s.item_group"
                    //+ "  where 1=1 " + " and s.item_group='" + cmb_grp.SelectedValue + "' " + " and g.group_pid is not null group by g.card_id,g.card_desc order by cast(g.card_id as int) ";
                }
              ...
              //  sumttlm = Math.Round((sumttld / (sumopnd - sumopnm)) * 100, 2);
               ...
               // dr[13] = "%" + sumttlm.ToString();
                ...
                {
                    dtrg.Cells[4].Value =  Math.Round(((Convert.ToDouble(dtrg.Cells[2].Value) / sumopnd) * 100), 2);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Pos.rpt.Pos_Report_Gr_rpt.rdlc";
