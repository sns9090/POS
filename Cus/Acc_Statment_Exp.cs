...
//};
          ...
          //  double seso = 0;
            ...
            if (aakey != "")
            {
                try
                {
                    ...
                    dataGridView1.DataSource = daml.SELECT_QUIRY(qstr2);
                    */
                    string qstr2, cond, condp, conddm, condt, condcmb, condcmb2, condcmb3, condpi;
                  ...
                  //  string condtx = txt_desc.Text;
                   //  dth = daml.SELECT_QUIRY_only_retrn_dt("select cu_name a_name,round(cu_opnbal,2) a_opnbal,cu_no a_key,whno from customers where cu_no='" + aakey + "' and cu_brno='" + BL.CLS_Session.brno + "'");
                    ...
                    if(chk_combin.Checked && !string.IsNullOrEmpty(dth.Rows[0][3].ToString()))
                        dths = daml.SELECT_QUIRY_only_retrn_dt("select round(isnull(su_opnbal,0),4) a_opnbal from suppliers where su_no='" + dth.Rows[0][3].ToString() + "' and su_brno='" + BL.CLS_Session.brno + "'");
                    ...
                    else
                    {
                        ...
                        condcmb3 = chk_combin.Checked ? ",'06','16'" : "";
                        condt = cmb_type.SelectedIndex != -1 ? " and a.a_type = '" + cmb_type.SelectedValue.ToString() + "' " : " ";
                         ...
                         conddm = chk_d.Checked ? " and a.jddbcr='C' " : (chk_m.Checked ? " and a.jddbcr='D' " : " ");
                         dtpi = daml.SELECT_QUIRY_only_retrn_dt("SELECT [a_brno],[a_type],[a_ref],[qst_ref],[qst_sl] FROM [acc_dtl]  where qst_ref<>0 and a_brno='" + BL.CLS_Session.brno + "' and cu_no='" + aakey + "' and a_type='02'");
                         condpi = chk_payed_inv.Checked ? " and qst_ref=0 and a.cu_no='" + aakey + "' " : " ";
                         qstr2 = "select a.a_type c1, a.a_ref c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),4) c6,b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.sl_no=b.sl_no " + condp + conddm + condt + condpi + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condcmb + " and a.a_type not in('04','14'" + condcmb3 + ") and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate,a.a_sysdate";
                    }
                  ...
                  //  MessageBox.Show(dtpi.Rows.Count.ToString());
                    if (dtpi.Rows.Count > 0 && chk_payed_inv.Checked)
                    {
                        foreach (DataRow rt in dtpi.Rows)
                        {                          
                            foreach (DataGridViewRow rw in dataGridView1.Rows)
                            {
                                if (rw.Cells["c2"].Value.ToString().Trim().Equals(rt[3].ToString()) && rw.Cells["sl"].Value.ToString().Trim().Equals(rt[4].ToString()) && rw.Cells["a_t"].Value.ToString().Trim().Equals("05"))
                                {
                                   // MessageBox.Show(" this " + rw.Index.ToString());
        ...
        private void button5_Click(object sender, EventArgs e)
        {
          ...
          ////  rf.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("details", dtprt));
            ...
            if (File.Exists(Directory.GetCurrentDirectory() + @"\reports\Cust_Balance_Dtl2_rpt.rdlc"))
                rf.reportViewer1.LocalReport.ReportPath = Directory.GetCurrentDirectory() + @"\reports\Cust_Balance_Dtl2_rpt.rdlc";
            else
