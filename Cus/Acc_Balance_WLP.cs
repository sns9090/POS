        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
              ...
              //  string cond = checkBox1.Checked ? "b.a_hdate" : "b.a_mdate";
                ...
                string conddc = chk_m.Checked ? " having round(( isnull(a.cu_opnbal+sum(isnull(b.a_damt,0)-isnull(b.a_camt,0)),0)),4)<0 " : (chk_d.Checked ? " having round(( isnull(a.cu_opnbal+sum(isnull(b.a_damt,0)-isnull(b.a_camt,0)),0)),4)>0 " : " having round(( isnull(a.cu_opnbal+sum(isnull(b.a_damt,0)-isnull(b.a_camt,0)),0)),4)<>0 ");
               ...
               //// string qstr = "select a.cu_no d1,a.cu_name d2,a.cu_opnbal d3,round(( isnull(sum(b.a_camt),0)),2) d4,round(( isnull(sum(b.a_damt),0)),2) d5,round((select isnull(a.cu_opnbal+sum(isnull(b.a_damt,0)-isnull(b.a_camt,0)),0)),2) d6,round((select isnull(max(iif(b.a_camt>0 and b.a_type not in('15') ,b.a_camt,0)),0)),2) d7,(select top 1 isnull(max(iif(b.a_camt>0 and b.a_type not in('15'),b.a_mdate,'        ')),'        ')) d8 from customers a left outer join acc_dtl b  inner join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and b.a_type not in ('04','14') on a.cu_brno=b.a_brno and a.cu_no=b.cu_no where a.cu_brno='" + BL.CLS_Session.brno + "' " + condc + condcity + condp + " group by a.cu_no ,a.cu_name ,a.cu_opnbal " + conddc + " order by cast(a.cu_no as int)";

                string qstr = "select a.cu_no d1,a.cu_name d2,a.cu_opnbal d3,round(( isnull(sum(b.a_camt),0)),4) d4,round(( isnull(sum(b.a_damt),0)),4) d5,round((select isnull(a.cu_opnbal+sum(isnull(b.a_damt,0)-isnull(b.a_camt,0)),0)),4) d6,0 d7,'        ' d8 from customers a left outer join acc_dtl b  inner join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and b.a_type not in ('04','14') on a.cu_brno=b.a_brno and a.cu_no=b.cu_no where a.cu_brno='" + BL.CLS_Session.brno + "' " + condc + condcity + condp + " group by a.cu_no ,a.cu_name ,a.cu_opnbal " + conddc + " order by cast(a.cu_no as int)";
               ...
               // dtacbal.Rows.Add(row);
                ...
                {
                    DataTable dtr0 = daml.SELECT_QUIRY_only_retrn_dt("select top 1 round(isnull(iif(b.a_camt>0 and b.a_type not in('04','05','14','15') ,b.a_camt,0),0),4),isnull((iif(b.a_camt>0 and b.a_type not in('04','05','14','15'),b.a_mdate,'        ')),'        ') from acc_dtl b where b.a_brno='" + BL.CLS_Session.brno + "' and b.cu_no='" + dtr[0] + "' order by a_mdate desc");
                    dtr[6]=dtr0.Rows[0][0];
                    dtr[7]=dtr0.Rows[0][1];
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //  rf.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("details", dtdtl));
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Cus.rpt.Cus_Balance_WLP_rpt.rdlc";
