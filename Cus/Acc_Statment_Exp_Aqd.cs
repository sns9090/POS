...
namespace POS.Cus
{
    public partial class Acc_Statment_Exp_Aqd : BaseForm
    {
...
//};
        ...
        private void button3_Click(object sender, EventArgs e)
        {
          ...
          // dataGridView2.DataSource = dt;
                ...
                if(BL.CLS_Session.lang.Equals("ع")) 
                    rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Cus.rpt.Acc_Statment_Exp_Aqd_rpt.rdlc";
                else
               ...
               // parameters[1] = new ReportParameter("mdate", dthdr.Rows[0][3].ToString().Substring(6, 2) + "- " + dthdr.Rows[0][3].ToString().Substring(4, 2) + "- " + dthdr.Rows[0][3].ToString().Substring(0, 4));
                ...
                parameters[2] = new ReportParameter("cur_bal", dt.Rows.Count > 0 ? seso.ToString() : txt_opnbal.Text);
        ...
        private void Load_Statment(string aakey)
        {
          ...
          //  double seso = 0;
            ...
            if (aakey != "")
            {
                string condaqd = string.IsNullOrEmpty(txt_aqd.Text) ? "" : " and c.aqd_no='"+txt_aqd.Text+"' ";
                try
                {
                   // dtall = daml.SELECT_QUIRY_only_retrn_dt("select c.* from acc_hdr c where exists(select 1 from  acc_dtl b where c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and b.cu_no='" + aakey + "') and c.a_brno='" + BL.CLS_Session.brno + "' order by c.serial_no");
                dtall = daml.SELECT_QUIRY_only_retrn_dt("select c.aqd_no from acc_hdr c where exists(select 1 from  acc_dtl b where c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and b.cu_no='" + aakey + "') and c.a_brno='" + BL.CLS_Session.brno + "' "+condaqd+" group by c.aqd_no order by c.aqd_no");
                    DataTable dtdtl2 = new DataTable();
                    DataTable dtdtlall = new DataTable();
                    foreach (DataRow dtra in dtall.Rows)
                    {
                        ...
                        dataGridView1.DataSource = daml.SELECT_QUIRY(qstr2);
                        */
                        string qstr2, cond, condp, conddm, condt, condcmb, condcmb2;

                        //  string condtx = txt_desc.Text;
                        //  dth = daml.SELECT_QUIRY_only_retrn_dt("select cu_name a_name,round(cu_opnbal,2) a_opnbal,cu_no a_key,whno from customers where cu_no='" + aakey + "' and cu_brno='" + BL.CLS_Session.brno + "'");
                       // dth = daml.SELECT_QUIRY_only_retrn_dt("select a.cu_name a_name,round((a.cu_opnbal + (select isnull(sum(b.a_damt-b.a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and c.a_mdate < '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' where a.cu_no=b.cu_no and b.a_type not in ('04','14'))),2) a_opnbal,a.cu_no a_key,a.whno  from customers a where a.cu_no='" + aakey + "' and a.cu_brno='" + BL.CLS_Session.brno + "'");
                        dth = daml.SELECT_QUIRY_only_retrn_dt("select a.cu_name a_name,round((a.cu_opnbal + (select isnull(sum(b.a_damt-b.a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and c.a_mdate < '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' where a.cu_no=b.cu_no and b.a_type not in ('04','14'))),2) a_opnbal,a.cu_no a_key,a.whno  from customers a where a.cu_no='" + aakey + "' and a.cu_brno='" + BL.CLS_Session.brno + "'");

                        if (chk_combin.Checked && !string.IsNullOrEmpty(dth.Rows[0][3].ToString()))
                            dths = daml.SELECT_QUIRY_only_retrn_dt("select round(isnull(su_opnbal,0),2) a_opnbal from suppliers where su_no='" + dth.Rows[0][3].ToString() + "' and su_brno='" + BL.CLS_Session.brno + "'");
                        ...
                        //////else
                        //////{
                        condcmb2 = string.IsNullOrEmpty(dth.Rows[0][3].ToString()) ? "a.su_no='XYX')" : "a.su_no='" + dth.Rows[0][3].ToString() + "')";
                        condcmb = chk_combin.Checked ? " and (a.cu_no='" + aakey + "' or " + condcmb2 : " and a.cu_no='" + aakey + "' ";
                        ...
                        conddm = chk_d.Checked ? " and a.jddbcr='C' " : (chk_m.Checked ? " and a.jddbcr='D' " : " ");
                       // qstr2 = "select a.a_type c1, b.serial_no c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),2) c6,b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu,b.a_ref rf from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.sl_no=b.sl_no " + condp + conddm + condt + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condcmb + " and a.a_type not in('04','14') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' and a.a_type='" + dtra[1] + "' and b.serial_no='" + dtra[22] + "'  order by a.a_mdate,a.a_sysdate";
                       // qstr2 = "select a.a_type c1, b.serial_no c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),2) c6,b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu,b.a_ref rf from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno " + condp + conddm + condt + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condcmb + " and a.a_type not in('04','14') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' and b.serial_no='" + dtra[0] + "'  order by a.a_mdate,a.a_sysdate";
                        qstr2 = "select a.a_type c1, b.aqd_no c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),2) c6,b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu,b.a_ref rf from acc_dtl a join acc_hdr b on a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.sl_no=b.sl_no " + condp + conddm + condt + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condcmb + " where b.aqd_no='" + dtra[0] + "' and a.cu_no='" + aakey + "' and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate,a.a_sysdate";
                        //////}
                        ...
                        dtdtl2 = daml.SELECT_QUIRY_only_retrn_dt(qstr2);
                        ...
                        if (dth.Rows.Count > 0)
                        {
                            ...
                            if (chk_combin.Checked && !string.IsNullOrEmpty(dth.Rows[0][3].ToString()))
                                txt_opnbal.Text = (Convert.ToDouble(dth.Rows[0][1]) + Convert.ToDouble(dths.Rows[0][0])).ToString();
                            else
                        ...
                        double sumc = 0, sumd = 0, sumd2 = 0;
                        foreach (DataRow dtr2 in dtdtl2.Rows)
                        {
                            sumc = sumc + Convert.ToDouble(dtr2[5]);
                           // sumc1 = sumc1 + (string.IsNullOrEmpty(dtr2[0].ToString()) ? Convert.ToDouble(dtr2[5]) : 0.00);
                          //  sumd2 = sumd2 + Convert.ToDouble(dtr2[11]);

                            // sumc1 = sumc1 + Convert.ToDouble(dtr2[11]);
                           // sumd1 = sumd1 + Convert.ToDouble(dtr2[11]);
                        }
                        ...
                        dr[0] = ""; //dr[1] = dtra[0].ToString(); 
                        dr[1] = " - - - - - "; dr[2] = "  -  -    ";
                        dr[3] = "  -  -    ";  //dr[5] = "------------------------------";
                        // dr[0] = "";
                        dr[4] = "اجـمــــالـي عقد رقم" + " " + dtra[0].ToString(); dr[5] = sumc; dr[6] = 0; dr[7] = true;

                        dr[8] = ""; dr[9] = ""; dr[10] = "";// dr[11] = "";
                        //dr["a_p"] = true; dr[13] = dtra[7].ToString(); dr[14] = dtra[8].ToString(); dr["sl"] = dtra[9].ToString();
                    ...
                    foreach (DataRow dtr2 in dtdtlall.Rows)
                    {
                        sumc1 = sumc1 + (string.IsNullOrEmpty(dtr2[0].ToString()) ? Convert.ToDouble(dtr2[5]) : 0.00);
                    }

                    DataRow dr2 = dtdtlall.NewRow();
                    dr2[0] = ""; dr2[1] = ""; dr2[2] = "  -  -    ";
                    dr2[3] = "  -  -    ";  //dr[5] = "------------------------------";
                    // dr[0] = "";
                    dr2[4] = "اجـمــــالـي الرصيد " + " " + ""; dr2[5] = Convert.ToDouble(txt_opnbal.Text) + sumc1; dr2[6] = 0; dr2[7] = true;

                    dr2[8] = ""; dr2[9] = ""; dr2[10] = "";// dr[11] = "";
                    //dr["a_p"] = true; dr[13] = dtra[7].ToString(); dr[14] = dtra[8].ToString(); dr["sl"] = dtra[9].ToString();

                    dtdtlall.Rows.Add(dr2);
                    ...
                    */
                    ...
                    foreach (DataGridViewRow rw in dataGridView1.Rows)
                    {
                      ...
                      //  rw.Cells["c1"].Value = rw.Cells["a_t"].Value.ToString().Equals("01") ? "ق عام" : rw.Cells["a_t"].Value.ToString().Equals("02") ? "س قبض" : "س صرف";
                        if (!(rw.Cells["c1"].Value).ToString().Trim().Equals(""))
        ...
        private void dataGridView1_KeyDown_1(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F9)
            {
                ...
                {
                    Acc.Acc_Tran f4 = new Acc.Acc_Tran(dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString(), "");
                ...
                {
                    Acc.Acc_Tax_Tran f4 = new Acc.Acc_Tax_Tran(dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString(), "");
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Acc.Acc_Tax_Tran2 f4 = new Acc.Acc_Tax_Tran2(dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString(), "");
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Sal.Sal_Ofr_Tran_E2 f4 = new Sal.Sal_Ofr_Tran_E2(dataGridView1.CurrentRow.Cells["sl"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                    f4.MdiParent = ParentForm;
                   // if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Cus.Acc_Tran f4 = new Cus.Acc_Tran(dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Cus.Acc_Tran_Q f4 = new Cus.Acc_Tran_Q(dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                if (dataGridView1.CurrentRow.Cells["src"].Value.Equals("Sal1"))
                {
                    ...
                    {
                        Sal.Sal_Tran_D_TF f4 = new Sal.Sal_Tran_D_TF(dataGridView1.CurrentRow.Cells["sl"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                        f4.MdiParent = ParentForm;
                        if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                            f4.Show();
                    }
                    ...
                    {
                        Sal.Sal_Tran_D f4 = new Sal.Sal_Tran_D(dataGridView1.CurrentRow.Cells["sl"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                        f4.MdiParent = ParentForm;
                        if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                            f4.Show();
                    }
                ...
                {
                    Sal.Sal_Tran_notax f4 = new Sal.Sal_Tran_notax(dataGridView1.CurrentRow.Cells["sl"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Pur.Pur_Tran_D f4 = new Pur.Pur_Tran_D(dataGridView1.CurrentRow.Cells["pu"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Pur.Pur_Tran_notax f4 = new Pur.Pur_Tran_notax(dataGridView1.CurrentRow.Cells["pu"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["rf"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
          ...
          ////  rf.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("details", dtprt));
            ...
            rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Cus.rpt.Cust_Balance_Dtl2_rpt.rdlc";
            ...
            parameters[3] = new ReportParameter("desc", "نموذج مطابقة الرصيد");
            parameters[4] = new ReportParameter("cu_name", txt_name.Text);
            parameters[5] = new ReportParameter("cur_bal", Math.Round(seso, 2).ToString());
            BL.ToWord toWord = new BL.ToWord(Convert.ToDecimal(seso < 0 ? seso * -1 : seso), currencies[Convert.ToInt32(BL.CLS_Session.cur)]);
            parameters[6] = new ReportParameter("cur_bal_w", BL.CLS_Session.lang.Equals("ع") ? toWord.ConvertToArabic() : toWord.ConvertToEnglish());

           // parameters[7] = new ReportParameter("cur_bal_w", BL.CLS_Session.brname);
           // parameters[8] = new ReportParameter("br_name", BL.CLS_Session.brname);
        ...
        private void txt_code_MouseUp(object sender, MouseEventArgs e)
        {
