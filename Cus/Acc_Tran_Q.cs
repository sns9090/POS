...
namespace POS.Cus
{
    public partial class Acc_Tran_Q : BaseForm
    {
        List<BL.CurrencyInfo> currencies = new List<BL.CurrencyInfo>();
       // DataTable dtg;
        DataTable dthdr, dtdtl, dttrtyps,dtcolman;
        string a_ref, a_type;
        ...
        public Acc_Tran_Q(string atype, string aref)
        {
            InitializeComponent();
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.F8 && dataGridView1.ReadOnly==false)
                {
                    ...
                    Search_All f4 = new Search_All("5","Cus1");
                    f4.ShowDialog();
             ...
             try
                {
                    if (dataGridView1.CurrentCell == dataGridView1[0, dataGridView1.CurrentRow.Index])
                    {
                        ...
                        SqlDataAdapter da = new SqlDataAdapter("select top 1 cu_no  ,cu_name ,cl_acc from customers a join cuclass b on a.cu_class=b.cl_no where a.inactive=0 and a.cu_no = '" + dataGridView1.CurrentRow.Cells[0].Value + "' and a.cu_brno='" + BL.CLS_Session.brno + "'", con);
...
/*
                ...
                */
                ...
                // MessageBox.Show(tr);
                dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('02','09')");
                ...
                string whatever = str2.Substring(str2.IndexOf("C121") + 5, 3);
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (BL.CLS_Session.mustequal && Convert.ToDouble(txt_amt.Text) != Convert.ToDouble(txt_camt.Text))
                {
                ...
                {
                    daml.SqlCon_Open();
                    int exexcuted = daml.Insert_Update_Delete_retrn_int("update acc_hdr set lastupdt=getdate(),a_mdate='" + mdate + "', a_hdate='" + hdate + "',a_text='" + txt_desc.Text + "',a_amt=" + txt_amt.Text + ",a_entries=" + (dataGridView1.RowCount - 1) + ",mainkey='" + txt_key.Text + "',usrid='" + txt_user.Text + "',jhcustno='" + (cmb_colman.SelectedIndex != -1 ? cmb_colman.SelectedValue.ToString() : "") + "',serial_no='" + txt_bookno.Text + "',aqd_no='" + txt_aqd.Text + "',jhlccrttl=" + txt_discnt.Text + ",cc_no='" + txt_ccno.Text + "' where a_ref=" + txt_ref.Text + " and a_type='" + comboBox1.SelectedValue + "' and a_brno='" + BL.CLS_Session.brno + "'", false);
                ...
                using (SqlCommand cmd = new SqlCommand("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cc_no) VALUES(@a0,@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11)", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@a6", Convert.ToDouble(txt_discnt.Text) > 0 ? Convert.ToDouble(txt_amt.Text) - Convert.ToDouble(txt_discnt.Text) : Convert.ToDouble(txt_amt.Text));
                ...
                if (Convert.ToDouble(txt_discnt.Text) > 0)
                {
                    using (SqlCommand cmd = new SqlCommand("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cc_no) VALUES(@a0,@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11)", con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@a1", comboBox1.SelectedValue);
                        cmd.Parameters.AddWithValue("@a2", (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)));
                        cmd.Parameters.AddWithValue("@a3", BL.CLS_Session.dtbranch.Rows[0]["dscalwser"]);
                        cmd.Parameters.AddWithValue("@a4", "خصم ممنوح");
                        // cmd.Parameters.AddWithValue("@a4", row.Cells[2].Value);
                        cmd.Parameters.AddWithValue("@a5", 0);
                        // cmd.Parameters.AddWithValue("@a5", (string.IsNullOrEmpty(row.Cells[3].Value.ToString())? 0 : Convert.ToDouble(row.Cells[3].Value)));
                        cmd.Parameters.AddWithValue("@a6", txt_discnt.Text);
    ...
    */
                    if (!row.IsNewRow && !string.IsNullOrEmpty(row.Cells[0].Value.ToString()))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cu_no,cc_no) VALUES(@a0,@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12)", con))
                        {
                            cmd.Parameters.AddWithValue("@a0", BL.CLS_Session.brno);
                            cmd.Parameters.AddWithValue("@a1", comboBox1.SelectedValue);
                            cmd.Parameters.AddWithValue("@a2", (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)));
        ...
        private void btn_Edit_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!dthdr.Rows[0][8].ToString().Equals("Cus1"))
                {
                ...
                if (btn_Post.Enabled == true)
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                        if (string.Equals(row.Cells[0].Value as string, txt_key.Text)  || string.Equals(row.Cells[0].Value as string, BL.CLS_Session.dtbranch.Rows[0]["dscalwser"]))
                            dataGridView1.Rows.Remove(row);
                    }
                    dataGridView1.AllowUserToAddRows = true;
                }
        ...
        private void Fill_Data(string atype, string aref)
        {
            try
            {
              ...
              //  hdate = txt_hdate.Text.Replace("-", "").Substring(4, 4) + txt_hdate.Text.Replace("-", "").Substring(2, 2) + txt_hdate.Text.Replace("-", "").Substring(0, 2);
                ...
                dthdr = daml.SELECT_QUIRY_only_retrn_dt("select * from acc_hdr where a_type='" + atype + "' and a_ref=" + aref + " and a_brno='" + BL.CLS_Session.brno + "' and jhsrc='Cus1'");
                dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select aa.cu_no a_key,bb.cu_name a_name,aa.a_text a_text,aa.a_damt a_damt,aa.a_camt a_camt,aa.a_key acc_key  from acc_dtl aa,customers bb where aa.cu_no=bb.cu_no and aa.a_brno=bb.cu_brno and aa.a_type='" + atype + "' and aa.a_ref=" + aref + " and aa.a_brno='" + BL.CLS_Session.brno + "' and aa.cu_no<>'' order by aa.a_folio");
                ...
                // load_key();
                 if (dthdr.Rows[0]["mainkey"].ToString().Length > 0 )
                 {
                     ...
                     txt_aqd.Text = dthdr.Rows[0]["aqd_no"].ToString();
        ...
        //    doc = null;
            try
            {
                ...
                if (File.Exists(Directory.GetCurrentDirectory() + @"\reports\Acc_Tran_Q_rpt.rdlc"))
                {
                    rf.reportViewer1.LocalReport.ReportPath = Directory.GetCurrentDirectory() + @"\reports\Acc_Tran_Q_rpt.rdlc";
                }
                else
                {
        ...
        private void Acc_Tran_Q_Shown(object sender, EventArgs e)
        {
