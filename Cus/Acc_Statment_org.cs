...
namespace POS.Cus
{
    public partial class Acc_Statment_org : BaseForm
    {
      ...
      //  DataTable dtt;
       // double seso = 0;
        ...
        public Acc_Statment_org(string akey)
        {
            InitializeComponent();
            ...
            try
            {
                if (txt_code.Text != "")
                {
                    ...
                    */
                    ...
                    dtpi = daml.SELECT_QUIRY_only_retrn_dt("SELECT [a_brno],[a_type],[a_ref],[qst_ref],[qst_sl] FROM [acc_dtl]  where qst_ref<>0 and a_brno='" + BL.CLS_Session.brno + "' and cu_no='" + txt_code.Text + "' and a_type='02'");
                    string condpi = chk_payed_inv.Checked ? " and qst_ref=0 and a.cu_no='" + txt_code.Text + "' " : " ";
                   // string qstr2 = "select a.a_type c1,a.a_ref c2,CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3,CONVERT(VARCHAR(10), CAST(a.a_hdate as date), 105) c9,a.a_text c4,round((case a.jddbcr when 'C' then a.a_camt else 0 end),2) c5,round((case a.jddbcr when 'D' then a.a_damt else 0 end),2) c6,b.a_posted,a.a_type a_t from acc_dtl a,acc_hdr b  where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type " + condp + conddm + " and " + cond + " <= '" + txt_tdate.Text.Replace("-", "") + "' and a.cu_no='" + txt_code.Text + "' and a.a_type not in('04','14') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate";
                    string qstr2 = "select a.a_type c1,cast(a.a_ref as varchar) c2,CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3,(substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else 0 end),2) c6,round((case a.jddbcr when 'C' then a.a_camt else 0 end),2) c7,b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu from acc_dtl a,acc_hdr b  where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.sl_no=b.sl_no " + condp + conddm + condt + condpi + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' and a.cu_no='" + txt_code.Text + "' and a.a_type not in('04','14') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate,a.a_sysdate";
                   
                  //  dth = daml.SELECT_QUIRY_only_retrn_dt("select cu_name a_name,round(cu_opnbal,2) a_opnbal,cu_no a_key from customers where cu_no='" + txt_code.Text + "' and cu_brno='" + BL.CLS_Session.brno + "'");
                    dth = daml.SELECT_QUIRY_only_retrn_dt("select a.cu_name a_name,round((a.cu_opnbal + (select isnull(sum(b.a_damt-b.a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and c.a_mdate < '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' where a.cu_no=b.cu_no and b.a_type not in ('04','14'))),2) a_opnbal,a.cu_no a_key  from customers a where a.cu_no='" + txt_code.Text + "' and a.cu_brno='" + BL.CLS_Session.brno + "'");
                   ...
                   // dataGridView1.DataSource = daml.SELECT_QUIRY_only_retrn_dt(qstr2);
                    ...
                    if (dth.Rows.Count > 0)
                    {
                        ...
                        if (dtpi.Rows.Count > 0 && chk_payed_inv.Checked)
                        {
                            ...
                            {
                                foreach (DataRow rw in dtn.Rows)
                                {
                                    if (rw["c2"].ToString().Trim().Equals(rt[3].ToString()) && rw["sl"].ToString().Trim().Equals(rt[4].ToString()) && rw["a_t"].ToString().Trim().Equals("05"))
                                    {
                                        rw.Delete();
                                    }
                                }
                    ...
                    for (int i = 0; i < dataGridView1.Rows.Count - 1 ; ++i)
                    {
                        if (i == 0)
                        {
                            seso = Convert.ToDouble(dth.Rows[0][1]) + Convert.ToDouble(dataGridView1.Rows[i].Cells["c6"].Value.ToString().Trim()) - Convert.ToDouble(dataGridView1.Rows[i].Cells["c7"].Value.ToString().Trim());
                           // dataGridView1.Rows[i].Cells["c8"].Value = seso > 0 ? Math.Round(seso, 4).ToString() : 0.ToString();
                            dataGridView1.Rows[i].Cells["c8"].Value = seso > 0 ? Math.Round(seso, 4).ToString() : Math.Round(seso, 4).ToString();
                           // dataGridView1.Rows[i].Cells["c9"].Value = seso < 0 ? (Math.Round(seso, 4) * -1).ToString() : 0.ToString();
                        }
                        else
                        {
