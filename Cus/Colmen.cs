...
namespace POS.Cus
{
    public partial class Colmen : BaseForm
    {
       // SqlConnection con1 = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
            ...
            if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        SqlDataAdapter da = new SqlDataAdapter("select * from acc_hdr where jhcustno='" + todel + "' and a_brno='" + BL.CLS_Session.brno + "'", con1);
                        DataTable dt = new DataTable();
                        da.Fill(dt);

                        if (dt.Rows.Count >= 1)
                        {
                            MessageBox.Show(" لا يمكن حذف محصل مرتبط بحركات ", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else
                        {
        ...
        private void Groups_Load(object sender, EventArgs e)
        {
          ...
          //  sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
            ...
            // sqlConnection.Open();
            sqlDataAdapter = new SqlDataAdapter("select sp_brno,sp_id,sp_name,sp_short,sp_percent,sp_target from col_men where sp_brno='" + BL.CLS_Session.brno + "' and sp_name like '%" + textBox1.Text + "%' order by sp_id", sqlConnection);
