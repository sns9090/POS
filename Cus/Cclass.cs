...
namespace POS.Cus
{
    public partial class Cclass : BaseForm
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlDataAdapter da = new SqlDataAdapter("select * from customers where cu_class='" + todel + "'", con1);
                DataTable dt = new DataTable();
                da.Fill(dt);

                if (dt.Rows.Count >= 1)
                {
                    MessageBox.Show(" لا يمكن حذف تصنيف لدية عملاء مرتبطين ", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
        ...
        private void Groups_Load(object sender, EventArgs e)
        {
            ...
            // sqlConnection.Open();
            sqlDataAdapter = new SqlDataAdapter("select cl_brno,cl_no,cl_desc,cl_acc from cuclass where cl_brno='" + BL.CLS_Session.brno + "' and cl_desc like '%" + textBox1.Text + "%' order by cl_no", sqlConnection);
