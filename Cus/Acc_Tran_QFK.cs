...
namespace POS.Cus
{
    public partial class Acc_Tran_QFK : BaseForm
    {
       // DataTable dtg;
        ...
        public Acc_Tran_QFK(string atype, string aref)
        {
            InitializeComponent();
        ...
        private void Acc_Tran_Load(object sender, EventArgs e)
        {
           ...
           // dtg = dataGridView1.DataSource;
            ...
            try
            {
                ...
                cmb_type.DataSource = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr1 + ") and tr_no in('04')");
...
/*
                ...
                */
                ...
                string whatever = str2.Substring(str2.IndexOf("C123") + 5, 3);
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //{
                    using (SqlCommand cmd = new SqlCommand("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,qst_ref,qst_sl) VALUES(@a0,@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12)", con))
                    {
                        cmd.Parameters.AddWithValue("@a0", BL.CLS_Session.brno);
                        cmd.Parameters.AddWithValue("@a1", comboBox1.SelectedValue);
                        cmd.Parameters.AddWithValue("@a2", (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)));
                        cmd.Parameters.AddWithValue("@a3", strdcash);
                        cmd.Parameters.AddWithValue("@a4", "دفعة من فاتورة مقسطة رقم" + " " + txt_fno.Text);
                        // cmd.Parameters.AddWithValue("@a4", row.Cells[2].Value);
        ...
        private void Fill_Data(string atype, string aref)
        {
            try
            {
              ...
              //  hdate = txt_hdate.Text.Replace("-", "").Substring(4, 4) + txt_hdate.Text.Replace("-", "").Substring(2, 2) + txt_hdate.Text.Replace("-", "").Substring(0, 2);
                ...
                dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select * from acc_dtl aa where aa.qst_ref<>0 and aa.a_type='" + atype + "' and aa.a_ref=" + aref + " and aa.a_brno='" + BL.CLS_Session.brno + "' order by aa.a_folio");
        ...
        {
            DataTable dt = daml.SELECT_QUIRY_only_retrn_dt("select * from sales_hdr where invpaid is not null and invpaid<>0 and invtype='" + cmb_type.SelectedValue + "' and slcenter='" + cmb_salctr.SelectedValue + "' and branch='" + BL.CLS_Session.brno + "' and ref=" + txt_fno.Text + "");
            if (dt.Rows.Count == 0)
            {
