        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
              ...
              //  string cond = checkBox1.Checked ? "b.a_hdate" : "b.a_mdate";
                ...
                string conddc = chk_m.Checked ? " having round((select isnull(a.cu_opnbal+sum(isnull(case when b.a_type not in ('04','14') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('04','14') then b.a_camt else 0 end ,0)),0)),4)<0 " : (chk_d.Checked ? " round((select isnull(a.cu_opnbal+sum(isnull(case when b.a_type not in ('04','14') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('04','14') then b.a_camt else 0 end ,0)),0)),4)>0 " : " having round((select isnull(a.cu_opnbal+sum(isnull(case when b.a_type not in ('04','14') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('04','14') then b.a_camt else 0 end ,0)),0)),4)<>0 ");
               ...
               // string qstr = "select a.cu_no d1,a.cu_name d2,a.cu_opnbal d3,round(( isnull(sum(b.a_camt),0)),2) d4,round(( isnull(sum(b.a_damt),0)),2) d5,round(( isnull(a.cu_opnbal+sum(b.a_damt-b.a_camt),0)),2) d6,round(( isnull(max(iif(b.a_camt>0 ,b.a_camt,0)),0)),2) d7,( isnull(max(iif(b.a_camt>0 ,b.a_mdate,'##-##-####')),'##-##-####')) d8 from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and b.a_type not in ('04','14') right Outer join customers a on a.cu_brno=b.a_brno and a.cu_no=b.cu_no where a.cu_no<>'' " + condc + condcity + condp + " group by a.cu_no ,a.cu_name ,a.cu_opnbal " + conddc + "";
                string qstr = "select a.cu_no d1,a.cu_name d2,a.cu_opnbal d3,round((isnull(sum(case when b.a_type in ('04','14') then b.a_camt + b.a_damt else 0 end),0)),4) d4,round(( isnull(sum(case when b.a_type in ('05','15') then b.a_camt + b.a_damt else 0 end),0)),4) d5,round(( isnull(sum(case when b.a_type in ('04','05','14','15') then b.a_camt + b.a_damt else 0 end),0)),4) d6,round(( isnull(sum(case when b.a_type not in ('04','05','14','15') then b.a_camt + b.a_damt else 0 end),0)),4) d7,round((select isnull(a.cu_opnbal+sum(isnull(case when b.a_type not in ('04','14') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('04','14') then b.a_camt else 0 end ,0)),0)),4) d8 from customers a left outer join acc_dtl b  inner join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no on a.cu_brno=b.a_brno and a.cu_no=b.cu_no where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' and a.cu_brno='" + BL.CLS_Session.brno + "' " + condc + condcity + condp + condcn + " group by a.cu_no ,a.cu_name ,a.cu_opnbal " + conddc + " order by cast(a.cu_no as int)";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //  rf.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("details", dtdtl));
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Cus.rpt.Cus_Balance_Sumry_rpt.rdlc";
