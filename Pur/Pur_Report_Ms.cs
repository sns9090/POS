...
namespace POS.Pur
{
    public partial class Pur_Report_Ms : BaseForm
    {
      ...
      //  DataTable dtt;
       ...
       // string typeno = "";
            ...
            try
            {
                ...
                */
                if (!chk_notequal.Checked && !chk_forrviw.Checked)
                {
                    ...
                    string conslctr = cmb_salctr.SelectedIndex != -1 ? " and pucenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " and branch + pucenter in(" + sl + ") ";
                    ...
                    string qstr = "select pucenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,case when supno='' then text else su_name end  a_text,(case when invtype in ('06','07') then (case when with_tax=0 then invttl else invttl-tax_amt_paid end) else -(case when with_tax=0 then invttl else invttl-tax_amt_paid end) end) invttl,(case when invtype in ('06','07') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('06','07') then (wst_amt) else -(wst_amt) end) totalms,(case when invtype in ('06','07') then (nettotal-invdsvl+wst_amt) else -(nettotal-invdsvl+wst_amt) end) nettotal,posted a_p,invtype a_t,glser src from pu_hdr left join suppliers on pu_hdr.supno=suppliers.su_no and pu_hdr.branch=suppliers.su_brno  where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";

                    if (!string.IsNullOrEmpty(txt_desc.Text))
                    {
                        qstr = qstr + " and text + su_name like '%" + txt_desc.Text + "%'";
                    }
                ...
                else 
                {
                    ...
                    {
                        string qstr = @"select pucenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('06','07') then (case when with_tax=0 then invttl+wst_amt else invttl+wst_amt-tax_amt_paid end) else -(case when with_tax=0 then invttl+wst_amt else invttl+wst_amt-tax_amt_paid end) end) invttl,(case when invtype in ('06','07') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('06','07') then (tax_amt_paid + etax) else -(tax_amt_paid + etax) end) tax,(case when invtype in ('06','07') then nettotal else -nettotal end) nettotal,posted a_p,invtype a_t,glser src
                                    FROM pu_hdr t1 where exists
                                        (
                                   SELECT a_brno,pu_no,a_type,a_ref, sum(a_damt) damt, sum(a_camt) camt
                                   FROM acc_dtl t2
                                   where t1.branch = t2.a_brno and t1.pucenter=t2.pu_no and t1.invtype=t2.a_type and t1.ref=t2.a_ref
                                   GROUP BY  a_brno,pu_no,a_type,a_ref
                    ...
                    {
                        string qstr = @"select pucenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('06','07') then (case when with_tax=0 then invttl+wst_amt else invttl+wst_amt-tax_amt_paid end) else -(case when with_tax=0 then invttl+wst_amt else invttl+wst_amt-tax_amt_paid end) end) invttl,(case when invtype in ('06','07') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('06','07') then (tax_amt_paid + etax) else -(tax_amt_paid + etax) end) tax,(case when invtype in ('06','07') then nettotal else -nettotal end) nettotal,posted a_p,invtype a_t,glser src
                                    FROM pu_hdr t1 where not exists
                                        (select * from acc_hdr t2 where t1.branch = t2.a_brno and t1.pucenter=t2.pu_no and t1.invtype=t2.a_type and t1.ref=t2.a_ref) and branch='" + BL.CLS_Session.brno + "' ";
                        dt = daml.SELECT_QUIRY_only_retrn_dt(qstr + " order by invmdate");
                    }
                    DataRow dr = dt.NewRow();
                    double sumopnd = 0, sumopnm = 0, sumtrd = 0, sumtrm = 0, sumttld = 0, sumttlm = 0;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                */
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Pur.rpt.Pur_Report_Ms_rpt.rdlc";
        ...
        private void chk_notequal_CheckedChanged(object sender, EventArgs e)
        {
            ...
            //else
            //    chk_forrviw.Checked = true;
        }

        private void txt_fdate_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
