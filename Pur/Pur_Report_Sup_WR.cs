...
namespace POS.Pur
{
    public partial class Pur_Report_Sup_WR : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                */
                ...
                string condft = " and " + (chk_sref.Checked ? "invsupno" : "ref") + " between " + condf + " and " + condt + " ";
               ...
               // string qstr = "select pucenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(invhdate as date), 105) a_hdate,text a_text,invttl invttl,invdsvl invdsvl,tax_amt_rcvd tax,nettotal nettotal,posted a_p,invtype a_t,round(((invttl - invdsvl) - invcst),2) invwin,('%' + CAST(round((((invttl - invdsvl) - invcst)/(invttl - invdsvl))*100,2) as varchar)) invwinp  from sales_hdr where branch='" + BL.CLS_Session.brno + "' and custno='" + txt_code.Text + "' " + condp + usrname + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                string qstr = "select pucenter sl_no,invtype a_type,ref a_ref,invsupno s_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('06','07') then invttl else -invttl end) invttl,(case when invtype in ('06','07') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('06','07') then (tax_amt_paid + etax) else -(tax_amt_paid + etax) end)  tax,(case when invtype in ('06','07') then nettotal else -nettotal end) nettotal,posted a_p,invtype a_t,glser src,ref a_r  from pu_hdr where branch='" + BL.CLS_Session.brno + "' and supno='" + txt_code.Text + "' " + condp + usrname + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                ...
                {
                    sumopnd = sumopnd + Convert.ToDouble(dtr[7]);
                    sumopnm = sumopnm + Convert.ToDouble(dtr[8]);
                    sumtrd = sumtrd + Convert.ToDouble(dtr[9]);
                    sumtrm = sumtrm + Convert.ToDouble(dtr[10]);
                ...
                //  dr[4] = "";
                dr[6] = "الاجمالي";
                dr[7] = Math.Round(sumopnd, 2);
                dr[8] = Math.Round(sumopnm, 2);
                dr[9] = Math.Round(sumtrd, 2);
                dr[10] = Math.Round(sumtrm, 2);
                dr[11] = true;
                // dr[11] = " ";
               ...
               // dr[13] = "%" + sumttlm.ToString();
                ...
                {
                    if (Convert.ToBoolean(rw.Cells[11].Value) == false)
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Pur.rpt.Pur_Report_MN_rpt.rdlc";
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F9)
            {
                ...
                {
                    //  Pur.Pur_Tran_D f4 = new Pur.Pur_Tran_D(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[2].Value.ToString());
                    Pur.Pur_Tran_D f4 = new Pur.Pur_Tran_D(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_r"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Pur.Pur_Tran_notax f4 = new Pur.Pur_Tran_notax(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_r"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
