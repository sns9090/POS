...
namespace POS.Pur
{
    public partial class Pur_Report_Gr : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //  string usrname = cmb_user.SelectedIndex != -1 ? " and usrid = '" + cmb_user.SelectedValue.ToString() + "' " : " ";
                string condst = cmb_salctr.SelectedIndex != -1 ? " and b.pucenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " and b.branch + b.pucenter in(" + sl + ") ";
              ...
              //  string condp = rad_posted.Checked ? " and posted=1 " : (rad_notpostd.Checked ? " and posted=0 " : " ");
                string condbr = chk_allbr.Checked ? "" : " and b.branch='" + BL.CLS_Session.brno + "' ";
              //  string condf = txt_fno.Text != "" ? txt_fno.Text : "1";
               ...
               // string conslctr = cmb_salctr.SelectedIndex != -1 ? " and slcenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " ";
                string condinvtype = cmb_type.SelectedIndex != -1 ? " and b.invtype = '" + cmb_type.SelectedValue.ToString() + "' " : " ";
              ...
              //
                ...
                if (cmb_grp.SelectedIndex == -1)
                {
                    ...
                    qstr = "  Select g.group_id br_no,g.group_name br_name,nettotal= isnull(round(sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*(b.price-(b.price*b.discpc/100)))-(case when a.with_tax=1 then b.tax_amt else 0 end)) "
                        // + ",tax=round(sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*b.pkqty)),2) "
                        + "+sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.tax_amt))-sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*(b.price-(b.price*b.discpc/100))*(a.invdspc/(100)))),2),0)"
                        ...
                        + "on  a.branch = b.branch and a.invtype = b.invtype and a.ref = b.ref and a.pucenter=b.pucenter and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condst + " on s.item_no=b.itemno on g.group_id=s.item_group"
                        + "  where 1=1 " + condgrp + condsgrp + " and g.group_pid is null group by g.group_id,g.group_name";
                ...
                else
                {
                    ...
                    qstr = "  Select g.group_id br_no,g.group_name br_name,nettotal= isnull(round(sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*(b.price-(b.price*b.discpc/100)))-(case when a.with_tax=1 then b.tax_amt else 0 end)) "
                       ...
                       +",tax=isnull(round(sum((case when b.invtype in ('06','07') then 1 else -1 end)*(b.qty*b.pkqty)),2),0) "
                       + ",salcost = 0 ,win = 0.00 ,winp ='' from groups g with (NOLOCK) left Join items s with (NOLOCK) inner Join pu_hdr a with (NOLOCK) inner Join pu_dtl b with (NOLOCK) "
                       + "on  a.branch = b.branch and a.invtype = b.invtype and a.ref = b.ref and a.pucenter=b.pucenter and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condst + " on s.item_no=b.itemno on g.group_id=s.sgroup"
                       + "  where 1=1 " + condgrp + condsgrp + " and g.group_pid is not null group by g.group_id,g.group_name";

                }

                ////////if (cmb_grp.SelectedIndex == -1)
                ////////    qstr = "Select a.item_group br_no,b.group_name br_name, nettotal=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-(case when h.with_tax=1 then c.tax_amt else 0 end))*IIF(c.invtype in ('06','07'),1,-1)),2)+round(sum((case when c.invtype in ('06','07') then 1 else -1 end)*(c.tax_amt)),2),salcost = round(sum(case when c.invtype in ('06','07') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end),2) ,win=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-(case when h.with_tax=1 then c.tax_amt else 0 end))*IIF(c.invtype in ('06','07'),1,-1))-sum((invdsvl/(nettotal+invdsvl))*iif(1=1,((c.QTY*(c.price-(c.price*c.discpc/100)))-(case when h.with_tax=1 then c.tax_amt else 0 end)),((c.QTY*(c.price-Floor(c.price*c.discpc/100)))-(case when h.with_tax=1 then c.tax_amt else 0 end)))*(case when h.invtype in ('06','07') then 1 else -1 end))-sum(case when c.invtype in ('06','07') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end)  ,2),winp='' from  items a join groups b on a.item_group = b.group_id join pu_dtl c on a.item_no=c.itemno join pu_hdr h on c.[branch]=h.[branch] and c.[pucenter]=h.[pucenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condgrp + condsgrp + condst + "  group By  a.item_group,b.group_name order By a.item_group";
                ////////else
                ////////    qstr = "Select a.sgroup br_no,b.group_name br_name, nettotal=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-(case when h.with_tax=1 then c.tax_amt else 0 end))*IIF(c.invtype in ('06','07'),1,-1)),2)+round(sum((case when c.invtype in ('06','07') then 1 else -1 end)*(c.tax_amt)),2),salcost = round(sum(case when c.invtype in ('06','07') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end),2) ,win=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-(case when h.with_tax=1 then c.tax_amt else 0 end))*IIF(c.invtype in ('06','07'),1,-1))-sum((invdsvl/(nettotal+invdsvl))*iif(1=1,((c.QTY*(c.price-(c.price*c.discpc/100)))-(case when h.with_tax=1 then c.tax_amt else 0 end)),((c.QTY*(c.price-Floor(c.price*c.discpc/100)))-(case when h.with_tax=1 then c.tax_amt else 0 end)))*(case when h.invtype in ('06','07') then 1 else -1 end))-sum(case when c.invtype in ('06','07') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end)  ,2),winp='' from  items a join groups b on a.sgroup = b.group_id and b.group_pid is not null join pu_dtl c on a.item_no=c.itemno join pu_hdr h on c.[branch]=h.[branch] and c.[pucenter]=h.[pucenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condgrp + condsgrp + condst + "  group By  a.sgroup,b.group_name order By a.sgroup";
              ...
              //  sumttlm = Math.Round((sumttld / (sumopnd - sumopnm)) * 100, 2);
                ...
                dr[4] = 0;// Math.Round(sumtrd, 2);
                dr[5] = 0;// Math.Round(sumtrm, 2);
               // dr[6] = "";
               ...
               // dr[13] = "%" + sumttlm.ToString();
                ...
                {
                    dtrg.Cells["Column6"].Value = 0;// Math.Round(((Convert.ToDouble(dtrg.Cells[4].Value)) - (Convert.ToDouble(dtrg.Cells[2].Value))), 2);
                    dtrg.Cells["Column7"].Value = "%" + "0";// Math.Round(((Convert.ToDouble(dtrg.Cells[5].Value)) / (Convert.ToDouble(dtrg.Cells[4].Value)) * 100), 2).ToString();
        ...
        private void SalesReport_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            //   dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('06','07','14','15')");
