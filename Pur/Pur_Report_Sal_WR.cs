...
namespace POS.Pur
{
    public partial class Pur_Report_Sal_WR : BaseForm
    {
       ...
       // string typeno = "";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                */
                string usrname = cmb_user.SelectedIndex != -1 ? " and c.usrid = '" + cmb_user.SelectedValue.ToString() + "' " : " ";
                ...
                string conslctr = cmb_salctr.SelectedIndex != -1 ? " and c.pucenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " ";
              ...
              //  string qstr = "select slcenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(invhdate as date), 105) a_hdate,text a_text,invttl invttl,invdsvl invdsvl,tax_amt_rcvd tax,nettotal nettotal,posted a_p,invtype a_t,round(((invttl - invdsvl) - invcst),2) invwin,('%' + CAST(round((((invttl - invdsvl) - invcst)/(invttl - invdsvl))*100,2) as varchar)) invwinp  from sales_hdr where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + " and " + cond + " between '" + txt_fdate.Text.Replace("-", "") + "' and '" + txt_tdate.Text.Replace("-", "") + "'";
                string qstr = "select c.pucenter sl_no,c.invtype a_type,c.ref a_ref,CONVERT(VARCHAR(10), CAST(c.invmdate as date), 105) a_mdate,(substring(c.invhdate,7,2) + '-' + substring(c.invhdate,5,2)+'-'+substring(c.invhdate,1,4)) a_hdate,c.text a_text,c.invttl invttl,c.invdsvl invdsvl,(c.tax_amt_paid + c.etax)  tax,c.nettotal nettotal,c.posted a_p,c.invtype a_t,round((b.qty * b.pkqty * i.item_price),2) inv_price,round(((b.qty * b.pkqty * i.item_price) - (b.qty * b.pkqty * b.fullcost)),2) inv_win,glser src from pu_hdr c join pu_dtl b on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and c.pucenter=b.pucenter join items i on i.item_no=b.itemno where c.branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                
                if (!string.IsNullOrEmpty(txt_desc.Text))
                {
                    qstr = qstr + " and c.text like '%" + txt_desc.Text + "%'";
                }

                if (!string.IsNullOrEmpty(txt_equl.Text))
                {
                  //  MessageBox.Show(comboBox2.SelectedValue.ToString());
                    qstr = qstr + " and c.invttl " + comboBox2.Text.Substring(0,1) + txt_equl.Text;
                }

                dt = daml.SELECT_QUIRY_only_retrn_dt(qstr + " and c.invtype like '%" + cmb_type.SelectedValue + "%' order by c.invmdate");
                ...
                foreach (DataRow dtr in dt.Rows)
                {
                    ...
                    sumttlm = sumttlm + Convert.ToDouble(dtr[13]); 
              ...
              //  dr[4] = "";
               ...
               // dr[11] = " ";
                ...
                dr[13] = Math.Round(sumttlm, 2);// "%" + sumttlm.ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                */
                ...
                workSheet.Range["E" + (dataGridView1.Rows.Count + 6).ToString(), "L" + (dataGridView1.Rows.Count + 6).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);

                workSheet.Range["A6", "L6"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // DataSet ds1 = new DataSet();
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Pur.rpt.Sal_Report_WR_rpt.rdlc";
