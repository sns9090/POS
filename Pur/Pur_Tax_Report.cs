       ...
       // string typeno = "";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                */
                ...
                string conds = rd_tax.Checked ? " and tax_amt_paid<>0 " : rd_ntax.Checked ? " and tax_amt_paid=0 " : " ";
                ...
                string conslctr = cmb_salctr.SelectedIndex != -1 ? " and pucenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " ";
               ...
               // string qstr = "select pucenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('06','07') then (case when with_tax=0 then invttl else invttl-tax_amt_paid end) else -(case when with_tax=0 then invttl else invttl-tax_amt_paid end) end) invttl,(case when invtype in ('06','07') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('06','07') then nettotal else -nettotal end) nettotal,(case when invtype in ('06','07') then (tax_amt_paid + etax) else -(tax_amt_paid + etax) end) tax,(case when supno<>'' then supno else casher end) su_no,(case when supno<>'' then su_name else 'مورد عادي' end) su_name,(case when vndr_taxcode<>'' then vndr_taxcode else taxid end) tax_no,posted a_p,invtype a_t,glser src,(case when invtype in ('06','07') then isnull(taxfree_amt,0) else -(isnull(taxfree_amt,0)) end) tf,(case when invtype in ('06','07') then (case when with_tax=0 then invttl else invttl-tax_amt_paid-etax end) - isnull(taxfree_amt,0) else -((case when with_tax=0 then invttl else invttl-tax_amt_paid-etax end) - isnull(taxfree_amt,0)) end) tt from pu_hdr left join suppliers on su_no=supno where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + conds + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                string qstr = "select pucenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('06','07') then (case when with_tax=0 then invttl*(iif(currate>0,currate,1)) else (invttl-tax_amt_paid)*(iif(currate>0,currate,1)) end) else -(case when with_tax=0 then invttl*(iif(currate>0,currate,1)) else (invttl-tax_amt_paid)*(iif(currate>0,currate,1)) end) end) invttl,(case when invtype in ('06','07') then invdsvl*(iif(currate>0,currate,1)) else -invdsvl*(iif(currate>0,currate,1)) end) invdsvl,(case when invtype in ('06','07') then nettotal*(iif(currate>0,currate,1)) else -nettotal*(iif(currate>0,currate,1)) end) nettotal,(case when invtype in ('06','07') then (tax_amt_paid + etax) else -(tax_amt_paid + etax) end) tax,(case when supno<>'' then supno else casher end) su_no,(case when supno<>'' then su_name else 'مورد عادي' end) su_name,(case when vndr_taxcode<>'' then vndr_taxcode else taxid end) tax_no,posted a_p,invtype a_t,glser src,(case when invtype in ('06','07') then isnull(taxfree_amt,0) else -(isnull(taxfree_amt,0)) end) tf,(case when invtype in ('06','07') then (case when with_tax=0 then nettotal-tax_amt_paid-etax else nettotal-tax_amt_paid-etax end) - isnull(taxfree_amt,0) else -((case when with_tax=0 then nettotal-tax_amt_paid-etax else nettotal-tax_amt_paid-etax end) - isnull(taxfree_amt,0)) end) tt,invsupno sno,with_tax wt,((case when invtype in ('06','07') then nettotal*(iif(currate>0,currate,1)) else -nettotal*(iif(currate>0,currate,1)) end) - (case when invtype in ('06','07') then (tax_amt_paid + etax) else -(tax_amt_paid + etax) end) ) ttlwotx from pu_hdr left join suppliers on su_no=supno where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + conds + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                ...
                double sumopnd = 0, sumopnm = 0, sumtrd = 0, sumtrm = 0, sumttld = 0, sumttlm = 0, sumtf = 0, sumttt = 0;
                foreach (DataRow dtr in dt.Rows)
                {
                    ...
                    sumtf = sumtf + Convert.ToDouble(dtr[16]);
                    sumttt = sumttt + Convert.ToDouble(dtr[17]);
              ...
              //  dr[4] = "";
                ...
                dr[12] = "";
                dr[13] = true;
                dr[16] = Math.Round(sumtf, 4);
                dr[17] = Math.Round(sumttt, 4);
                dr[18] = "";
                dr[19] = false;
               // dr[11] = "";
                ...
                {
                    if (Convert.ToBoolean(rw.Cells[13].Value) == false)
                   ...
                   // rw.Cells[0].Value = rw.Cells[0].Value.ToString().Equals("01") ? "ق عام" : rw.Cells[0].Value.ToString().Equals("02")? "س قبض" : "س صرف";
                    ...
                    if (Math.Round(Convert.ToDouble(rw.Cells[9].Value), 4) != (Convert.ToBoolean(rw.Cells["wt"].Value) ? Math.Round((((Convert.ToDouble(rw.Cells[6].Value) - Convert.ToDouble(rw.Cells[7].Value) + Convert.ToDouble(rw.Cells[9].Value)) * taxper) / (100 + taxper)), 4) : Math.Round(((Convert.ToDouble(rw.Cells[6].Value) - Convert.ToDouble(rw.Cells[7].Value)) * taxper / 100), 4)))
        ...
        private void SalesReport_Load(object sender, EventArgs e)
        {
         ...
         //   dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('04','05','14','15')");
            cmb_type.DataSource = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('06','07','16','17')");
            ...
            cmb_salctr.DataSource = daml.SELECT_QUIRY_only_retrn_dt("select * from pucenters where pu_brno + pu_no in(" + sl + ") and pu_brno='" + BL.CLS_Session.brno + "'");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                */
                ...
                workSheet.Range["E" + (dataGridView1.Rows.Count + 6).ToString(), "R" + (dataGridView1.Rows.Count + 6).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);

                workSheet.Range["A6", "S6"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                */
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sal.rpt.Sal_Tax_Report_rpt.rdlc";
                ...
                parameters[4] = new ReportParameter("name", "اسم المورد");
                parameters[5] = new ReportParameter("no", "رقم المورد");
                parameters[6] = new ReportParameter("hdr", this.Text);
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (dataGridView1.CurrentRow.Cells["src"].Value.ToString().Trim().Equals("Pur1"))
                {
                  //  Pur.Pur_Tran_D f4 = new Pur.Pur_Tran_D(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[2].Value.ToString());
                    Pur.Pur_Tran_D f4 = new Pur.Pur_Tran_D(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[2].Value.ToString());
                ...
                if (dataGridView1.CurrentRow.Cells["src"].Value.ToString().Trim().Equals("Pur2"))
                {
                    Pur.Pur_Tran_notax f4 = new Pur.Pur_Tran_notax(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[2].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }          
