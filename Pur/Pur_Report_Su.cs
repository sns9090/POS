    ...
    public partial class Pur_Report_Su : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //  string usrname = cmb_user.SelectedIndex != -1 ? " and usrid = '" + cmb_user.SelectedValue.ToString() + "' " : " ";
                ...
                string condc = cmb_class.SelectedIndex != -1 ? " and b.su_class='" + cmb_class.SelectedValue + "' " : " ";
                string condcity = cmb_city.SelectedIndex != -1 ? " and b.su_city='" + cmb_city.SelectedValue + "' " : " ";
              ...
              ////  qstr = "Select b.su_no br_no,b.su_name br_name, nettotal=round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2),tax=round(sum(case when a.invtype in ('06','07') then a.tax_amt_paid else -a.tax_amt_paid end),2),salcost = round(sum((case when a.invtype in ('06','07') then round(d.qty * d.pkqty * i.item_price,2) else - round(d.qty * d.pkqty * round(i.item_price,2),2) end)),2) ,win = round(sum((case when a.invtype in ('06','07') then round(d.qty * d.pkqty * i.item_price,2) else - round(d.qty * d.pkqty * round(i.item_price,2),2) end)),2) - round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2) ,winp =''  from  pu_hdr a join suppliers b on a.branch = b.su_brno and a.supno=b.su_no join pu_dtl d on a.branch=d.branch and a.pucenter=d.pucenter and a.invtype=d.invtype and a.ref=d.ref join items i on i.item_no=d.itemno where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condp + condinvtype + condft + condc + condcity + "  group By  b.su_no,b.su_name order By b.su_no";
                qstr = "Select b.su_no br_no,b.su_name br_name, nettotal=round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2),tax=round(sum(case when a.invtype in ('06','07') then a.tax_amt_paid else -a.tax_amt_paid end),2),salcost =round(sum(case when a.invtype in ('06','07') then a.nettotal else -(a.nettotal) end),2)   from  pu_hdr a join suppliers b on a.branch = b.su_brno and a.supno=b.su_no  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condp + condinvtype + condft + condc + condcity + "  group By  b.su_no,b.su_name order By b.su_no";
               
              //  else
               //     qstr = "Select a.branch br_no,b.br_name, nettotal=round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2),tax=round(sum(case when a.invtype in ('06','07') then a.tax_amt_paid else -a.tax_amt_paid end),2),salcost = round(sum((case when invtype in ('06','07') then round(a.invcst,2) else - round(a.invcst,2) end)),2) ,win = round(sum((case when a.invtype in ('06','07') then round(((a.invttl - a.invdsvl) - a.invcst),2) else -round(((a.invttl - a.invdsvl) - a.invcst),2) end)),2),winp = ('%' + cast( round(sum(case when a.invtype in ('06','07') then ((a.invttl - a.invdsvl) - a.invcst)/(a.invttl - a.invdsvl)*100 else ((a.invttl - a.invdsvl) - a.invcst)/(a.invttl - a.invdsvl)*100 end),2)  as varchar))  from  sales_hdr a join branchs b with (NOLOCK) on a.branch = b.br_no  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condp + condinvtype + "  group By  a.branch,b.br_name order By a.branch";
                //////if (rd_day.Checked)
                //////{
              ...
              //  dr[6] = "%" + ((Math.Round(sumopnd, 2) - Math.Round(sumtrd, 2)) / (Math.Round(sumopnd, 2)) * 100 ).ToString(); //round(sum(case when a.invtype in ('06','07') then ((a.invttl - a.invdsvl) - a.invcst)/(a.invttl - a.invdsvl)*100 else ((a.invttl - a.invdsvl) - a.invcst)/(a.invttl - a.invdsvl)*100 end),2)
               ...
               // dr[13] = "%" + sumttlm.ToString();
                ...
                ////    dtrg.Cells["Column7"].Value = "%" + Math.Round(((Convert.ToDouble(dtrg.Cells[5].Value)) / (Convert.ToDouble(dtrg.Cells[4].Value)) * 100), 2).ToString();
                ////    //  sumttlm = sumttlm + Convert.ToDouble(dtr[13].ToString().Replace("%",""));
                ////}
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Pur.rpt.Pur_Report_Su_rpt.rdlc";
        ...
        private void chk_allbr_CheckedChanged()
        {
        ...
        private void txt_fno_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F8)
            {
                // var selectedCell = dataGridView1.SelectedCells[0];
                // do something with selectedCell...
                Search_All f4 = new Search_All("7", "Cus");
                f4.ShowDialog();

                try
                {

                    txt_fno.Text = f4.dataGridView1.CurrentRow.Cells[0].Value.ToString();
                   // txt_name.Text = f4.dataGridView1.CurrentRow.Cells[1].Value.ToString();
                    //  dataGridView1.CurrentCell = this.dataGridView1[3, dataGridView1.CurrentRow.Index];
        ...
        private void txt_tno_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F8)
            {
                ...
                try
                {
                    ...
                    txt_tno.Text = f4.dataGridView1.CurrentRow.Cells[0].Value.ToString();
                    // txt_name.Text = f4.dataGridView1.CurrentRow.Cells[1].Value.ToString();
                    //  dataGridView1.CurrentCell = this.dataGridView1[3, dataGridView1.CurrentRow.Index];
