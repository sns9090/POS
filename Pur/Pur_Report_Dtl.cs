...
namespace POS.Pur
{
    public partial class Pur_Report_Dtl : BaseForm
    {
       ...
       // string typeno = "";
            ...
            try
            {
               ...
               // MessageBox.Show(datval.convert_to_yyyyDDdd(BL.CLS_Session.end_dt));
                if (!chk_notequal.Checked)
                {
                    ...
                    */
                    string usrname = cmb_user.SelectedIndex != -1 ? " and bb.usrid = '" + cmb_user.SelectedValue.ToString() + "' " : " ";
                    string cond = checkBox1.Checked ? "bb.invhdate" : "bb.invmdate";
                    string condp = rad_posted.Checked ? " and bb.posted=1 " : (rad_notpostd.Checked ? " and bb.posted=0 " : " ");

                    string condf = txt_fno.Text != "" ? txt_fno.Text : "1";
                    string condt = txt_tno.Text != "" ? txt_tno.Text : "999999999";
                    string condft = " and bb.ref between " + condf + " and " + condt + " ";
                    string condfdt = cmb_exityear.Checked ? " > '" + datval.convert_to_yyyyDDdd(BL.CLS_Session.end_dt) + "' or " + cond + " < '" + datval.convert_to_yyyyDDdd(BL.CLS_Session.start_dt) + "'" : " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                    string conslctr = cmb_salctr.SelectedIndex != -1 ? " and bb.pucenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " ";

                    //  string qstr = "select a_type a_type,a_ref a_ref,CONVERT(VARCHAR(10), CAST(a_mdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(a_hdate as date), 105) a_hdate,a_text a_text,a_amt a_amt,a_posted,a_type a_t from acc_hdr where jhsrc like 'Acc%' and a_brno='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + " and " + cond + " between '" + txt_fdate.Text.Replace("-", "") + "' and '" + txt_tdate.Text.Replace("-", "") + "'";
                    string qstr = "select bb.invtype a_type,bb.ref a_ref,CONVERT(VARCHAR(10), CAST(bb.invmdate as date), 105) a_mdate,(substring(bb.invhdate,7,2) + '-' + substring(bb.invhdate,5,2)+'-'+substring(bb.invhdate,1,4)) a_hdate,bb.text a_text,bb.invttl a_amt,bb.posted,bb.invtype a_t,bb.glser src,bb.pucenter sl from  pu_hdr bb  where bb.branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + " and " + cond + condfdt + "";


                    dt = daml.SELECT_QUIRY_only_retrn_dt(qstr + " and bb.invtype like '%" + cmb_type.SelectedValue + "%' order by bb.released");
                    DataTable dtdtl2 = new DataTable();
                    DataTable dtdtlall = new DataTable();
                    sumc1=0;  sumd1=0 ;
                    foreach (DataRow dtr in dt.Rows)
                    {
                        string qstr2 = "select aa.invtype a_type,cast(aa.ref  as varchar) a_ref,CONVERT(VARCHAR(10), CAST(aa.invmdate as date), 105) a_mdate,(substring(aa.invhdate,7,2) + '-' + substring(aa.invhdate,5,2)+'-'+substring(aa.invhdate,1,4)) a_hdate,aa.itemno a_itemno,cc.item_name a_name,dd.unit_name a_unit,aa.pkqty a_shd,round(aa.qty,2) a_qty,round(aa.price,2) a_price,round(aa.discpc,2) a_desc,round(((aa.price * aa.qty) - ((aa.price * aa.qty)*discpc/100)),2) a_total,bb.posted a_p,aa.invtype a_t,bb.glser src,aa.pucenter sl from pu_dtl aa join pu_hdr bb on aa.branch=bb.branch and aa.invtype=bb.invtype and aa.ref=bb.ref and aa.pucenter=bb.pucenter join items cc on  aa.itemno=cc.item_no join units dd on dd.unit_id= aa.pack where aa.branch='" + BL.CLS_Session.brno + "' and aa.invtype='" + dtr[0] + "' and aa.ref=" + dtr[1] + " and aa.pucenter='" + dtr[9] + "'";

                        if (!string.IsNullOrEmpty(txt_desc.Text))
                        {
                            qstr2 = qstr2 + " and bb.text like '%" + txt_desc.Text + "%' order by aa.invmdate";
                        }

                        if (!string.IsNullOrEmpty(txt_equl.Text))
                        {
                            //  MessageBox.Show(comboBox2.SelectedValue.ToString());
                            qstr2 = qstr2 + " and bb.invttl " + comboBox2.Text.Substring(0, 1) + txt_equl.Text + " or bb.nettotal " + comboBox2.Text.Substring(0, 1) + txt_equl.Text;
                        }
                     ...
                     //   dtdtl2 = daml.SELECT_QUIRY_only_retrn_dt("select aa.a_type a_type,cast(aa.a_ref  as varchar) a_ref,CONVERT(VARCHAR(10), CAST(aa.a_mdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(aa.a_hdate as date), 105) a_hdate,aa.a_key,cc.a_name a_name,aa.a_text a_text,round(aa.a_camt,2) a_camt,round(aa.a_damt,2) a_damt,bb.a_posted,aa.a_type a_t,bb.jhsrc src,aa.sl_no sl,aa.pu_no pu from acc_dtl aa join acc_hdr bb on aa.a_brno=bb.a_brno and aa.a_type=bb.a_type and aa.a_ref=bb.a_ref and aa.sl_no=bb.sl_no and aa.pu_no=bb.pu_no join accounts cc on aa.a_brno=cc.a_brno and aa.a_key=cc.a_key where aa.a_type='" + dtr[0] + "' and aa.a_ref='" + dtr[1] + "' and aa.sl_no='" + dtr[9] + "' and aa.pu_no='" + dtr[10] + "' and aa.a_brno='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + " and " + cond + condfdt + "");
                        ...
                        {
                            sumc = sumc + Convert.ToDouble(dtr2[8]);
                            sumd = sumd + Convert.ToDouble(dtr2[9]);
                       ...
                       // double sum = 0;
                        ...
                        dr[3] = "  -  -    "; dr[4] = "---------"; //dr[5] = "------------------------------";
                       // dr[0] = "";
                        dr[5] = "اجـمــــالـي حركة رقم" + " " + dtr[1].ToString(); dr[6] = 0; dr[7] = 0;

                        dr[8] = sumc; dr[9] = sumd; dr[10] = 0; dr[11] = sumd2; dr["a_p"] = true; dr[13] = dtr[7].ToString(); dr[14] = dtr[8].ToString(); dr["sl"] = dtr[9].ToString();
                    ...
                    foreach (DataGridViewRow rw in dataGridView1.Rows)
                    {
                        ...
                        if (!(rw.Cells["dataGridViewTextBoxColumn1"].Value).ToString().Trim().Equals(""))
                            rw.Cells["dataGridViewTextBoxColumn1"].Value = datval.validate_trtypes((rw.Cells["dataGridViewTextBoxColumn1"].Value).ToString().Trim());
                        else
            ...
            try
            {
             ...
             //   Acc.rpt.Acc_frmPrint rf = new Acc.rpt.Acc_frmPrint();
               ...
               // DataSet ds1 = new DataSet();
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sal.rpt.Sal_Report_Dtl_rpt.rdlc";
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F9)
            {
                ...
                {
                    Acc.Acc_Tran f4 = new Acc.Acc_Tran(dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[1].Value.ToString(),"");
                ...
                {
                    Acc.Acc_Tran_Q f4 = new Acc.Acc_Tran_Q(dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[1].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                    f4.Show();
                }
                ...
                {
                    Pur.Pur_Tran_D f4 = new Pur.Pur_Tran_D(dataGridView1.CurrentRow.Cells["sl"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[1].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
                ...
                {
                    Pur.Pur_Tran_notax f4 = new Pur.Pur_Tran_notax(dataGridView1.CurrentRow.Cells["sl"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[1].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
