...
namespace POS.Pur
{
    public partial class Pur_Report_Br : BaseForm
    {
            ...
            try
            {
              ...
              //
                ...
                if(chk_allsalctr.Checked)
                    qstr = "Select a.branch br_no,b.pu_name br_name, nettotal=round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2),tax=round(sum(case when a.invtype in ('06','07') then a.tax_amt_paid else -a.tax_amt_paid end),2),salcost = round(sum((case when a.invtype in ('06','07') then round(d.qty * d.pkqty * i.item_price,2) else - round(d.qty * d.pkqty * round(i.item_price,2),2) end)),2) ,win = round(sum((case when a.invtype in ('06','07') then round(d.qty * d.pkqty * i.item_price,2) else - round(d.qty * d.pkqty * round(i.item_price,2),2) end)),2) - round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2) ,winp ='' from  pu_hdr a join pucenters b on a.branch = b.pu_brno and a.pucenter=b.pu_no join pu_dtl d on a.branch=d.branch and a.pucenter=d.pucenter and a.invtype=d.invtype and a.ref=d.ref join items i on i.item_no=d.itemno where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condp + condinvtype + "  group By  a.branch,b.pu_name order By a.branch";
               
                else
                    qstr = "Select a.branch br_no,b.br_name, nettotal=round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2),tax=round(sum(case when a.invtype in ('06','07') then a.tax_amt_paid else -a.tax_amt_paid end),2),salcost = round(sum((case when a.invtype in ('06','07') then round(d.qty * d.pkqty * i.item_price,2) else - round(d.qty * d.pkqty * round(i.item_price,2),2) end)),2) ,win = round(sum((case when a.invtype in ('06','07') then round(d.qty * d.pkqty * i.item_price,2) else - round(d.qty * d.pkqty * round(i.item_price,2),2) end)),2) - round(sum(case when a.invtype in ('06','07') then a.nettotal-a.tax_amt_paid else -(a.nettotal-a.tax_amt_paid) end),2) ,winp ='' from  pu_hdr a join branchs b on a.branch = b.br_no join pu_dtl d on a.branch=d.branch and a.pucenter=d.pucenter and a.invtype=d.invtype and a.ref=d.ref join items i on i.item_no=d.itemno where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condp + condinvtype + "  group By  a.branch,b.br_name order By a.branch";
                //////if (rd_day.Checked)
                //////{
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Pur.rpt.Pur_Report_Br_rpt.rdlc";
