...
namespace POS.Pur
{
    public partial class PUcenters : BaseForm
    {
        ...
        {
            if (dataGridView1.CurrentRow.Cells[1].Value.ToString().Equals("01"))
            {
                MessageBox.Show("لا يمكن مسح مركز الشراء الاساسي", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
             ...
             if (result == DialogResult.Yes)
             {
                 try
                 {
                     ...
                     SqlDataAdapter da = new SqlDataAdapter("select * from pu_hdr where pucenter='" + todel + "' and branch='" + BL.CLS_Session.brno + "'", con1);
                     DataTable dt = new DataTable();
                     da.Fill(dt);

                     if (dt.Rows.Count >= 1)
                     {
                         MessageBox.Show(" لا يمكن حذف مركز شراء فية فواتير شراء ", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Error);
                     }
                     else
                     {
        ...
        private void Groups_Load(object sender, EventArgs e)
        {
           ...
           // this.dataGridView1.Columns[0].Frozen = true;
            ...
            // sqlConnection.Open();
            sqlDataAdapter = new SqlDataAdapter("select pu_brno,pu_no,pu_name,dp_cashser,dp_pucash,dp_pucrdt,dp_repucash,dp_repucrdt,dp_descont,dp_taxacc,dp_mainwh,dp_rtrnwh,serfrt,serins,sercst,serpvl,sersmp,cstcode,dp_mkhzon_acc,dp_sabr_acc from pucenters where  pu_brno='" + BL.CLS_Session.brno + "' and pu_name like '%" + textBox1.Text + "%' order by pu_no", sqlConnection);
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[3] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[4] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[5] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[6] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[7] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[8] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[9] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[12] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[13] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[14] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[15] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[16] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[18] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[19])
                {
                ...
                {
                    Search_All f4 = new Search_All("4", "Cus");
                    f4.ShowDialog();
                    //dataGridView1.CurrentRow.Cells[10].Value = f4.dataGridView1.CurrentRow.Cells[0].Value;
                    dataGridView1.CurrentCell.Value = f4.dataGridView1.CurrentRow.Cells[0].Value;
                }

                if (dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells["cst_code"])
                {
            ...
            //        Search_All f4 = new Search_All("1");
            //        f4.ShowDialog();
