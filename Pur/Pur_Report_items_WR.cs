...
namespace POS.Pur
{
    public partial class Pur_Report_items_WR : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (typeno == "")
                {
                    ...
                    SqlDataAdapter da3 = new SqlDataAdapter("select [ref] ,[contr] ,[type],[date] ,[total] ,[count] ,[payed],[total_cost],[saleman] ,[req_no] ,[cust_no],[discount],[net_total] from pu_hdr where date between'" + xxx + "' and '" + zzz + "'", con3);
                    SqlDataAdapter da4 = new SqlDataAdapter("select sum(total),sum(total_cost),sum(discount),sum(net_total) from pu_hdr where date between'" + xxx + "' and '" + zzz + "'", con3);
                ...
                {// and type = " + typeno + "
                    ...
                    string zzz;
                    zzz = maskedTextBox2.Text.ToString();
                    SqlDataAdapter da3 = new SqlDataAdapter("select [ref] ,[contr] ,[type],[date] ,[total] ,[count] ,[payed],[total_cost],[saleman] ,[req_no] ,[cust_no],[discount],[net_total] from pu_hdr where date between'" + xxx + "' and '" + zzz + "' and type = " + typeno + "", con3);
                    SqlDataAdapter da4 = new SqlDataAdapter("select sum(total),sum(total_cost),sum(discount),sum(net_total) from pu_hdr where date between'" + xxx + "' and '" + zzz + "' and type = " + typeno + "", con3);
                    DataSet ds3 = new DataSet();
                    da3.Fill(ds3, "0");
                    da4.Fill(ds3, "1");
                    dataGridView1.DataSource = ds3.Tables["0"];
                ...
                */
                ...
                string condfcst = txt_code.Text != "" ? " and c.supno='"+txt_code.Text+"' " : " ";
              ...
              /*  string qstr = "select a.item_no item_no,a.item_name item_name,( select sum(b.qty) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and b.type in ('04','05') where a.item_no=b.itemno) sal_qty"
                            ...
                            +"  from items a ";
                */
                string qstr = "select a.item_no item_no,a.item_name item_name,(select isnull(sum(b.qty * b.pkqty),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('06','07') " + condft + condp + condfcst + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' where a.item_no=b.itemno) sal_qty"
                           + " ,(select isnull(sum(b.qty * b.pkqty),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('16','17') " + condft + condp + condfcst + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' where a.item_no=b.itemno) rsal_qty"
                           + " ,0 net_sal_qty"
                           + " ,round(((select isnull(sum(b.qty * b.pkqty * b.fullcost),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('06','07') " + condft + condp + condfcst + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' where a.item_no=b.itemno) - (select isnull(sum(b.qty * b.pkqty * b.fullcost),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('16','17') " + condft + condp + condfcst + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' where a.item_no=b.itemno)),2) net_qty_price"
                           + " , 0 item_win,'0' item_winp, a.item_price price, a.item_curcost cost from items a  " + conditem + congrp + congrsp + ""
                           + " and round(((select isnull(sum(b.qty * b.pkqty * b.fullcost),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('06','07') " + condft + condp + condfcst + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' where a.item_no=b.itemno) - (select isnull(sum(b.qty * b.pkqty * b.fullcost),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('16','17') " + condft + condp + condfcst + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' where a.item_no=b.itemno)),2) <>0 ";
                         //  +" where cast((select isnull(sum(b.qty),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('04','05') where a.item_no=b.itemno) as float) <> 0"
                        //   + " and cast((select isnull(sum(b.qty),0) from pu_dtl b join pu_hdr c on c.branch=b.branch and c.ref=b.ref and c.invtype=b.invtype and b.invtype in ('14','15') where a.item_no=b.itemno) as float) <> 0";
                /*
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                */
                ...
                workSheet.Range["B" + (dataGridView1.Rows.Count + 6).ToString(), "H" + (dataGridView1.Rows.Count + 6).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
