...
namespace POS.Sup
{
    public partial class Acc_Statment_All : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (txt_code.Text == "")
            {
                MessageBox.Show(BL.CLS_Session.lang.Equals("E")?"Enter Supplier Account":"يجب ادخال رقم المورد", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txt_code.Focus();
                return;
            ...
            if (aakey != "")
            {
                try
                {
                    ...
                    foreach (DataRow dtrow in dtr2.Rows)
                    {
                        ...
                        dataGridView1.DataSource = daml.SELECT_QUIRY(qstr2);
                        */
                        string qstr2, condt, condp, conddm, condcur, ccur;
                        ...
                        condt = cmb_type.SelectedIndex != -1 ? " and a.a_type = '" + cmb_type.SelectedValue.ToString() + "' " : " ";
                            ...
                            conddm = chk_d.Checked ? " and a.jddbcr='C' " : (chk_m.Checked ? " and a.jddbcr='D' " : " ");
                             ...
                             condcur = cmb_cur.SelectedIndex == -1 ? "round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),2) c6" : "round((case a.jddbcr when 'D' then a.jdfcamt else - a.jdfcamt end),2) c6";
                             ccur = cmb_cur.SelectedIndex != -1 ? " and a.jdcurr='" + cmb_cur.SelectedValue + "' " : " ";
                           // qstr2 = "select a.a_type c1, a.a_ref c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),2) c6,b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu,'"+ dtrow[0].ToString()+"' yr from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type " + condp + conddm + " and a.cu_no='" + aakey + "' and a.a_type not in('04','14') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate";
                             qstr2 = "select a.a_type c1, a.a_ref c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5," + condcur + ",b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu,'" + dtrow[0].ToString() + "' yr from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.pu_no=b.pu_no " + ccur + condp + conddm + condt + "  and a.su_no='" + aakey + "' and a.a_type not in('06','16') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate,a.a_sysdate";
                        ////////}
                       ...
                       // dataGridView1.DataSource = daml.SELECT_QUIRY_only_retrn_dt(qstr2);
                        // dth = daml.SELECT_QUIRY_only_retrn_dt("select cu_name a_name,round(cu_opnbal,2) a_opnbal,cu_no a_key from customers where cu_no='" + aakey + "' and cu_brno='" + BL.CLS_Session.brno + "'");
                         //   dth = daml.SELECT_QUIRY_only_retrn_dt("select su_name a_name,round(su_opnbal,2) a_opnbal,su_no a_key from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'");

                            if (count == 0)
                            {
                              //  SqlDataAdapter darown = new SqlDataAdapter("select su_name a_name,round(su_opnbal,2) a_opnbal,su_no a_key from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'", contyr);

                             //   SqlDataAdapter darown = new SqlDataAdapter("select su_name a_name,case when su_cur ='" + cmb_cur.SelectedValue + "' then su_opnfcy else su_opnbal end a_opnbal,su_no a_key from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'", contyr);
                                SqlDataAdapter darown = new SqlDataAdapter("select su_name a_name,round(" + (cmb_cur.SelectedIndex == -1 ? " su_opnbal " : " su_opnfcy ") + ",2) a_opnbal,su_no a_key from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'", contyr);
                                SqlDataAdapter darown2 = new SqlDataAdapter("select su_name a_name,round(" + (cmb_cur.SelectedIndex == -1 ? " su_opnbal " : " su_opnfcy ") + ",2) a_opnbal,su_no a_key from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'", contyr);
