       ...
       // if(txt_code.Text !="")
        ...
        private void button3_Click(object sender, EventArgs e)
        {
          ...
          // dataGridView2.DataSource = dt;
               ...
               //// parameters[1] = new ReportParameter("a_toword", cmb_cur.SelectedIndex != -1 ? toWord.ConvertToArabic().Substring(0, toWord.ConvertToArabic().Length - 17) + cmb_cur.Text : toWord.ConvertToArabic());
          ...
          //  double seso = 0;
            ...
            if (aakey != "")
            {
                try
                {
                    ...
                    dataGridView1.DataSource = daml.SELECT_QUIRY(qstr2);
                    */
                   // dth = daml.SELECT_QUIRY_only_retrn_dt("select su_name a_name,round(" + (cmb_cur.SelectedIndex == -1 ? " su_opnbal " : " su_opnfcy ") + ",2) a_opnbal,su_no a_key,whno from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'");
                    dth = daml.SELECT_QUIRY_only_retrn_dt("select a.su_name a_name,round((" + (cmb_cur.SelectedIndex == -1 ? " a.su_opnbal " : " a.su_opnfcy ") + " + (select isnull(sum(b.a_damt-b.a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and c.a_mdate < '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' where a.su_no=b.su_no and b.a_type not in ('06','16'))),2) a_opnbal,a.su_no a_key,a.whno  from suppliers a where a.su_no='" + aakey + "' and a.su_brno='" + BL.CLS_Session.brno + "'");
                   
                    if (chk_combin.Checked && !string.IsNullOrEmpty(dth.Rows[0][3].ToString()))
                        dths = daml.SELECT_QUIRY_only_retrn_dt("select round(isnull(cu_opnbal,0),4) a_opnbal from customers where cu_no='" + dth.Rows[0][3].ToString() + "' and cu_brno='" + BL.CLS_Session.brno + "'");

                    string condcmb2 = string.IsNullOrEmpty(dth.Rows[0][3].ToString()) ? "a.cu_no='XYX')" : "a.cu_no='" + dth.Rows[0][3].ToString() + "')";
                    string condcmb = chk_combin.Checked ? " and (a.su_no='" + aakey + "' or " + condcmb2 : " and a.su_no='" + aakey + "' ";
                    string condcmb3 = chk_combin.Checked ? ",'04','14'" :  "";
                  //   string condcmb = chk_combin.Checked ? " and (a.su_no='" + aakey + "' or a.cu_no='" + dth.Rows[0][3].ToString() + "') " : " and a.su_no='" + aakey + "' ";
                  ...
                  //  string condtx = txt_desc.Text;
                    string condcur = cmb_cur.SelectedIndex == -1 ? "round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),4) c6" : "round((case a.jddbcr when 'D' then a.jdfcamt else - a.jdfcamt end),4) c6";
                    string ccur = cmb_cur.SelectedIndex != -1 ? " and a.jdcurr='" + cmb_cur.SelectedValue + "' " : " ";
                   // string qstr2 = "select a.a_type c1, a.a_ref c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),2) c6,b.a_posted,a.a_type a_t from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type " + condp + conddm + " and " + cond + " <= '" + txt_tdate.Text.Replace("-", "") + "' and a.su_no='" + aakey + "' and a.a_type not in('06','16') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate";
                  //////  string qstr2 = "select a.a_type c1, a.a_ref c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5," + condcur + ",b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.pu_no=b.pu_no " + ccur + condp + conddm + condt + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' "+condcmb+" and a.a_type not in('06','16') and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate,a.a_sysdate";
                    string qstr2 = "select a.a_type c1, a.a_ref c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,(case when a.a_type in ('06','07','16','17') then a.a_text + '  ' + c.invsupno else a.a_text end) c5," + condcur + ",b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu from acc_dtl a join acc_hdr b on a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.pu_no=b.pu_no left join pu_hdr c on a.a_brno=c.branch and a.a_ref=c.ref and a.a_type=c.invtype and a.pu_no=c.pucenter where 1=1 " + ccur + condp + conddm + condt + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condcmb + " and a.a_type not in('06','16'" + condcmb3 + ") and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate,a.a_sysdate";

                  //  dth = daml.SELECT_QUIRY_only_retrn_dt("select su_name a_name,case when su_cur ='"+cmb_cur.SelectedValue+"' then su_opnfcy else su_opnbal end  a_opnbal,su_no a_key from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'");
                  //  dth = daml.SELECT_QUIRY_only_retrn_dt("select su_name a_name,round(" + (cmb_cur.SelectedIndex == -1 ? " su_opnbal " : " su_opnfcy ") + ",2) a_opnbal,su_no a_key from suppliers where su_no='" + aakey + "' and su_brno='" + BL.CLS_Session.brno + "'");
                    ...
                    */
                    if (dataGridView1.RowCount == 0)
                        seso = Convert.ToDouble(txt_opnbal.Text);
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(Directory.GetCurrentDirectory() + @"\reports\Sup_Balance_Dtl2_rpt.rdlc"))
                rf.reportViewer1.LocalReport.ReportPath = Directory.GetCurrentDirectory() + @"\reports\Sup_Balance_Dtl2_rpt.rdlc";
            else
