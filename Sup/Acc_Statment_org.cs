        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (txt_code.Text != "")
                {
                  ...
                  //  dth = daml.SELECT_QUIRY_only_retrn_dt("select su_name a_name,round(" + (cmb_cur.SelectedIndex == -1 ? " su_opnbal " : " su_opnfcy ") + ",2) a_opnbal,su_no a_key from suppliers where su_no='" + txt_code.Text + "' and su_brno='" + BL.CLS_Session.brno + "'");
                    dth = daml.SELECT_QUIRY_only_retrn_dt("select a.su_name a_name,round((" + (cmb_cur.SelectedIndex == -1 ? " a.su_opnbal " : " a.su_opnfcy ") + " + (select isnull(sum(b.a_damt-b.a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and c.a_mdate < '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' where a.su_no=b.su_no and b.a_type not in ('06','16'))),2) a_opnbal,a.su_no a_key from suppliers a where a.su_no='" + txt_code.Text + "' and a.su_brno='" + BL.CLS_Session.brno + "'");
