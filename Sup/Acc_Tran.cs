...
namespace POS.Sup
{
    public partial class Acc_Tran : BaseForm
    {
       // DataTable dtg;
        ...
        public Acc_Tran(string atype,string aref)
        {
            InitializeComponent();
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (dataGridView1.CurrentCell == dataGridView1[1, dataGridView1.CurrentRow.Index])
                {
                    ...
                    SqlDataAdapter da = new SqlDataAdapter("select top 1 a_key a ,a_name b from accounts where a_active=1 and accontrol=0 and a_key = '" + dataGridView1.CurrentRow.Cells[1].Value + "' and a_brno='" + BL.CLS_Session.brno + "'", con);
    ...
    */
                    if (!row.IsNewRow && !string.IsNullOrEmpty(row.Cells[0].Value.ToString() + row.Cells[1].Value.ToString()))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,  a_camt,a_folio,a_mdate,a_hdate,jddbcr,su_no,cc_no) VALUES(@a0,@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12)", con))
                        {
        ...
        private void Fill_Data(string atype, string aref)
        {
            try
            {
              ...
              //  hdate = txt_hdate.Text.Replace("-", "").Substring(4, 4) + txt_hdate.Text.Replace("-", "").Substring(2, 2) + txt_hdate.Text.Replace("-", "").Substring(0, 2);
                ...
                dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select  aa.su_no su_no,(case aa.su_no when '' then aa.a_key else '' end) a_key,bb.a_name a_name,aa.a_text a_text,aa.a_damt a_damt,aa.a_camt a_camt,(case when aa.su_no <> '' then aa.a_key else '' end) acc_key  from acc_dtl aa,accounts bb where aa.a_key=bb.a_key and aa.a_type='" + atype + "' and aa.a_ref=" + aref + " and aa.a_brno='" + BL.CLS_Session.brno + "' order by aa.a_folio");
