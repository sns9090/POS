...
namespace POS.Sup
{
    public partial class Acc_Balance_Sumry : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                */
                ...
                string qstr = "select a.su_no d1,a.cu_name d2,round(a.su_opnbal +(select isnull(sum(a_damt-a_camt),0) from acc_dtl b where a.su_no=b.su_no and " + cond + "<='" + maskedTextBox2.Text.Replace("-", "") + "'),2) d3"
              ...
              //  string cond = checkBox1.Checked ? "b.a_hdate" : "b.a_mdate";
                string cond = checkBox1.Checked ? "b.a_hdate" : "b.a_mdate";
                string condcn = string.IsNullOrEmpty(txt_cuno.Text) ? " " : " and a.su_no='" + txt_cuno.Text + "' ";
              ...
              //  string conddc = chk_m.Checked ? " having round((select isnull(a.su_opnbal+sum(isnull(case when b.a_type not in ('06','16') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('06','16') then b.a_camt else 0 end ,0)),0)),2)<0 " : (chk_d.Checked ? " round((select isnull(a.su_opnbal+sum(isnull(case when b.a_type not in ('06','16') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('06','16') then b.a_camt else 0 end ,0)),0)),2)>0 " : " having round((select isnull(a.su_opnbal+sum(isnull(case when b.a_type not in ('06','16') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('06','16') then b.a_camt else 0 end ,0)),0)),2)<>0 ");
                string conddc = chk_m.Checked ? " having round((select isnull(a.su_opnbal+sum(isnull(case when b.a_type not in ('06','16') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('06','16') then b.a_camt else 0 end ,0)),0)),2)<0 " : (chk_d.Checked ? " having round((select isnull(a.su_opnbal+sum(isnull(case when b.a_type not in ('06','16') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('06','16') then b.a_camt else 0 end ,0)),0)),2)>0 " : " having 1<>0 ");

               // string qstr = "select a.su_no d1,a.cu_name d2,a.su_opnbal d3,round(( isnull(sum(b.a_camt),0)),2) d4,round(( isnull(sum(b.a_damt),0)),2) d5,round(( isnull(a.su_opnbal+sum(b.a_damt-b.a_camt),0)),2) d6,round(( isnull(max(iif(b.a_camt>0 ,b.a_camt,0)),0)),2) d7,( isnull(max(iif(b.a_camt>0 ,b.a_mdate,'##-##-####')),'##-##-####')) d8 from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and b.a_type not in ('06','16') right Outer join customers a on a.su_brno=b.a_brno and a.su_no=b.su_no where a.su_no<>'' " + condc + condcity + condp + " group by a.su_no ,a.cu_name ,a.su_opnbal " + conddc + "";
                string qstr = "select a.su_no d1,a.su_name d2,a.su_opnbal d3,round((isnull(sum(case when b.a_type in ('06','16') then b.a_camt + b.a_damt else 0 end),0)),4) d4,round(( isnull(sum(case when b.a_type in ('07','17') then b.a_camt + b.a_damt else 0 end),0)),4) d5,round(( isnull(sum(case when b.a_type in ('06','07','16','17') then b.a_camt + b.a_damt else 0 end),0)),4) d6,round(( isnull(sum(case when b.a_type not in ('06','07','16','17') then b.a_camt + b.a_damt else 0 end),0)),4) d7,round((select isnull(a.su_opnbal+sum(isnull(case when b.a_type not in ('06','16') then b.a_damt else 0 end,0)-isnull(case when b.a_type not in ('06','16') then b.a_camt else 0 end ,0)),0)),4) d8 from suppliers a left outer join acc_dtl b  inner join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.pu_no=b.pu_no on a.su_brno=b.a_brno and a.su_no=b.su_no where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' and a.su_brno='" + BL.CLS_Session.brno + "' " + condc + condcity + condp + condcn + " group by a.su_no ,a.su_name ,a.su_opnbal " + conddc + " order by cast(a.su_no as int)";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //  rf.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("details", dtdtl));
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sup.rpt.Sup_Balance_Sumry_rpt.rdlc";
