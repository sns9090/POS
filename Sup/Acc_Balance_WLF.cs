                ...
                */
                ...
                if (!chk_openbalonly.Checked)
                {
                    qstr = "select a.su_no d1,a.su_name d2,round(a.su_opnbal +(select isnull(sum(a_damt-a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.pu_no=b.pu_no and b.a_type not in ('06','16') " + condp + " where b.a_brno='" + BL.CLS_Session.brno + "' and a.su_no=b.su_no and " + cond + "<='" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'),4) d3"
                              + " ,0.00 d4,su_pymnt d5,'' d6 from suppliers a where round(a.su_opnbal +(select isnull(sum(a_damt-a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.pu_no=b.pu_no and b.a_type not in ('06','16') " + condp + " where b.a_brno='" + BL.CLS_Session.brno + "' and a.su_no=b.su_no and " + cond + "<='" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'),4) <>0 "
                              + " and su_brno='" + BL.CLS_Session.brno + "'" + condc + condcity + " and a.su_no like '%" + txt_cuno.Text + "%'"
                              + " order by cast(a.su_no as int);";
                }
                else
                {
                    qstr = "select a.su_no d1,a.su_name d2,round(a.su_opnbal ,4) d3,0.00 d4,su_pymnt d5,'' d6"
                                 + " from suppliers a where round(a.su_opnbal ,4) <>0 "
                ...
                foreach (DataRow dtr1 in dtacbal.Rows)
                {
                   ...
                   // sum1 = sum1 + Convert.ToDouble(dtr[3]);
                    dtmustp = daml.SELECT_QUIRY_only_retrn_dt("select round(a.su_opnbal +(select isnull(sum(-a_camt),0) from acc_dtl b where b.a_mdate <= DATEADD(day,-" + dtr1[4] + ",CAST('" + datval.convert_to_yyyyDDdd(strt_dt) + "' as date)) and a.su_no=b.su_no and a.su_brno='" + BL.CLS_Session.brno + "' and b.a_type not in ('06','16')) + (select isnull(sum(a_damt),0) from acc_dtl b where  a.su_no=b.su_no and a.su_brno='" + BL.CLS_Session.brno + "' and b.a_type not in ('06','16')),4) from suppliers a where a.su_no = '" + dtr1[0] + "'");
                    dtr1[3] = Convert.ToDouble(dtmustp.Rows[0][0]) < 0 ? dtmustp.Rows[0][0].ToString() : "0.00"; //dtmustp.Rows[0][0].ToString();
