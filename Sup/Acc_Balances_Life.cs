                  ...
                  //  string condtx = txt_desc.Text;
                    ...
                    else
                    {
                       // condt = cmb_type.SelectedIndex != -1 ? " and a.a_type = '" + cmb_type.SelectedValue.ToString() + "' " : " ";
                         ...
                         conddm = chk_d.Checked ? " and a.jddbcr='C' " : (chk_m.Checked ? " and a.jddbcr='D' " : " ");
                          condc = cmb_class.SelectedIndex != -1 ? " and c.su_class='" + cmb_class.SelectedValue + "' " : " ";
                          condcity = cmb_city.SelectedIndex != -1 ? " and c.su_city='" + cmb_city.SelectedValue + "' " : " ";
                          condft = (!string.IsNullOrEmpty(txt_cuno.Text) && !string.IsNullOrEmpty(txt_tosup.Text)) ? " and a.su_no between '" + txt_cuno.Text + "' and '" + txt_tosup.Text + "' " : "";
                         ...
                         qstr2 = "select  c.su_no  c2,c.su_name c5,"
                              ...
                              + "sum(iif(CAST(a.a_mdate as date) <= DATEADD(day, -361, CAST('" + datval.convert_to_yyyyDDdd(strt_dt) + "' as date)), round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),4),0)) + round(c.su_opnbal,4) c13 "
                            //  + ", '' c14 "
                              + ",cast(ISNULL(c.su_pymnt,0) as varchar) + ' ' + 'يوم' c14"
                              + ", sum(iif(CAST(a.a_mdate as date) <= DATEADD(day, -c.su_pymnt, CAST('" + datval.convert_to_yyyyDDdd(strt_dt) + "' as date)), round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),4),0)) + round(c.su_opnbal,4) c15"
                              + " from suppliers c join acc_dtl a on a.a_brno=c.su_brno and a.su_no=c.su_no join acc_hdr b on a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.sl_no=b.sl_no where 1=1 " + condp + conddm + condc + condcity + condft + " and " + cond + " <= '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'  and c.su_brno='" + BL.CLS_Session.brno + "' and a.a_type not in('06','16') group by c.su_no,c.su_name,c.su_opnbal,c.su_pymnt order by cast(c.su_no as int)  ";
                    }
