...
namespace POS.Sal
{
    public partial class Sal_Report_Gr_Daily : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            WaitForm wf = new WaitForm("جاري احتساب مبيعات المجموعات .. يرجى الانتظار ...");
            ...
            try
            {
                ...
                */
                ...
                // if (cmb_grp.SelectedIndex == -1)
                //     qstr = "Select a.item_group br_no,b.group_name br_name, nettotal=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),tax=round(sum(case when c.invtype in ('04','05') then c.QTY else - c.QTY end),2),salcost = round(sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end),2) ,win=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1))-sum((invdsvl/(nettotal+invdsvl))*iif(1=1,((c.QTY*(c.price-(c.price*c.discpc/100)))-c.offer_amt),((c.QTY*(c.price-Floor(c.price*c.discpc/100)))-c.offer_amt))*(case when h.invtype in ('04','05') then 1 else -1 end))-sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end)  ,2),winp='' from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condgrp + condsgrp + condst + "  group By  a.item_group,b.group_name order By a.item_group";
                // else
                //     qstr = "Select a.sgroup br_no,b.group_name br_name, nettotal=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),tax=round(sum(case when c.invtype in ('04','05') then c.QTY else - c.QTY end),2),salcost = round(sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end),2) ,win=round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1))-sum((invdsvl/(nettotal+invdsvl))*iif(1=1,((c.QTY*(c.price-(c.price*c.discpc/100)))-c.offer_amt),((c.QTY*(c.price-Floor(c.price*c.discpc/100)))-c.offer_amt))*(case when h.invtype in ('04','05') then 1 else -1 end))-sum(case when c.invtype in ('04','05') then c.QTY * c.pkqty *(c.cost) else -(c.QTY * c.pkqty *(c.cost)) end)  ,2),winp='' from  items a join groups b on a.sgroup = b.group_id and b.group_pid is not null join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' " + condbr + condinvtype + condgrp + condsgrp + condst + "  group By  a.sgroup,b.group_name order By a.sgroup";

                qstr = "select REPLACE((SUBSTRING(CONVERT(varchar,(dateadd(DAY,0, datediff(day,0, invmdate))),111),1,10)),'/','-') as 'date',case (DATENAME ( weekday , [invmdate] )) when 'Monday' then  'الاثنين' when 'Tuesday' then 'الثلاثاء' when 'Wednesday' then 'الأربعاء' when 'Thursday' then 'الخميس' when 'Friday' then 'الجمعة' when 'Saturday' then 'السبت' when 'Sunday' then 'الأحد' end 'day' " + strqadd + "  from sales_hdr WHERE branch='" + BL.CLS_Session.brno + "' and invmdate between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' AND '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' group by dateadd(DAY,0, datediff(day,0, invmdate)),DATENAME ( weekday , [invmdate] ) ORDER BY dateadd(DAY,0, datediff(day,0, invmdate)),DATENAME ( weekday , [invmdate] ) ";
                //////if (rd_day.Checked)
                //////{
                ...
                */
                ...
                //foreach (DataRow dtrr in dt.Rows)
                //{
                //    foreach (DataColumn col in dtrr.c)
                //    {


                //        // DataTable dtbr =daml.SELECT_QUIRY_only_retrn_dt( "Select isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id and b.group_pid is not null join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where b.group_id='01' and h.invmdate between '" + dtrr[0].ToString().Replace("-", "") + "' and '" + dtrr[0].ToString().Replace("-", "") + "' ");

                //        if (i < 5)
                //        {
                //            DataTable dtbr = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref] where b.group_id='01' and h.invmdate = '" + dtrr[0].ToString().Replace("-", "") + "' ");
                //            dtrr[i] = decimal.Parse(dtbr.Rows[0][0].ToString());
                //        }
                //        else
                //        {
                //            DataTable dtbr = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref] where and h.invmdate = '" + dtrr[0].ToString().Replace("-", "") + "' ");
                //            dtrr[i] = decimal.Parse(dtbr.Rows[0][0].ToString());
                //        }
              ...
              //  MessageBox.Show("here");
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        // MessageBox.Show(dataGridView1.Columns[cell.ColumnIndex].DataPropertyName.Replace("val", ""));
                        if (cell.ColumnIndex >= 2 && cell.ColumnIndex <= dataGridView1.Columns.Count - 2 && cell.RowIndex != dataGridView1.RowCount )
                        {
                            DataTable dtbr = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt-(case when h.with_tax=0 then 0 else tax_amt end))*IIF(c.invtype in ('04','05'),1,-1)),2)+round(sum((tax_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref] where b.group_id='" + dataGridView1.Columns[cell.ColumnIndex].DataPropertyName.Replace("val", "") + "' and h.invmdate = '" + row.Cells[0].Value.ToString().Replace("-", "") + "' ");
                            row.Cells[cell.ColumnIndex].Value = decimal.Parse(dtbr.Rows[0][0].ToString());
                        }
                        else if (cell.ColumnIndex >= dataGridView1.Columns.Count - 1 && cell.RowIndex != dataGridView1.RowCount)
                        {
                            DataTable dtbr = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt-(case when h.with_tax=0 then 0 else tax_amt end))*IIF(c.invtype in ('04','05'),1,-1)),2)+round(sum((tax_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref] where h.invmdate = '" + row.Cells[0].Value.ToString().Replace("-", "") + "' ");
                            row.Cells[cell.ColumnIndex].Value = decimal.Parse(dtbr.Rows[0][0].ToString());
                        }
                        //else if (cell.ColumnIndex >= 2 && cell.ColumnIndex <= dataGridView1.Columns.Count - 2 && cell.RowIndex == dataGridView1.RowCount )
                        //{
                        //    DataTable dtbr = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt +(case when h.with_tax=0 then 0 else tax_amt end))*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref] where b.group_id='" + dataGridView1.Columns[cell.ColumnIndex].DataPropertyName.Replace("val", "") + "' and h.invmdate between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' AND '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'" + "");
                        //    row.Cells[cell.ColumnIndex].Value = decimal.Parse(dtbr.Rows[0][0].ToString());
                        //}
                        //else if (cell.ColumnIndex >= dataGridView1.Columns.Count - 1 && cell.RowIndex == dataGridView1.RowCount )
                        //{
                        //    DataTable dtbr = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt +(case when h.with_tax=0 then 0 else tax_amt end))*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref] where h.invmdate between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' AND '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'" + "");
                        //    row.Cells[cell.ColumnIndex].Value = decimal.Parse(dtbr.Rows[0][0].ToString());
                        //}
                    }
                }
        ...
        private void SalesReport_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            dtslctr = daml.SELECT_QUIRY_only_retrn_dt("select group_id,group_name from groups where group_pid is null ");


            rwcnt = dtslctr.Rows.Count;
            //cmb_wh.DataSource = dtslctr;
            //cmb_wh.DisplayMember = "wh_name";
            //cmb_wh.ValueMember = "wh_no";
            //cmb_wh.SelectedIndex = -1;

            foreach (DataRow dtrr in dtslctr.Rows)
            {
              ...
              //  strqadd = strqadd + " ,sum(case when b.group_id='" + dtrr[0].ToString() + "' then (Select nettotal=isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id and b.group_pid is not null join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where b.group_id='" + dtrr[0].ToString() + "' and h.invmdate between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "') else 0.00 end) as val" + dtrr[0].ToString();
             //   strqadd = strqadd + " ,(Select nettotal=isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id and b.group_pid is not null join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where b.group_id='" + dtrr[0].ToString() + "' and h.invmdate between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "') as val" + dtrr[0].ToString();
                strqadd = strqadd + " ,0.00 val" + dtrr[0].ToString();
            ...
            col2.HeaderText = "الاجمالي";// "Column3";
            col2.Name = "الاجمالي"; //"Column3";
            // col.FillWeight = 15;
            col2.Width = 100;
            col2.DataPropertyName = "valtotal";
           // strqadd = strqadd + " ,sum((Select nettotal=isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id and b.group_pid is not null join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where h.invmdate between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "')) as valtotal ";
           // strqadd = strqadd + " ,(Select nettotal=isnull(round(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100))))-offer_amt)*IIF(c.invtype in ('04','05'),1,-1)),2),0) from  items a join groups b on a.item_group = b.group_id and b.group_pid is not null join sales_dtl c on a.item_no=c.itemno join sales_hdr h on c.[branch]=h.[branch] and c.[slcenter]=h.[slcenter] and c.[invtype]=h.[invtype] and  c.[ref]=h.[ref]  where h.invmdate between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "') as valtotal ";
            strqadd = strqadd + " ,0.00 as valtotal ";
            //  col4.HeaderText = "Column4";
            //  col4.Name = "Column4";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                */
                ...
                workSheet.Range["A" + (dataGridView1.Rows.Count + 6).ToString(), "Z" + (dataGridView1.Rows.Count + 6).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);

                workSheet.Range["A6", "Z6"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            GridPrintDocument gridPrintDocument = new GridPrintDocument(this.dataGridView1, this.dataGridView1.Font, true);
            ...
            ////   // dt = daml.SELECT_QUIRY_only_retrn_dt("select b.item_no c1,a.item_name c2,e.unit_name c3,round(a.item_curcost,2) c4,a.item_price c5,(b.qty + b.openbal) c6 , c.wh_name c7 from items a inner join whbins b on a.item_no = b.item_no inner join whouses c on c.wh_no= b.wh_no and c.wh_brno=b.br_no inner join units e on e.unit_id=a.item_unit");
            ...
            ////    rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sal.rpt.Sal_Report_GR_Daily_rpt.rdlc";
