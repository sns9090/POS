...
namespace POS.Sal
{
    public partial class Sal_Tax_Report : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                */
                ...
                string conds = rd_tax.Checked ? " and tax_amt_rcvd<>0 " : rd_ntax.Checked ? " and tax_amt_rcvd=0 " : " ";
               ...
               // string qstr = "select slcenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(invhdate as date), 105) a_hdate,text a_text,invttl invttl,invdsvl invdsvl,tax_amt_rcvd tax,nettotal nettotal,posted a_p,invtype a_t from sales_hdr where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + " and " + cond + " between '" + txt_fdate.Text.Replace("-", "") + "' and '" + txt_tdate.Text.Replace("-", "") + "'";
                //string qstr = "select slcenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('04','05') then (case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) else -(case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) end) invttl,(case when invtype in ('04','05') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('04','05') then nettotal else -nettotal end) nettotal,(case when invtype in ('04','05') then tax_amt_rcvd else -tax_amt_rcvd end) tax,(case when custno<>'' then custno else casher end) cu_no,(case when custno<>'' then cu_name else 'عميل عادي' end) cu_name,(case when vndr_taxcode<>'' then vndr_taxcode else '' end) tax_no,posted a_p,invtype a_t,glser src,(case when invtype in ('04','05') then isnull(taxfree_amt,0) else -(isnull(taxfree_amt,0)) end) tf,(case when invtype in ('04','05') then (case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) - isnull(taxfree_amt,0) else -((case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) - isnull(taxfree_amt,0)) end) tt from sales_hdr left join customers on cu_no=custno where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + conds + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                string qstr = "select slcenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('04','05') then (case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) else -(case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) end) invttl,(case when invtype in ('04','05') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('04','05') then nettotal else -nettotal end) nettotal,(case when invtype in ('04','05') then tax_amt_rcvd else -tax_amt_rcvd end) tax,(case when custno<>'' then custno else casher end) cu_no,(case when custno<>'' then cu_name else 'عميل عادي' end) cu_name,(case when vndr_taxcode<>'' then vndr_taxcode else '' end) tax_no,posted a_p,invtype a_t,glser src,(case when invtype in ('04','05') then isnull(taxfree_amt,0) else -(isnull(taxfree_amt,0)) end) tf,(case when invtype in ('04','05') then (case when with_tax=0 then nettotal-tax_amt_rcvd else nettotal-tax_amt_rcvd end) - isnull(taxfree_amt,0) else -((case when with_tax=0 then nettotal-tax_amt_rcvd else nettotal-tax_amt_rcvd end) - isnull(taxfree_amt,0)) end) tt,with_tax wt from sales_hdr left join customers on cu_no=custno where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + conds + condft + conslctr + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
              ...
              //  dr[4] = "";
                ...
                dr[12] = "";
                dr[13] = true;
                dr[16] = Math.Round(sumtf, 2);
                dr[17] = Math.Round(sumttt, 2);
                dr[18] = false;
               // dr[11] = "";
                ...
                foreach (DataGridViewRow rw in dataGridView1.Rows)
                {
                   ...
                   // rw.Cells[0].Value = rw.Cells[0].Value.ToString().Equals("01") ? "ق عام" : rw.Cells[0].Value.ToString().Equals("02")? "س قبض" : "س صرف";
                    ...
                    if (Math.Round(Convert.ToDouble(rw.Cells[9].Value), 2) != (Convert.ToBoolean(rw.Cells["wt"].Value) ? Math.Round((((Convert.ToDouble(rw.Cells[6].Value) - Convert.ToDouble(rw.Cells[7].Value) + Convert.ToDouble(rw.Cells[9].Value)) * taxper) / (100 + taxper)), 2) : Math.Round(((Convert.ToDouble(rw.Cells[6].Value) - Convert.ToDouble(rw.Cells[7].Value)) * taxper / 100), 2)))
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                */
                ...
                workSheet.Range["A6", "R6"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                */
                ...
                parameters[4] = new ReportParameter("name", "اسم العميل");
                parameters[5] = new ReportParameter("no", "رقم العميل");
