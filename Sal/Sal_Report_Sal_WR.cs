...
namespace POS.Sal
{
    public partial class Sal_Report_Sal_WR : BaseForm
    {
      ...
      //  DataTable dtt;
       ...
       // string typeno = "";
        ...
        {
            WaitForm wf = new WaitForm("جاري احتساب ارباح المبيعات  .. يرجى الانتظار ...");
            ...
            try
            {
                ...
                */
                ...
                string conslctr = cmb_salctr.SelectedIndex != -1 ? " and sales_hdr.slcenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " ";

                string condplc = chk_plc.Checked ? " and  (case when sales_hdr.invtype in ('04','05') then round(((sales_hdr.invttl - sales_hdr.invdsvl) - sales_hdr.invcst),2) else -round(((invttl - invdsvl) - invcst),2) end)  < 0 " : " ";
              ...
              //  string qstr = "select slcenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('04','05') then (case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) else -(case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) end) invttl,(case when invtype in ('04','05') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('04','05') then tax_amt_rcvd else -tax_amt_rcvd end) tax,(case when invtype in ('04','05') then nettotal else -nettotal end) nettotal,posted a_p,invtype a_t,(case when invtype in ('04','05') then round(((invttl - invdsvl) - invcst),2) else -round(((invttl - invdsvl) - invcst),2) end) invwin,('%' + (case when invtype in ('04','05') then CAST(round((((invttl - invdsvl) - invcst)/(invttl - invdsvl))*100,2) as varchar) else CAST(-round((((invttl - invdsvl) - invcst)/(invttl - invdsvl))*100,2) as varchar) end)) invwinp  from sales_hdr where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + condplc + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
               //// string qstr = "select slcenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('04','05') then (case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) else -(case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) end) invttl,(case when invtype in ('04','05') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('04','05') then tax_amt_rcvd else -tax_amt_rcvd end) tax,(case when invtype in ('04','05') then nettotal else -nettotal end) nettotal,posted a_p,invtype a_t,(case when invtype in ('04','05') then round(((nettotal - tax_amt_rcvd) - invcst),2) else -round(((nettotal - tax_amt_rcvd) - invcst),2) end) invwin,('%' + (case when invtype in ('04','05') then CAST(round((((nettotal - tax_amt_rcvd) - invcst)/(nettotal - tax_amt_rcvd))*100,2) as varchar) else CAST(-round((((nettotal - tax_amt_rcvd) - invcst)/(nettotal - tax_amt_rcvd))*100,2) as varchar) end)) invwinp  from sales_hdr where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + condplc + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                string qstr = "select slcenter sl_no,invtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(invmdate as date), 105) a_mdate,(substring(invhdate,7,2) + '-' + substring(invhdate,5,2)+'-'+substring(invhdate,1,4)) a_hdate,text a_text,(case when invtype in ('04','05') then (case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) else -(case when with_tax=0 then invttl else invttl-tax_amt_rcvd end) end) invttl,(case when invtype in ('04','05') then invdsvl else -invdsvl end) invdsvl,(case when invtype in ('04','05') then tax_amt_rcvd else -tax_amt_rcvd end) tax,(case when invtype in ('04','05') then nettotal else -nettotal end) nettotal,posted a_p,invtype a_t,'' invwin,'' invwinp  from sales_hdr where branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + condplc + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                
                if (!string.IsNullOrEmpty(txt_desc.Text))
                {
                    qstr = qstr + " and sales_hdr.text like '%" + txt_desc.Text + "%'";
                }

                if (!string.IsNullOrEmpty(txt_equl.Text))
                {
                  //  MessageBox.Show(comboBox2.SelectedValue.ToString());
                    qstr = qstr + " and sales_hdr.invttl " + comboBox2.Text.Substring(0, 1) + txt_equl.Text;
                }

                dt = daml.SELECT_QUIRY_only_retrn_dt(qstr + " and sales_hdr.invtype like '%" + cmb_type.SelectedValue + "%' order by sales_hdr.invmdate");

                DataRow dr = dt.NewRow();
                double sumopnd = 0, sumopnm = 0, sumtrd = 0, sumtrm = 0, sumttld = 0, sumttlm = 0;
                foreach (DataRow dtr in dt.Rows)
                {
                    DataTable dtrw = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(sum(((qty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100)))-(qty*pkqty*item_curcost))-offer_amt-(case when sales_hdr.with_tax=0 then 0 else tax_amt end))*IIF(sales_hdr.invtype in('04','05'),1,-1)),0) from sales_hdr inner Join sales_dtl LEFT OUTER JOIN items  ON sales_dtl.itemno=items.item_no on sales_hdr.branch=sales_dtl.branch and sales_hdr.invtype=sales_dtl.invtype and sales_hdr.ref=sales_dtl.ref and sales_hdr.slcenter=sales_dtl.slcenter where sales_hdr.branch='" + BL.CLS_Session.brno + "' and sales_hdr.invtype='" + dtr[1] + "' and sales_hdr.ref=" + dtr[2] + " and sales_hdr.slcenter='" + dtr[0] + "'");
                    dtr[12] = (Math.Round(Convert.ToDouble(dtrw.Rows[0][0].ToString()), 2)).ToString();
               ...
               // sumttlm = Math.Round((sumttld / (sumopnd - sumopnm)) * 100, 2);
                sumttlm = Math.Round((sumttld / sumtrm) * 100, 2);
              //  dr[0] = "";
              //  dr[1] = "";
              //  dr[2] = "";
              //  dr[3] = "";
              //  dr[4] = "";
               ...
               // dr[11] = " ";
                ...
                dr[12] = Math.Round(sumttld, 2).ToString();
               // dr[12] = "";
              ...
              //  dataGridView1.ClearSelection();
                ...
                foreach (DataGridViewRow rw in dataGridView1.Rows)
                {
                  ...
                  //  DataTable dtrw=new DataTable();
                   // if (!string.IsNullOrEmpty(rw.Cells[0].ToString().Trim()))
                    //(Select isnull(sum(((qty*pkqty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100)))-item_curcost)-offer_amt-(case when sales_hdr.with_tax=0 then 0 else tax_amt end))*IIF(sales_hdr.invtype in('04','05'),1,-1)),0) from sales_hdr inner Join sales_dtl LEFT OUTER JOIN items  ON sales_dtl.itemno=items.item_no on sales_hdr.branch=sales_dtl.branch and sales_hdr.invtype=sales_dtl.invtype and sales_hdr.ref=sales_dtl.ref and sales_hdr.slcenter=sales_dtl.slcenter where sales_hdr.branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + conslctr + condplc + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' )
                   // {
                       // DataTable dtrw = daml.SELECT_QUIRY_only_retrn_dt("Select isnull(sum(((qty*pkqty*(price-iif(1=1,(price*discpc/100),floor(price*discpc/100)))-item_curcost)-offer_amt-(case when sales_hdr.with_tax=0 then 0 else tax_amt end))*IIF(sales_hdr.invtype in('04','05'),1,-1)),0) from sales_hdr inner Join sales_dtl LEFT OUTER JOIN items  ON sales_dtl.itemno=items.item_no on sales_hdr.branch=sales_dtl.branch and sales_hdr.invtype=sales_dtl.invtype and sales_hdr.ref=sales_dtl.ref and sales_hdr.slcenter=sales_dtl.slcenter where sales_hdr.branch='" + BL.CLS_Session.brno + "' and sales_hdr.invtype='" + rw.Cells[1].Value + "' and sales_hdr.ref=" + rw.Cells[2].Value + " and sales_hdr.slcenter='" + rw.Cells[0].Value + "'");
                       // rw.Cells[12].Value = (Math.Round(Convert.ToDouble(dtrw.Rows[0][0].ToString()), 2)).ToString();

                        //else
                        //{
                        //    sumttld = sumttld + Convert.ToDouble(rw.Cells[12].Value);
                        //    dr[12] = Math.Round(sumttld, 2).ToString();
                        //}
                        rw.Cells[13].Value = "%" + Math.Round((Convert.ToDouble(rw.Cells[12].Value.ToString().Trim()) / (Convert.ToDouble(rw.Cells[9].Value.ToString().Trim()))) * 100, 2).ToString();
                   // }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // DataSet ds1 = new DataSet();
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sal.rpt.Sal_Report_WR_rpt.rdlc";
                ...
                parameters[4] = new ReportParameter("par1", "الضريبة");
                parameters[5] = new ReportParameter("par2", "الصافي");
                rf.reportViewer1.LocalReport.SetParameters(parameters);
