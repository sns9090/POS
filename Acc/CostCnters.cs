...
namespace POS.Acc
{
    public partial class CostCnters : BaseForm
    {
       // SqlConnection con1 = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
            ...
            if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        SqlDataAdapter da = new SqlDataAdapter("select * from acc_dtl where cc_no='" + todel + "' and a_brno='" + BL.CLS_Session.brno + "'", con1);
                        DataTable dt = new DataTable();
                        da.Fill(dt);

                        if (dt.Rows.Count >= 1)
                        {
                            MessageBox.Show(" لا يمكن حذف مركز التكلفة لانه مرتبط بحركات ", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else
                        {
        ...
        private void Groups_Load(object sender, EventArgs e)
        {
          ...
          //  sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
            ...
            // sqlConnection.Open();
            sqlDataAdapter = new SqlDataAdapter("select cc_brno,cc_id,cc_name from costcenters where cc_brno='" + BL.CLS_Session.brno + "' and cc_name like '%" + textBox1.Text + "%' order by cc_id", sqlConnection);
