...
namespace POS.Acc
{
    public partial class Acc_QD_Report : BaseForm
    {
        ...
        {
          //  MessageBox.Show(datval.convert_to_yyyyDDdd(txt_tmdate.Text));
                ...
                try
                {
                        ...
                        /*
                        string qstr = "select a.a_key s,a.a_name d"
                                   + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) f"
                                   + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2) g"
                                   + " from accounts a left outer join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key "
                                   + " where a.a_brno='" + BL.CLS_Session.brno + "' and a.glcurrency<>'01' and a.a_type in('2')"
                                   + " group by a.a_key,a.a_name,a.a_opnbal"
                                   + " having round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                   + " order by a.a_key ";
                         */
                        /*
                        ...
                        string qstr = "select a.a_key s,a.a_name d"
                                 + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) f"
                                 + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2) g"
                                 + " from accounts a left outer join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + ""
                                 + " where a.a_brno='" + BL.CLS_Session.brno + "' and a.glcurrency<>'01' and a.a_type in('2')"
                                 + " group by a.a_key,a.a_name,a.a_opnbal"
                                 + " having round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                 + " order by a.a_key ";
                    ...
                    string condallbr = chk_allbr.Checked ? " " : " and a.a_brno='" + BL.CLS_Session.brno + "' ";
                        string condp = rad_posted.Checked ? " and c.a_posted=1 " : (rad_notpostd.Checked ? " and c.a_posted=0 " : " ");
                        // string conft= " a.a_opnbal "
                        ////////string qstr = "select a.a_key s,a.a_name d"
                        ////////         + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) f"
                        ////////         + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2) g"
                        //string qstr = "select a.a_key s,a.a_name d"
                        //           + " ,iif((round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
                        //           + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2))>0,(round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
                        //           + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2)),0) f"
                        //           + " ,iif((round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
                        //           + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2))<0,-(round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
                        //           + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2)),0) g"
                        //         + " from accounts a left outer join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no " + condp + " and " + (chk_date.Checked ? " c.a_hdate" : " c.a_mdate") + " between '" + datval.convert_to_yyyyDDdd(txt_fmdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                        //     //    + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "

                        //         + " where a.glcurrency<>'01' and a.a_type in('2','3') and len(a.a_key)=9 " + condallbr + " "
                        //         + " group by a.a_key,a.a_name,a.a_opnbal"
                        //         + " having round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                        //         + " order by a.a_key ";
                        string qstr = "select a.a_key s,a.a_name d"
                                + " ,iif(round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)>0,round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2),0.00) f"
                                + " ,iif(round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<0,-round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2),0.00) g"
                                  + " from accounts a left join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key left join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no " + condp + " and " + (chk_date.Checked ? " c.a_hdate" : " c.a_mdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                            //    + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "

                               //   + " where a.a_brno='" + BL.CLS_Session.brno + "' and a.glcurrency<>'01' and a.a_type in('2')"
                                  + " where a.a_type in('2','3') " + condallbr + " and len(a.a_key)=9"
                                  + " group by a.a_key,a.a_name,a.a_opnbal"
                                  + " having round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                  + " order by a.a_key ";
                        dtf = daml.SELECT_QUIRY_only_retrn_dt(qstr);
                        ...
                        if (!BL.CLS_Session.is_dorymost.Equals("2"))
                        {
                           ...
                           ////// dr1[0] = "";
                           ////// dr1[1] = "قيمة المخزون الافتتاحي";
                           ////// //dr1[2] = Convert.ToDouble(dtmk.Rows[0][0]) < 0 ? Convert.ToDouble(dtmk.Rows[0][0]) * -1 : 0;
                           ////// //dr1[3] = Convert.ToDouble(dtmk.Rows[0][0]) > 0 ? Convert.ToDouble(dtmk.Rows[0][0]) : 0;
                           ////// dr1[2] = 0;
                           //////// dr1[3] = Convert.ToDouble(dtmk.Rows[0][0]);
                           ////// dr1[3] =Math.Round(Convert.ToDouble(txt_mko.Text),2);
                           ////// // dr[4] = sumtrd;
                           ////// // dr[5] = sumtrm;
                           ////// // dr[6] = sumttld;
                           ////// // dr[7] = sumttlm;
                           ...
                           ////// dtf.Rows.Add(dr1);
                            ...
                            DataRow dr0 = dtf.NewRow();

                            dr0[0] = "";
                            dr0[1] = "قيمة المخزون";
                            //dr0[2] = Convert.ToDouble(txt_mkhzon.Text) < 0 ? Convert.ToDouble(txt_mkhzon.Text) * -1 : 0;
                            //dr0[3] = Convert.ToDouble(txt_mkhzon.Text) > 0 ? Convert.ToDouble(txt_mkhzon.Text) : 0;
                            dr0[3] = Convert.ToDouble(txt_mkhzon.Text);
                            dr0[2] = 0;
                            // dr[4] = sumtrd;
                            // dr[5] = sumtrm;
                            // dr[6] = sumttld;
                        ...
                        foreach (DataRow dtr in dtf.Rows)
                        {
                        ...
                        sumnet = sumopnd - sumopnm;
                        DataRow dr2 = dtf.NewRow();
                        dr2[0] = "";
                       // dr2[1] = sumnet > 0 ? "مجمل ربح" : sumnet < 0 ? "مجمل خسارة" : "نقطة تعادل";
                        dr2[1] = sumnet > 0 ? "صافي ربح" : sumnet < 0 ? "صافي خسارة" : "نقطة تعادل";
                        dr2[2] = sumnet > 0 ? sumnet : 0;
                        dr2[3] = sumnet < 0 ? -sumnet : 0;
                        dtf.Rows.Add(dr2);
                        ...
                        if (!BL.CLS_Session.is_dorymost.Equals("2"))
                        dataGridView1.Rows[dataGridView1.Rows.Count - 3].DefaultCellStyle.BackColor = Convert.ToDouble(txt_mkhzon.Text) > 0 ? Color.LightGreen : Convert.ToDouble(txt_mkhzon.Text) < 0 ? Color.Pink : Color.SkyBlue;
                        dataGridView1.Rows[dataGridView1.Rows.Count - 2].DefaultCellStyle.BackColor = Color.LightSteelBlue;
                        dataGridView1.Rows[dataGridView1.Rows.Count - 1].DefaultCellStyle.BackColor = sumnet > 0 ? Color.LightGreen : sumnet < 0 ? Color.Pink : Color.SkyBlue;
                        ...
                        if (!arbah)
                        {
                            ...
                            // string condp2 = rad_posted.Checked ? " and c.a_posted=1 " : (rad_notpostd.Checked ? " and c.a_posted=0 " : " ");
                            // string conft= " a.a_opnbal "
                            //////string qstr2 = "select a.a_key s,a.a_name d"
                            //////         + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) f"
                            //////         + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2) g"
                            string qstr2 = "select a.a_key s,a.a_name d"
                               + " ,iif((round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
                               ...
                               + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2))<0,-(round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
                               + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2)),0) f"
                                     + " from accounts a  join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no " + condp + " and " + (chk_date.Checked ? " c.a_hdate" : " c.a_mdate") + " between '" + datval.convert_to_yyyyDDdd(txt_fmdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                                //  + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "

                                     + " where a.glcurrency<>'01' and a.a_type in('3') and len(a.a_key)=9 " + condallbr + " "
                                     + " group by a.a_key,a.a_name,a.a_opnbal"
                                     + " having round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                     + " order by a.a_key ";
                            //dtf.Clear();

                            dtf2 = daml.SELECT_QUIRY_only_retrn_dt(qstr2);

                            DataRow dr222 = dtf2.NewRow();
                            dr222[0] = "";
                            dr222[1] = sumnet > 0 ? "مجمل ربح" : sumnet < 0 ? "مجمل خسارة" : "نقطة تعادل";
                            dr222[2] = sumnet > 0 ? sumnet : 0;
                            dr222[3] = sumnet < 0 ? -sumnet : 0;

                            dtf2.Rows.InsertAt(dr222, 0);

                            DataRow dr23 = dtf2.NewRow();
                            double sumopnd2 = 0, sumopnm2 = 0, sumnet2 = 0;//, sumtrd = 0, sumtrm = 0, sumttld = 0, sumttlm = 0;
                            foreach (DataRow dtr in dtf2.Rows)
                            {
                                sumopnd2 = sumopnd2 + Convert.ToDouble(dtr[3]);
                                sumopnm2 = sumopnm2 + Convert.ToDouble(dtr[2]);
                                //  sumtrd = sumtrd + Convert.ToDouble(dtr[4]);
                                //  sumtrm = sumtrm + Convert.ToDouble(dtr[5]);
                                //  sumttld = sumttld + Convert.ToDouble(dtr[6]);
                                //  sumttlm = sumttlm + Convert.ToDouble(dtr[7]);
                            ...
                            sumnet2 = sumopnm2 - sumopnd2;
                            DataRow dr22 = dtf2.NewRow();
                            dr22[0] = "";
                            dr22[1] = sumnet2 > 0 ? "صافي ربح" : sumnet2 < 0 ? "صافي خسارة" : "نقطة تعادل";
                            dr22[2] = sumnet2 > 0 ? sumnet2 : 0;
                            dr22[3] = sumnet2 < 0 ? -sumnet2 : 0;
                            dtf2.Rows.Add(dr22);




                            dataGridView1.DataSource = dtf2;
                            //dataGridView1.Columns[1].Width= 400;
                            //dataGridView1.Columns[0].Width = 200;
                            //dataGridView1.Columns[2].Width = 150;
                            //dataGridView1.Columns[3].Width = 150;
                            dataGridView1.Rows[0].DefaultCellStyle.BackColor = sumnet > 0 ? Color.LightGreen : sumnet < 0 ? Color.Pink : Color.SkyBlue;
                            dataGridView1.Rows[dataGridView1.Rows.Count - 2].DefaultCellStyle.BackColor = Color.LightSteelBlue;
                            dataGridView1.Rows[dataGridView1.Rows.Count - 1].DefaultCellStyle.BackColor = sumnet2 > 0 ? Color.LightGreen : sumnet2 < 0 ? Color.Red : Color.SkyBlue;
                            dataGridView1.ClearSelection();
        ...
        private void Load_Statment()
        {
          ...
          //  double seso = 0;
            try
            {
                ...
                dataGridView1.DataSource = daml.SELECT_QUIRY_only_retrn_dt(qstr);
        ...
        private void SalesReport_Load(object sender, EventArgs e)
        {
            ...
            string query = "select isnull(sum(round(((b.qty + b.openbal) * a.item_curcost),2)),0) mkh from items a WITH (NOLOCK) join whbins b WITH (NOLOCK) on a.item_no=b.item_no where br_no='" + BL.CLS_Session.brno + "'";


            //DataTable dtmk = daml.SELECT_QUIRY_only_retrn_dt(" select (select isnull(sum(d.openbal * openlcost),0) from whbins d where d.br_no='" + BL.CLS_Session.brno + "') mko,(select isnull(sum(d.openbal * openlcost),0) from whbins d where d.br_no='" + BL.CLS_Session.brno + "') + ((select  isnull(sum(iif(c.invtype in ('06','07') ,(c.qty*c.pkqty*a.item_curcost),-(c.qty*c.pkqty*a.item_curcost))),0) from pu_dtl c join items a on a.item_no=c.itemno where c.branch='" + BL.CLS_Session.brno + "' and " + (chk_date.Checked ? " c.invhdate" : " c.invmdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "') "
            //                                                     + "- (select  isnull(sum(iif(b.invtype in ('04','05') ,(b.qty*b.pkqty*a.item_curcost),-(b.qty*b.pkqty*a.item_curcost))),0) from sales_dtl b join items a on a.item_no=b.itemno where b.branch='" + BL.CLS_Session.brno + "' and " + (chk_date.Checked ? " b.invhdate" : " b.invmdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "') "
            //                                                     + "+ (select  isnull(sum(b.qty*pkqty*a.item_curcost),0) from sto_dtl b join items a on a.item_no=b.itemno where   b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('31')  and " + (chk_date.Checked ? "b.trhdate" : " b.trmdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "') "
            //                                                     + "+ (select  isnull(sum(b.qty*pkqty*a.item_curcost),0) from sto_dtl b join items a on a.item_no=b.itemno where   b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('35')  and " + (chk_date.Checked ? "b.trhdate" : " b.trmdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "') "
            //                                                     + "- (select  isnull(sum(b.qty*pkqty*a.item_curcost),0) from sto_dtl b join items a on a.item_no=b.itemno where   b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('32')  and " + (chk_date.Checked ? "b.trhdate" : " b.trmdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "')) ");
            DataTable dtmk = daml.SELECT_QUIRY_only_retrn_dt(query);
            ////////string query = "select isnull(sum(round(((b.qty + b.openbal) * a.item_curcost),2)),0) mkh,isnull(sum(round((b.openbal * a.item_curcost),2)),0) mko from items a WITH (NOLOCK) join whbins b WITH (NOLOCK) on a.item_no=b.item_no where br_no='" + BL.CLS_Session.brno + "'";

            ////////DataTable dtmk = daml.SELECT_QUIRY_only_retrn_dt(query);
            //////  MessageBox.Show(dtmk.Rows[0][0].ToString());
            if (BL.CLS_Session.is_dorymost.Equals("2"))
                txt_mkhzon.Text = "0.00";
            else
            {
                txt_mkhzon.Text = Math.Round(Convert.ToDouble(dtmk.Rows[0][0]), 2).ToString();
                // txt_mkhzon.Text = dtmk.Rows[0][1].ToString();
               // txt_mkhzon.Text =Math.Round(Convert.ToDouble(dtmk.Rows[0][1]), 2).ToString();
               // txt_mko.Text = Math.Round(Convert.ToDouble(dtmk.Rows[0][0]), 2).ToString();
            }
           ...
           //DataTable dt = daml.SELECT_QUIRY_only_retrn_dt(query);
           // txt_mkhzon.Text = dt.Rows[0][0].ToString();
           // double sum = 0;
           // double seso = 0;
           // try
           // {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                */
                ...
                workSheet.Range["A1", "D1"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
                workSheet.Range["A2", "D2"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
                workSheet.Range["A3", "D3"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
                if (chk_m.Checked)
                {
                    workSheet.Range["A" + (dataGridView1.Rows.Count + 3).ToString(), "D" + (dataGridView1.Rows.Count + 3).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormatReport7);
                    workSheet.Range["A" + (dataGridView1.Rows.Count + 4).ToString(), "D" + (dataGridView1.Rows.Count + 4).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
                    workSheet.Range["A" + (dataGridView1.Rows.Count + 5).ToString(), "D" + (dataGridView1.Rows.Count + 5).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects1);
                }
                else
                {
                    workSheet.Range["A" + 6, "D" + 6].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects1);
                    workSheet.Range["A" + (dataGridView1.Rows.Count + 4).ToString(), "D" + (dataGridView1.Rows.Count + 4).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
                    workSheet.Range["A" + (dataGridView1.Rows.Count + 5).ToString(), "D" + (dataGridView1.Rows.Count + 5).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormatReport8);
                }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // rf.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("account", dth));
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Acc.rpt.Acc_QD_Report_rpt.rdlc";
               ...
               // parameters[3] = new ReportParameter("todate", maskedTextBox2.Text);
                parameters[1] = new ReportParameter("br_name", BL.CLS_Session.brname);
                parameters[2] = new ReportParameter("frmdate", txt_fmdate.Text);
                parameters[3] = new ReportParameter("todate", txt_tmdate.Text);
...
*/
        ...
        private void txt_mkhzon_TextChanged(object sender, EventArgs e)
        {
           
        }

        private void txt_mkhzon_Validating(object sender, CancelEventArgs e)
        {
            datval.ValidateText_float(txt_mkhzon);
        }
