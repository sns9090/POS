    ...
    public partial class Acc_Final_SC_Report : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
          //  MessageBox.Show(datval.convert_to_yyyyDDdd(txt_tmdate.Text));
            ...
            if (cmb_sc.SelectedIndex == -1)
            {
                MessageBox.Show("يرجى اختيار القسم او القطاع","",MessageBoxButtons.OK,MessageBoxIcon.Error);
                cmb_sc.Focus();
                return;
            }
                ...
                try
                {
                    ...
                    if (chk_ak.Checked || chk_m.Checked)
                    {
                        ...
                        string qstr = "select a.a_key s,a.a_name d"
                                 ...
                                 + " order by a.a_key ";
                        */
                       // DataTable stn = daml.SELECT_QUIRY_only_retrn_dt("");
                        ...
                        string qstr = "select a.a_key s,a.a_name d"
                               ...
                               + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2)),0) g"
                                 + " from accounts a left outer join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key left join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no " + condp + " and " + (chk_date.Checked ? " c.a_hdate" : " c.a_mdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                            //    + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                              ...
                              //   + " where a.a_brno='" + BL.CLS_Session.brno + "' and a.glcurrency<>'01' and a.a_type in('2')"
                                 + " where a.a_type in('2') and section='" + cmb_sc.SelectedValue + "' " + condallbr + " and len(a.a_key)=9"
                                 + " group by a.a_key,a.a_name,a.a_opnbal"
                                 + " having round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                 + " order by a.a_key ";
                        ...
                        dr3[0] = "";
                        dr3[1] = "صافي البضاعة الصادرة والواردة";
                        dr3[2] = sumn < 0 ? sumn : 0;
                        dr3[3] = sumn > 0 ? sumn : 0;
                        dtf.Rows.Add(dr3);
                        ...
                        dataGridView1.Rows[dataGridView1.Rows.Count - 4].DefaultCellStyle.BackColor = Color.SkyBlue;
                        ...
                        if (chk_ak.Checked)
                        {
                               ...
                               + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2)),0) g"
                                ...
                                //  + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                                   ...
                                   //  + " where a.a_brno='" + BL.CLS_Session.brno + "' and a.glcurrency<>'01' and a.a_type in('3')"
                                     + " where a.a_type in('3') and section='"+cmb_sc.SelectedValue+"' " + condallbr + " and len(a.a_key)=9"
                                     + " group by a.a_key,a.a_name,a.a_opnbal"
                                     + " having round(a.a_opnbal + isnull(sum(case b.jddbcr when 'D' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                     + " order by a.a_key ";
                            //dtf.Clear();
                            ...
                            {
                                sumopnd2 = sumopnd2 + Convert.ToDouble(dtr[2]);
                                sumopnm2 = sumopnm2 + Convert.ToDouble(dtr[3]);
                                //  sumtrd = sumtrd + Convert.ToDouble(dtr[4]);
                                //  sumtrm = sumtrm + Convert.ToDouble(dtr[5]);
                                //  sumttld = sumttld + Convert.ToDouble(dtr[6]);
                                //  sumttlm = sumttlm + Convert.ToDouble(dtr[7]);
                    ...
                    if (chk_mm.Checked)
                    {
                        ...
                        ////////         + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) f"
                        ////////         + " ,round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2) g"
                        ////////         + " from accounts a left outer join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no " + condp + " and " + (chk_date.Checked ? " c.a_hdate" : " c.a_mdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                        ////////    //    + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                        string qstr = "select a.a_key s,a.a_name d"
                               + " ,iif((round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
                               + " round(a.a_opnbal+isnull(sum(case b.jddbcr when 'D'then b.a_damt else 0 end),0),2))>0,(round(a.a_opnbal+isnull(sum(case b.jddbcr when 'C'then b.a_camt else 0 end),0),2) - "
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // parameters[3] = new ReportParameter("todate", maskedTextBox2.Text);
                ...
                parameters[2] = new ReportParameter("rpt_type", chk_m.Checked ? chk_m.Text + " " + cmb_sc.Text : chk_ak.Checked ? chk_ak.Text + " " + cmb_sc.Text : chk_mm.Text);
...
*/
        ...
        private void cmb_sc_Leave(object sender, EventArgs e)
        {
            string query = "select isnull(sum(round(((b.qty + b.openbal) * b.lcost),2)),0) mkh from whbins b join whouses w on b.wh_no=w.wh_no where br_no='"+BL.CLS_Session.brno+"' and w.sc_code='"+cmb_sc.SelectedValue+"'";
            ...
            dtn = daml.SELECT_QUIRY_only_retrn_dt("select [wh_no] from whouses where wh_brno='" + BL.CLS_Session.brno + "' and sc_code='" + cmb_sc.SelectedValue + "'");

            foreach (DataRow dtr in dtn.Rows)
            {
                dtns= daml.SELECT_QUIRY_only_retrn_dt("select isnull(sum((case when trtype in('31','33') and towhno='01' then amnttl else 0 end) - (case when trtype in('32','33') and whno='01' then amnttl else 0 end)),0) from sto_hdr where branch='"+BL.CLS_Session.brno+"' and (towhno='"+dtr[0].ToString()+"' or whno='"+dtr[0].ToString()+"')");
                sumn = sumn + Convert.ToDouble(dtns.Rows[0][0].ToString());
            }
