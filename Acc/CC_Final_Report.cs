...
namespace POS.Acc
{
    public partial class CC_Final_Report : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
          //  MessageBox.Show(datval.convert_to_yyyyDDdd(txt_tmdate.Text));
                ...
                try
                {
                    if (txt_ccno.Text == "")
                    {
                        MessageBox.Show("يجب ادخال رقم مركز التكلفة", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_ccno.Focus();
                        return;
                    ...
                    DataTable dtmk = daml.SELECT_QUIRY_only_retrn_dt(" select (select isnull(sum(d.openbal * openlcost),0) from whbins d where d.br_no='" + BL.CLS_Session.brno + "') mko,(select isnull(sum(d.openbal * openlcost),0) from whbins d where d.br_no='" + BL.CLS_Session.brno + "') + ((select  isnull(sum(iif(c.invtype in ('06','07') ,(c.qty*c.pkqty*a.item_curcost),-(c.qty*c.pkqty*a.item_curcost))),0) from pu_dtl c join items a on a.item_no=c.itemno where c.branch='" + BL.CLS_Session.brno + "' and " + (chk_date.Checked ? " c.invhdate" : " c.invmdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "') "
                                                                        ...
                                                                        + "- (select  isnull(sum(b.qty*pkqty*a.item_curcost),0) from sto_dtl b join items a on a.item_no=b.itemno where   b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('32')  and " + (chk_date.Checked ? "b.trhdate" : " b.trmdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "')) from whouses where cstcode='" + txt_ccno.Text + "'");
                    ...
                    else
                    {
                        ...
                        txt_mkhzon.Text = Convert.ToDouble(txt_mkhzon.Text) == 0 ? Math.Round(Convert.ToDouble(dtmk.Rows[0][1]), 2).ToString() : txt_mkhzon.Text;
                        ...
                        ////////string qstr = "select a.a_key s,a.a_name d"
                        ////////         + " ,round((select 0+isnull(sum(case when b.jddbcr='C' and b.cc_no='" + txt_ccno.Text + "' then b.a_camt else 0 end),0)),2) f"
                        ////////         + " ,round((select 0+isnull(sum(case when b.jddbcr='D' and b.cc_no='" + txt_ccno.Text + "' then b.a_damt else 0 end),0)),2)  g"
                        string qstr = "select a.a_key s,a.a_name d"
                                     + " ,iif((round(0+isnull(sum(case when b.jddbcr='C' and b.cc_no='" + txt_ccno.Text + "' then b.a_camt else 0 end),0),2) - "
                                     + " round(0+isnull(sum(case when b.jddbcr= 'D' and b.cc_no='" + txt_ccno.Text + "' then b.a_damt else 0 end),0),2))>0,(round(0+isnull(sum(case when b.jddbcr= 'C' and b.cc_no='" + txt_ccno.Text + "' then b.a_camt else 0 end),0),2) - "
                                     + " round(0+isnull(sum(case when b.jddbcr= 'D' and b.cc_no='" + txt_ccno.Text + "' then b.a_damt else 0 end),0),2)),0) g"
                                     + " ,iif((round(0+isnull(sum(case when b.jddbcr= 'C' and b.cc_no='" + txt_ccno.Text + "' then b.a_camt else 0 end),0),2) - "
                                     + " round(0+isnull(sum(case when b.jddbcr= 'D' and b.cc_no='" + txt_ccno.Text + "' then b.a_damt else 0 end),0),2))<0,-(round(0+isnull(sum(case when b.jddbcr= 'C' and b.cc_no='" + txt_ccno.Text + "' then b.a_camt else 0 end),0),2) - "
                                     + " round(0+isnull(sum(case when b.jddbcr= 'D' and b.cc_no='" + txt_ccno.Text + "' then b.a_damt else 0 end),0),2)),0) f"
                                 + " from accounts a left join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key left join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no and b.cc_no='" + txt_ccno.Text + "' " + condp + " and " + (chk_date.Checked ? " c.a_hdate" : " c.a_mdate") + "<='" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                             //    + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                                 ...
                                 + " where a.a_brno='" + BL.CLS_Session.brno + "' and a.glcurrency<>'01' and a.a_type in('2')"
                                 + " group by a.a_key,a.a_name,b.cc_no"
                                 + " having round(0 + isnull(sum(case when b.jddbcr='D' and b.cc_no='" + txt_ccno.Text + "' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                 + " order by a.a_key ";
                        ...
                        dr0[2] = Convert.ToDouble(txt_mkhzon.Text);
                        dr0[3] = 0;
                        // dr[4] = sumtrd;
                        // dr[5] = sumtrm;
                        // dr[6] = sumttld;
                       ...
                       // string condp2 = rad_posted.Checked ? " and c.a_posted=1 " : (rad_notpostd.Checked ? " and c.a_posted=0 " : " ");
                        ...
                        string qstr2 = "select a.a_key s,a.a_name d"
                               ...
                               //  + " from accounts a join acc_dtl b on a.a_brno=b.a_brno and a.a_key=b.a_key join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " and c.a_mdate <= '" + datval.convert_to_yyyyDDdd(txt_tmdate.Text) + "' "
                                 ...
                                 + " where a.a_brno='" + BL.CLS_Session.brno + "' and a.glcurrency<>'01' and a.a_type in('3')"
                                 + " group by a.a_key,a.a_name,b.cc_no"
                                 + " having round(0 + isnull(sum(case when b.jddbcr='D' and b.cc_no='" + txt_ccno.Text + "' then b.a_damt else -(b.a_camt) end),0),2)<>0"
                                 + " order by a.a_key ";
                      //dtf.Clear();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // rf.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("account", dth));
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Acc.rpt.Acc_Final_Report_rpt.rdlc";
              ...
              //  parameters[2] = new ReportParameter("rpt_type", chk_m.Checked? chk_m.Text: chk_kd.Checked? chk_kd.Text : chk_mm.Text);
                parameters[2] = new ReportParameter("rpt_type", (chk_m.Checked ? chk_m.Text : chk_kd.Checked ? chk_kd.Text : chk_mm.Text) + "   " + txt_ccno.Text + " - " + txt_ccname.Text);
...
*/
        ...
        private void txt_ccno_DoubleClick(object sender, EventArgs e)
        {
            Search_All f4 = new Search_All("M", "");
            f4.ShowDialog();
