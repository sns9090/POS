...
namespace POS.Acc
{
    public partial class Acc_Statment_All : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (txt_code.Text == "")
            {
                MessageBox.Show(BL.CLS_Session.lang.Equals("E")?"Enter Account No.":"يجب ادخال رقم الحساب", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txt_code.Focus();
                return;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
          ...
          // dataGridView2.DataSource = dt;
               ...
               // parameters[1] = new ReportParameter("mdate", dthdr.Rows[0][3].ToString().Substring(6, 2) + "- " + dthdr.Rows[0][3].ToString().Substring(4, 2) + "- " + dthdr.Rows[0][3].ToString().Substring(0, 4));
                ...
                parameters[6] = new ReportParameter("openbal", "01-01-" + cmb_fyr.Text);
            ...
            if (aakey != "")
            {
                ...
                try
                {
                    ...
                    foreach (DataRow dtrow in dtr2.Rows)
                    {
                        ...
                        dataGridView1.DataSource = daml.SELECT_QUIRY(qstr2);
                        */
                        string qstr2, condt, condp, conddm;
                        ...
                        //////}
                            qstr2 = "select a.a_type c1, a.a_ref c2, CONVERT(VARCHAR(10), CAST(a.a_mdate as date), 105) c3, (substring(a.a_hdate,7,2) + '-' + substring(a.a_hdate,5,2)+'-'+substring(a.a_hdate,1,4)) c4,a.a_text c5,round((case a.jddbcr when 'D' then a.a_damt else - a.a_camt end),2) c6,b.a_posted,a.a_type a_t,b.jhsrc src,b.sl_no sl,b.pu_no pu,'" + dtrow[0].ToString() + "' yr  from acc_dtl a,acc_hdr b where a.a_brno=b.a_brno and a.a_ref=b.a_ref and a.a_type=b.a_type and a.sl_no=b.sl_no and a.pu_no=b.pu_no " + condp + conddm + condt + " and a.a_key='" + aakey + "' and a.a_brno='" + BL.CLS_Session.brno + "' and a.a_text like '%" + txt_desc.Text.Replace(" ", "%") + "%' order by a.a_mdate,a.a_sysdate";
                         ...
                         //   dth = daml.SELECT_QUIRY_only_retrn_dt("select a_name,round(a_opnbal,2) a_opnbal,a_key  from accounts where a_key='" + aakey + "' and a_brno='" + BL.CLS_Session.brno + "'");
                           ...
                           // MessageBox.Show(count.ToString());
                            if (count == 0)
                            {
                                SqlDataAdapter darown = new SqlDataAdapter("select a_name,round(a_opnbal,2) a_opnbal,a_key  from accounts where a_key='" + aakey + "' and a_brno='" + BL.CLS_Session.brno + "'" + conkey + "", contyr);
                                SqlDataAdapter darown2 = new SqlDataAdapter("select a_name,round(a_opnbal,2) a_opnbal,a_key  from accounts where a_key='" + aakey + "' and a_brno='" + BL.CLS_Session.brno + "'" + conkey + "", con3);
                                // darown.Fill(dth);
                                DataTable dtn = new DataTable();
                                DataTable dtn2 = new DataTable();
                                darown.Fill(dtn);
                                darown2.Fill(dtn2);
                                isaccpro = dtn.Rows.Count;
                                if (dtn.Rows.Count > 0)
                                {
