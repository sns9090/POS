...
namespace POS.Acc
{
    public partial class Acc_Tax_Tran : BaseForm
    {
       // DataTable dtg;
       // Timer timer;
        ...
        public Acc_Tax_Tran(string atype, string aref,string condi)
        {
            InitializeComponent();
           ...
           // dtg = dataGridView1.DataSource;
            ...
            try
            {
               ...
               // dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('01','02','03')");
                ...
                string whatever = str2.Substring(str2.IndexOf("A124") + 5, 3);
    ...
    */
                  //  if (!row.IsNewRow && row.Cells[0].Value != null)
                    if (!row.IsNewRow && !string.IsNullOrEmpty(row.Cells[0].Value.ToString() + row.Cells[1].Value.ToString() + row.Cells[2].Value.ToString()))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text,  a_damt, a_camt,a_folio,a_mdate,a_hdate,jddbcr,taxcatId,su_no,cu_no,cc_no) VALUES(@a0,@a1,@a2,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@su,@cu,@cc)", con))
                        {
        ...
        private void btn_Edit_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!dthdr.Rows[0][8].ToString().Equals("Acct") && !dthdr.Rows[0][8].ToString().Equals("Acc1"))
                {
        ...
        {
            btn_Rfrsh_Click(sender, e);
            if (!dthdr.Rows[0][8].ToString().Equals("Acct"))
            {
        ...
        private void Fill_Data(string atype, string aref, string acondi)
        {
            ...
            try
            {
              ...
              //  hdate = txt_hdate.Text.Replace("-", "").Substring(4, 4) + txt_hdate.Text.Replace("-", "").Substring(2, 2) + txt_hdate.Text.Replace("-", "").Substring(0, 2);
                ...
                dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select cast(aa.cu_no as varchar) cust,cast(aa.su_no as varchar) sup,cast(aa.a_key as varchar) a_key,bb.a_name a_name,aa.a_text a_text,aa.a_damt a_damt,aa.a_camt a_camt,iif(aa.taxcatId = '0',null,aa.taxcatId) taxcatId,bb.acckind acckind,(case when aa.cu_no <> '' then aa.a_key when aa.su_no <> '' then aa.a_key else '' end) acckey from acc_dtl aa,accounts bb where aa.a_key=bb.a_key and aa.a_type='" + atype + "' and aa.a_ref=" + aref + " and aa.a_brno='" + BL.CLS_Session.brno + "'and aa." + condit + " order by aa.a_folio");
        ...
        //    doc = null;
            ...
            {/*
              ...
              //rf.reportViewer1.LocalReport.ReportEmbeddedResource
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Acc.rpt.Acc_Tran_All_rpt.rdlc";
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    sumc = sumc + Convert.ToDouble(row.Cells[5].Value);
                    sumd = sumd + Convert.ToDouble(row.Cells[6].Value);
                }
