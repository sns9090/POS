...
namespace POS.Acc
{
    public partial class CC_Balance : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (txt_ccno.Text == "")
            {
                ...
                */
                ...
                //string qstr = "select a.a_key d1,a.a_name d2,round(a.a_opnbal +(select isnull(sum(a_damt-a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " where a.a_key=b.a_key and " + cond + "<='" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'),2) d3"
                //            + " from accounts a where round(a.a_opnbal +(select isnull(sum(a_damt-a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type " + condp + " where a.a_key=b.a_key and " + cond + "<='" +datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'),2) <>0 "
                //            + " and a_brno='" + BL.CLS_Session.brno + "' " + condtax + " and a.a_key like '" + textBox3.Text + "%'"
                //            + " order by a.a_key;";
                string qstr = "select a.a_key d1,a.a_name d2,round((select isnull(sum(a_damt-a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no " + condp + " where a.a_key=b.a_key and b.cc_no='" + txt_ccno.Text + "' and " + cond + "<='" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'),2) d3"
                           + " from accounts a where round((select isnull(sum(a_damt-a_camt),0) from acc_dtl b join acc_hdr c on c.a_brno=b.a_brno and c.a_ref=b.a_ref and c.a_type=b.a_type and c.sl_no=b.sl_no and c.pu_no=b.pu_no " + condp + " where a.a_key=b.a_key and b.cc_no='" + txt_ccno.Text + "' and " + cond + "<='" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'),2) <>0 "
                           + " and a_brno='" + BL.CLS_Session.brno + "' " + condtax + " and a.a_key like '" + textBox3.Text + "%'"
                           + " order by a.a_key;";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                */
                ...
                workSheet.Range["A" + (dataGridView1.Rows.Count +5 ).ToString(), "C" +( dataGridView1.Rows.Count + 5 ).ToString()].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);

                workSheet.Range["A5", "C5"].AutoFormat(Microsoft.Office.Interop.Excel.XlRangeAutoFormat.xlRangeAutoFormat3DEffects2);
                // workSheet.Cells[2, "D"]
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //  parameters[1] = new ReportParameter("mdate", dthdr.Rows[0][3].ToString().Substring(6, 2) + "- " + dthdr.Rows[0][3].ToString().Substring(4, 2) + "- " + dthdr.Rows[0][3].ToString().Substring(0, 4));
                ...
                parameters[3] = new ReportParameter("desc", "ارصدة حسابات م التكلفة" +"   " + txt_ccno.Text + " - " +txt_ccname.Text);
