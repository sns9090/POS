...
namespace POS.Sto
{
    public partial class Sto_Whbins_Br : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (cmb_grp.SelectedIndex == -1)
                qstr = "select b.br_no br_no,b.br_name br_name, nettotal=round(sum((" + condopenbal + ") * i.item_curcost),2),tax=round(sum((" + condopenbal + ")),2),salcost =cast (round(sum((" + condopenbal + ") * i.item_price),2) as decimal(10,2) ),win = 0 ,winp ='' from  branchs b join whbins c on b.br_no=c.br_no join items i on i.item_no = c.item_no where b.br_no is not null " + condbr + condgrp + condsgrp + condwh + " group By b.br_no,b.br_name order By b.br_no";
            else
                qstr = "Select b.wh_no br_no,b.wh_name br_name, nettotal=round(sum((" + condopenbal + ") * i.item_curcost),2),tax=round(sum((" + condopenbal + ")),2),salcost =cast (round(sum((" + condopenbal + ") * i.item_price),2) as decimal(10,2)),win = 0 ,winp ='' from  whouses b join whbins c on c.br_no = b.wh_no  join items i on i.item_no=c.item_no where b.br_no is not null " + condbr + condgrp + condsgrp + condwh + "  group By b.wh_no,b.wh_name order By b.wh_no";
        ...
        private void chk_allsalctr_CheckedChanged(object sender, EventArgs e)
        {
