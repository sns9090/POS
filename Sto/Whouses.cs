...
namespace POS.Sto
{
    public partial class Whouses : BaseForm
    {
       // SqlConnection con1 = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
            ...
            if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        SqlDataAdapter da = new SqlDataAdapter("select (select count(*) from whbins where wh_no='" + todel + "' and br_no='" + BL.CLS_Session.brno + "') + (select count(*) from sales_dtl where whno = '" + todel + "' and branch='" + BL.CLS_Session.brno + "') + (select count(*) from pu_dtl where whno = '" + todel + "' and branch='" + BL.CLS_Session.brno + "') + (select count(*) from sto_dtl where whno = '" + todel + "' or towhno='" + todel + "' and branch='" + BL.CLS_Session.brno + "')", con1);
                        ...
                        {
                            MessageBox.Show(" لا يمكن حذف مخزن لديه مخزون ", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else
                        {
        ...
        private void Groups_Load(object sender, EventArgs e)
        {
          ...
          //  sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
            ...
            // sqlConnection.Open();
            sqlDataAdapter = new SqlDataAdapter("select wh_brno,wh_no,wh_name,mkhzon_acc,cstcode,sc_code,frq_sar_acc from whouses where wh_brno='" + BL.CLS_Session.brno + "' and wh_name like '%" + textBox1.Text + "%' order by wh_no", sqlConnection);
        ...
        {
            if (dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[3] || dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[6])
            {
            ...
            {
                Search_All f4 = new Search_All("SC", " ");
                f4.ShowDialog();
                //dataGridView1.CurrentRow.Cells[10].Value = f4.dataGridView1.CurrentRow.Cells[0].Value;
                dataGridView1.CurrentCell.Value = f4.dataGridView1.CurrentRow.Cells[0].Value;
            }
