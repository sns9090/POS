...
namespace POS.Sto
{
    public partial class Sto_Whbins_RM : BaseForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                /*
                if (rad_stopd.Checked)
                {
                    DataTable dtrm = daml.SELECT_QUIRY_only_retrn_dt("get_whbins_RM @r_or_m = '0',@brno = '" + BL.CLS_Session.brno + "' ");
                    dataGridView1.DataSource = dtrm;
                
                }
                else
                {
                    DataTable dtrm2 = daml.SELECT_QUIRY_only_retrn_dt("get_whbins_RM @r_or_m = '1',@brno = '" + BL.CLS_Session.brno + "' ");
                    dataGridView1.DataSource = dtrm2;
                }
                ...
                // string condopnbalonly = chk_opnbalonly.Checked ? " and b.openbal <> 0 " : " ";
                ...
                //if(rad_stopd.Checked)
                //{
                query = @"select top " + txt_count.Text + " a.item_no a1,a.item_name a2,a.item_obalance a3,("
                      + "(select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('06','07')) "
                      + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('14','15'))"
                      + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('31','45'))) a4,"
                      + "((select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('16','17')) "
                      + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('04','05'))"
                      + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('32','46'))"
                      + ") a5,'' a6,0 a9, round(a.item_curcost,2) a7,b.su_name a8  from items a left join suppliers b on a.supno = b.su_no where b.su_brno='" + BL.CLS_Session.brno + "' " + congrp + congrsp + conit + consup + ""
                      + " and "
                      + "a.item_obalance + ((select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('06','07')) "
                      + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('14','15'))"
                      + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('31','45')))"
                      + "+((select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('16','17')) "
                      + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('04','05'))"
                      + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('32','46'))) <>0"
                      + " order by "
                      + "a.item_obalance + ((select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('06','07')) "
                      + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('14','15'))"
                      + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('31','45')))"
                      + "-((select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('16','17')) "
                      + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('04','05'))"
                      + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('32','46'))) " + (rad_stopd.Checked ? "DESC" : "ASC") + "";

                //}
                //else
                //{
                //    query = @"select top 100 a.item_no a1,a.item_name a2,a.item_obalance a3,("
                //          + "(select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('06','07')) "
                //          + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('14','15'))"
                //          + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('31','45'))) a4,"
                //          + "((select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('16','17')) "
                //          + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('04','05'))"
                //          + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('32','46'))"
                //          + ") a5,0 a6, round(a.item_curcost,2) a7,b.su_name a8  from items a left join suppliers b on a.supno = b.su_no where b.su_brno='" + BL.CLS_Session.brno + "' " + congrp + congrsp + conit + consup + ""
                //          + " and "
                //          + "a.item_obalance + ((select  isnull(sum(c.qty*pkqty),0) from pu_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('06','07')) "
                //          + "+ (select  isnull(sum(c.qty*pkqty),0) from sales_dtl c where a.item_no=c.itemno and c.branch='" + BL.CLS_Session.brno + "' and c.invtype in ('14','15'))"
                //          + "+ (select  isnull(sum(b.qty*pkqty),0) from sto_dtl b where a.item_no=b.itemno and b.branch='" + BL.CLS_Session.brno + "' and b.trtype in ('31','45')))"
                ...
                */
                ...
                foreach (DataRow dtr in dt.Rows)
                {
                    dtr[5] = "%" + Math.Round(((Convert.ToDouble(dtr[4]) * 100) / (Convert.ToDouble(dtr[2]) + Convert.ToDouble(dtr[3]))), 0).ToString();
                    dtr[6] = Convert.ToDouble(dtr[2]) + Convert.ToDouble(dtr[3]) - Convert.ToDouble(dtr[4]);
                }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                */
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sto.rpt.Sto_whbins_RM_rpt.rdlc";
                ...
                parameters[2] = new ReportParameter("title", rad_stopd.Checked? "الاصناف الراكدة":"الاصناف المتحركة");
