...
namespace POS.Sto
{
    public partial class Sto_ItemStmt_All : BaseForm
    {
       ...
       // if(txt_code.Text !="")
                     ...
                     if (!BL.CLS_Session.sysno.Equals("1"))
                     {
                         ...
                         try
                         {
                             ...
                             */
                             ...
                             if (count == 0)
                             {
                                 SqlDataAdapter darown = new SqlDataAdapter("select item_name a_name," + (chk_allsp.Checked ? " round(item_obalance,2) " : " 0 ") + " a_opnbal,item_no a_key  from items where item_no='" + aakey + "'", contyr);
                                 // darown.Fill(dth);
                             ...
                             for (int i = 0; i < dt_fill.Rows.Count; ++i)
                             {
                                 if (i == 0)
                                 {
                                     seso = Convert.ToDouble(txt_opnbal.Text) + Convert.ToDouble(dt_fill.Rows[i]["c6"].ToString().Trim());
                                     dt_fill.Rows[i]["c8"] = Convert.ToDouble(seso);
                                 }
                                 else
                                 {
                                     seso = seso + Convert.ToDouble(dt_fill.Rows[i]["c6"].ToString().Trim());
                                     dt_fill.Rows[i]["c8"] = Convert.ToDouble(seso);
                             ...
                             dataGridView1.DataSource = dt_fill; //daml.SELECT_QUIRY_only_retrn_dt(qstr2);
                     ...
                     if (BL.CLS_Session.sysno.Equals("1"))
                     {
                         ...
                         try
                         {
                             ...
                             */
                             ...
                             if (count == 0)
                             {
                                 SqlDataAdapter darown = new SqlDataAdapter("select item_name a_name,round(item_obalance,2) a_opnbal,item_no a_key  from items where item_no='" + aakey + "'", contyr);
                                 // darown.Fill(dth);
                                 DataTable dtn = new DataTable();
                                 darown.Fill(dtn);
                                 if (dtn.Rows.Count > 0)
                             ...
                             string qstrsal = "select a.slcenter dptno,a.invtype c1, a.ref c2, CONVERT(VARCHAR(10), CAST(a.invmdate as date), 105) c3, (substring(a.invhdate,7,2) + '-' + substring(a.invhdate,5,2)+'-'+substring(a.invhdate,1,4)) c4,a.text c5,case when a.invtype in('04','05') then isnull(sum(b.qty*b.pkqty),0) * -1 else isnull(sum(b.qty*b.pkqty),0) end c6,a.posted a_posted,a.invtype a_t,CONVERT(VARCHAR(10), CAST(a.invmdate as date), 112) dto from salofr_hdr a,salofr_dtl b,items d where b.itemno=d.item_no and a.branch=b.branch and a.ref=b.ref and a.invtype=b.invtype and d.item_no='" + aakey + "' " + condp + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "' and a.branch='" + BL.CLS_Session.brno + "' and a.text like '%" + txt_desc.Text.Replace(" ", "%") + "%' GROUP BY  a.branch,a.slcenter,a.invtype,a.branch,a.ref,a.invmdate,a.invhdate,a.text,a.posted order by a.invmdate";
        ...
        private void dataGridView1_KeyDown_1(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F9)
            {
                ...
                if (dataGridView1.CurrentRow.Cells["a_t"].Value.ToString().Equals("45") || (dataGridView1.CurrentRow.Cells["a_t"].Value.ToString().Equals("46")))
                {
                    Sto_item_Combine f4 = new Sto_item_Combine("", dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells["a_ref"].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
        ...
        private void dataGridView1_calculate()
        {
            ...
            //////    {
            //////        seso = Convert.ToDouble(txt_opnbal.Text) + Convert.ToDouble(dataGridView1.Rows[i].Cells["c6"].Value.ToString().Trim());
            //////        dataGridView1.Rows[i].Cells["c8"].Value = Convert.ToDouble(seso);
            //////    }
            //////    else
            //////    {
            //////        seso = seso + Convert.ToDouble(dataGridView1.Rows[i].Cells["c6"].Value.ToString().Trim());
        ...
        private void Sto_ItemStmt_All_Shown(object sender, EventArgs e)
        {
            Load_Statment(a_key);
        }
    }
