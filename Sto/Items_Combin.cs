...
namespace POS.Sto
{
    public partial class Items_Combin : BaseForm
    {
       ...
       // SqlConnection con1 = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
        ...
        bool is_moved;
        public Items_Combin(string itemno, string itemname,bool moved)
        {
            InitializeComponent();
            item_no=itemno;
            item_name=itemname;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                daml.Insert_Update_Delete_retrn_int("delete from items_cmbin where itemno="+item_no+"",false);
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (!row.IsNewRow && row.Cells[0].Value != null)
                    {
                      ...
                      //  sumcost = (Convert.ToDouble(row.Cells[3].Value) * Convert.ToDouble(row.Cells[4].Value)) / sumbal;
                        ...
                        string StrQuery = " insert into items_cmbin(itemno, itemno_cmbin,barcode,nqty,srno)"
                                       + " values('" + item_no + "', '" + row.Cells[0].Value + "','" + row.Cells[1].Value + "'," + row.Cells[3].Value + "," + srn + ");";
        ...
        private void Groups_Load(object sender, EventArgs e)
        {
          ...
          //  sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString);
            ...
            // sqlConnection.Open();
            sqlDataAdapter = new SqlDataAdapter("select a.itemno_cmbin,a.barcode,b.item_name,a.nqty from items_cmbin a, items b where a.itemno_cmbin=b.item_no and a.itemno='" + item_no + "' order by a.srno", sqlConnection);
