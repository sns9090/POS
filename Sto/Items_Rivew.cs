...
namespace POS.Sto
{
    public partial class Items_Rivew : BaseForm
    {
        ...
        {
            WaitForm wf = new WaitForm("جاري جلب البيانات .. يرجى الانتظار ...");
           ...
           // string conwh=cmb_wh.SelectedIndex !=-1 ? " and b.wh_no='"+ cmb_wh.SelectedValue +"'" :"";
            ...
            string congplc = chk_plc.Checked ? " and b.price < (" + (chk_scst.Checked ? "a.static_cost" : "a.item_curcost") + " * b.pk_qty) " : " ";
          ...
          //  string query = "select a.item_no a1, a.item_name b1, b.qty + b.openbal c1,b.lcost d1, (b.qty + b.openbal) * b.lcost e1 from items a join whbins b on a.item_no=b.item_no and a.item_no + a.item_name like '%" + textBox2.Text + "%' " + conwh + conall +" ";
           ...
           // string query = "select a.item_no a1, a.item_name b1,case when a.item_cbalance > 0 then FLOOR((b.qty + b.openbal)/a.uq2) else CEILING((b.qty + b.openbal)/a.uq2) end trd,(c.unit_name + ' ' + cast(cast(a.uq2 as decimal(10,0)) as varchar)) shd,((b.qty + b.openbal)%a.uq2) frt,(b.qty + b.openbal) totalq,round(a.item_curcost,2) d1,a.item_price price,round(((b.qty + b.openbal) * a.item_curcost),2) e1, round(((b.qty + b.openbal) * a.item_price),2) totalp,b.wh_no whno from items a WITH (NOLOCK) join units c WITH (NOLOCK) on a.item_unit=c.unit_id join whbins b WITH (NOLOCK) on a.item_no=b.item_no " + conit + consup + conwh + conall + congrp + congrsp + condopnbalonly + condminslonly + conaball + congplc + "";
            ...
            query = "select a.item_no a1,b.barcode ab, a.item_name b1,c.unit_name unit,b.pk_qty shd,(" + (chk_scst.Checked ? "a.static_cost" : "a.item_curcost") + " * b.pk_qty) d1, b.price price, 0.00 e1 , 0.00 c1 from items a WITH (NOLOCK) join items_bc b WITH (NOLOCK) on a.item_no=b.item_no join units c WITH (NOLOCK) on b.pack=c.unit_id " + conit + consup + congrp + congrsp + congplc + "";
           ...
           //////// BL.DAML.con.Close();
            ...
            foreach (DataRow dtr in dt.Rows)
            {
                //if (!BL.CLS_Session.multi_bc)
                //{
                sumopnd = Convert.ToDouble(dtr[6]) * Convert.ToDouble(BL.CLS_Session.tax_per) / (100 + Convert.ToDouble(BL.CLS_Session.tax_per));
                dtr[7] =Math.Round((Convert.ToDouble(dtr[6]) - Convert.ToDouble(dtr[5]) - (BL.CLS_Session.isshamltax.Equals("2")? sumopnd : 0)),2);
                dtr[8] =Convert.ToDouble(dtr[6])==0? 0 : Math.Round(((Convert.ToDouble(dtr[7]) / (Convert.ToDouble(dtr[6])- (BL.CLS_Session.isshamltax.Equals("2")? sumopnd : 0))) * 100), 2);
               // dtr[8] = Convert.ToDouble(dtr[7]) == 0 ? 0 : Math.Round((((Convert.ToDouble(dtr[6]) - Convert.ToDouble(dtr[7])) / Convert.ToDouble(dtr[6])) * 100), 2);
           ...
           //////////////// dt.Rows.Add(dr);
            ...
            {
                if (Convert.ToDouble(rw.Cells[7].Value) < 0)
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sto.rpt.Sto_balcost_rpt.rdlc";
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sto.rpt.Items_Rivew_rpt.rdlc";
        ...
        private void dataGridView1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                int testInt = Convert.ToDouble(dataGridView1.CurrentRow.Cells[7].Value)<0 ? 0 : 1;
