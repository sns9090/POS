...
namespace POS.Sto
{
    public partial class Sto_Cnvrt_Comp : BaseForm
    {
       ...
       // int count = 0;
        int ref_max,jrdacc;
        string bprinter, ptype,prpt, a_slctr, a_ref, a_type, strcash, strrcash, strrcrdt, strcrdt, strdsc, strtax, strcashr, stwhno, strsndoq, shehn, gmark, tamin, msfr, mother, waseet, stcstcode, stmkhzon;
        double gmark_amt = 0, tamin_amt = 0, shahen_amt = 0, msfr_amt = 0, mother_amt = 0, waseet_amt = 0, etax_amt=0;
        bool isnew,isupdate,notposted,shameltax,onebyone=false;
        ...
        public Sto_Cnvrt_Comp(string slctr, string atype, string aref)
        {
            InitializeComponent();
    ...
    */
                    ...
                    if ((!row.IsNewRow && row.Cells[0].Value != null) && (!row.IsNewRow && row.Cells[1].Value != null) && (!row.IsNewRow && row.Cells[3].Value != null))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO sto_dtl(branch,trtype,ref,trmdate, trhdate, itemno, qty, lprice, pack, pkqty,folio,whno,towhno,barcode,lcost,expdate) VALUES(@a1,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12,@a14,@a15,@a16,@a17,@exp)", con))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@a16", row.Cells[1].Value);
                            cmd.Parameters.AddWithValue("@a17", Convert.ToDouble(row.Cells[6].Value) / Convert.ToDouble(row.Cells[4].Value));
       ...
       // public delegate string AsyncMethodCaller(string s1, string s2, string s3);
           ...
           // btn_Rfrsh_Click(sender, e);
            ...
            if (result == DialogResult.Yes)
            {
                ...
                try
                {
                    inv_t = cmb_type.SelectedValue.ToString();
                    inv_r = txt_ref.Text;
                    inv_s = cmb_ftwh.SelectedValue.ToString();
 
                    DataTable dttbld = daml.SELECT_QUIRY_only_retrn_dt("select itemno item_no,pkqty * qty item_qty from pu_dtl where invtype='" + cmb_type.SelectedValue + "' and ref=" + txt_ref.Text + " and branch='" + BL.CLS_Session.brno + "' and pucenter='" + cmb_ftwh.SelectedValue + "'");
                    // daml.Exec_Query_only("bld_whbins_cost_all_by_tran @items_tran_tb=" + dttbld + "");
                    ...
                    ////}
                    int exexcuted = daml.Insert_Update_Delete_retrn_int("delete from pu_hdr where posted=0 and branch='" + BL.CLS_Session.brno + "' and pucenter='" + cmb_ftwh.SelectedValue + "' and invtype='" + cmb_type.SelectedValue + "' and ref=" + txt_ref.Text + "", false);
                    daml.Insert_Update_Delete_Only("delete from acc_hdr where a_posted=0 and a_brno='" + BL.CLS_Session.brno + "' and pu_no='" + cmb_ftwh.SelectedValue + "' and a_type='" + cmb_type.SelectedValue + "' and a_ref=" + txt_ref.Text + "", false);
        ...
        private void Fill_Data(string slctr, string atype, string aref)
        {
            try
            {
                ...
                dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select aa.itemno itemno,aa.barcode barcode,bb.item_name item_name,aa.pack pack,aa.pkqty pkqty,aa.qty qty,aa.lprice lcost,0.00 discpc,0.00 tax_amt,(aa.qty*aa.lprice) total,0 tax_id,aa.lcost cost,1 stk_or_ser,iif(rtrim(ltrim(expdate))<>'', convert(varchar(10) ,cast([expdate] as date) ,110),'') expdate  from sto_dtl aa,items bb where aa.itemno=bb.item_no and aa.trtype='" + atype + "' and aa.ref=" + aref + " and aa.branch='" + BL.CLS_Session.brno + "' order by aa.folio");// and aa.a_type='" + atype + "' and aa.a_ref=" + aref + "");
                ...
                accdthdr = daml.SELECT_QUIRY_only_retrn_dt("select * from acc_hdr where a_type='21' and a_ref = " + jrdacc + " and a_brno='" + BL.CLS_Session.brno + "'");
        ...
        {/*
          ...
          */
            ...
            //    {
            //        daml.SqlCon_Open();
            //        int exexcuted = daml.Insert_Update_Delete_retrn_int("update pu_hdr set posted=1 where branch='" + BL.CLS_Session.brno + "' and pucenter='" + cmb_ftwh.SelectedValue + "' and invtype='" + cmb_type.SelectedValue + "' and ref=" + txt_ref.Text + "", false);
            //        int exexcuted0 = daml.Insert_Update_Delete_retrn_int("update acc_hdr set a_posted=1 where a_brno='" + BL.CLS_Session.brno + "' and pu_no='" + cmb_ftwh.SelectedValue + "' and a_type='" + cmb_type.SelectedValue + "' and a_ref=" + txt_ref.Text + "", false);
            //        // exexcuted = exexcuted + daml.Insert_Update_Delete("delete from acc_dtl where a_ref=" + txt_ref.Text + " and a_type='" + comboBox1.SelectedValue + "'", false);
            //        daml.SqlCon_Close();
          ...
          // MessageBox.Show(dvc);
            try
            {
                if (BL.CLS_Session.up_sal_icost)
                    txt_icost.Text =Math.Round(Convert.ToDouble(dataGridView1.CurrentRow.Cells[11].Value.ToString()),4).ToString();
                else
                    txt_icost.Text = "-";
            }
            catch { }
       ...
       // private static readonly Regex IsDouble = new Regex(@"^(0|(-?(((0|[1-9]\d*)\.\d+)|([1-9]\d*))))$");
           ...
           // cmb_salctr_Leave(null, null);
            ...
            try
            {
              ...
              //  DataTable dt = daml.SELECT_QUIRY_only_retrn_dt("select isnull(max(a_ref)," + BL.CLS_Session.dtbranch.Rows[0]["jv_" + cmb_type.SelectedValue + ""] + ") from acc_hdr where a_type='" + cmb_type.SelectedValue + "' and a_brno='" + BL.CLS_Session.brno + "'");
                DataTable dt = daml.SELECT_QUIRY_only_retrn_dt("select isnull(max(a_ref),0) from acc_hdr where a_type='" + cmb_type.SelectedValue + "' and a_brno='" + BL.CLS_Session.brno + "' and pu_no='" + cmb_ftwh.SelectedValue + "' and a_ref=" + ref_max + "");
                ...
                if (isnew)
                {
                    daml.SqlCon_Open();
                     if(dt.Rows[0][0].ToString().Equals("0"))
                         daml.Insert_Update_Delete_retrn_int("INSERT INTO acc_hdr(a_brno,a_type,a_ref,a_mdate, a_hdate,a_text,a_amt,a_entries,usrid,jhsrc,pu_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + ref_max + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "'," + ((shameltax ? (Convert.ToDouble(txt_total.Text)) : Convert.ToDouble(txt_total.Text) + Convert.ToDouble(txt_tax.Text)) + (string.IsNullOrEmpty(waseet) ? 0 : waseet_amt)) + "," + (dataGridView1.RowCount - 1) + ",'" + txt_user.Text + "','Pur1','" + cmb_ftwh.SelectedValue + "')", false);
                    daml.SqlCon_Close();                                          
                }
                else
                {
                    DataTable dtifex = daml.SELECT_QUIRY_only_retrn_dt("select count(*) from acc_hdr where a_type='" + cmb_type.SelectedValue + "' and a_brno='" + BL.CLS_Session.brno + "' and pu_no='" + cmb_ftwh.SelectedValue + "' and a_ref=" + txt_ref.Text + "");
                ...
                switch (cmb_type.SelectedValue.ToString())
                {
                    ...
                    case "17":
                      ...
                      //  daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strrcrdt + "','" + txt_desc.Text + "',0," + (Convert.ToDouble(txt_net.Text) + Convert.ToDouble(txt_des.Text) - Convert.ToDouble(txt_tax.Text)) + ",1,'" + mdate + "','" + hdate + "','D')", false);
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + (BL.CLS_Session.is_dorymost.Equals("2") ? stmkhzon : strrcrdt) + "','" + txt_desc.Text + "',0," + (shameltax ? (Convert.ToDouble(txt_total.Text) - Convert.ToDouble(txt_tax.Text)) : Convert.ToDouble(txt_total.Text)) + ",1,'" + mdate + "','" + hdate + "','C','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                        
                     
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,su_no,pu_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strkey + "','" + txt_desc.Text + "'," + txt_net.Text + "," + 0 + ",2,'" + mdate + "','" + hdate + "','D','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "')", false);
                        
                        if (Convert.ToDouble(txt_des.Text) > 0)
                        {
                            daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strdsc + "','خصومات مشتريات'," + txt_des.Text + "," + 0 + ",3,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                        }
                        if (Convert.ToDouble(txt_tax.Text) > 0)
                        {
                            daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,taxcatId) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strtax + "','ضريبة مشتريات'," + 0 + "," + txt_tax.Text + "," + (Convert.ToDouble(txt_des.Text) > 0 ? "4" : "3") + ",'" + mdate + "','" + hdate + "','C','" + cmb_ftwh.SelectedValue + "',5)", false);
                        }
                        daml.SqlCon_Close();
                        break;

                    case "06":
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text,  a_damt,a_camt, a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + (BL.CLS_Session.is_dorymost.Equals("2") ? stmkhzon : strcash) + "','" + txt_desc.Text + "'," + (shameltax ? (Convert.ToDouble(txt_total.Text) - Convert.ToDouble(txt_tax.Text)) : Convert.ToDouble(txt_total.Text)) + ",0,1,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                       
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text,  a_damt,a_camt, a_folio,a_mdate,a_hdate,jddbcr,su_no,pu_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + txt_key.Text + "','" + txt_desc.Text + "',0," + txt_net.Text + ",2,'" + mdate + "','" + hdate + "','C','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "')", false);
                    ...
                    case "16":
                      //  daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strrcash + "','" + txt_desc.Text + "',0," + (Convert.ToDouble(txt_net.Text) + Convert.ToDouble(txt_des.Text) - Convert.ToDouble(txt_tax.Text)) + ",1,'" + mdate + "','" + hdate + "','D')", false);
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + (BL.CLS_Session.is_dorymost.Equals("2") ? stmkhzon : strrcash) + "','" + txt_desc.Text + "',0," + (shameltax ? (Convert.ToDouble(txt_total.Text) - Convert.ToDouble(txt_tax.Text)) : Convert.ToDouble(txt_total.Text)) + ",1,'" + mdate + "','" + hdate + "','C','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                      
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt, a_camt, a_folio,a_mdate,a_hdate,jddbcr,su_no,pu_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + txt_key.Text + "','" + txt_desc.Text + "'," + txt_net.Text + "," + 0 + ",2,'" + mdate + "','" + hdate + "','D','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "')", false);
                ...
                {
                    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt, a_folio,a_mdate,a_hdate,jddbcr,pu_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + waseet + "','" + txt_desc.Text + "',0," + waseet_amt + ",5,'" + mdate + "','" + hdate + "','C','" + cmb_ftwh.SelectedValue + "')", false);
                    if (gmark_amt > 0)
                    {
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + gmark + "','جمارك مشتريات'," + gmark_amt + ", 0,6,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                    }
                  //  daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strkey + "','" + txt_desc.Text + "',0," + txt_net.Text + ",3,'" + mdate + "','" + hdate + "','C','" + cmb_salctr.SelectedValue + "')", false);

                    if (tamin_amt > 0)
                    {
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + tamin + "','تامين مشتريات'," + tamin_amt + ", 0,7,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                    }
                    if (shahen_amt > 0)
                    {
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + shehn + "','شحن مشتريات'," + shahen_amt + ", 0,8,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                    }
                    if (msfr_amt > 0)
                    {
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + msfr + "','مصاريف سفر مشتريات'," + msfr_amt + ", 0,9,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                    }
                    if (mother_amt > 0)
                    {
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + mother + "','مصاريف متنوعة مشتريات'," + mother_amt + ", 0,10,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "','" + stcstcode + "')", false);
                    }
                    if (etax_amt > 0)
                    {
                        daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt,a_camt,  a_folio,a_mdate,a_hdate,jddbcr,pu_no,taxcatId) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strtax + "','ضريبة مدفوعة مشتريات'," + etax_amt + ", 0,11,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "',5)", false);
                    }
                    daml.SqlCon_Close();
                }
        ...
        {
           //// DataRow[] dr = dtslctr.Select("pu_no = '" + cmb_ftwh.SelectedValue + "'");
        ...
        {
            Pur.Pu_Masarif pu = new Pur.Pu_Masarif();
        ...
        private void btn_prtbar_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtprbar = daml.SELECT_QUIRY_only_retrn_dt("select itemno,(qty*pkqty) qty from pu_dtl where invtype='" + cmb_type.SelectedValue.ToString() + "' and ref=" + txt_ref.Text + " and branch='" + BL.CLS_Session.brno + "' and pucenter='" + cmb_ftwh.SelectedValue.ToString() + "' order by folio");
        ...
        private void Sto_Cnvrt_Comp_Shown(object sender, EventArgs e)
        {
