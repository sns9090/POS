...
namespace POS.Sto
{
    public partial class Sto_item_Combine : BaseForm
    {
       ...
       // DataTable dt2,dtunits;
        ...
        public Sto_item_Combine(string slctr, string atype, string aref)
        {
            InitializeComponent();
        ...
        private void Acc_Tran_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    // MessageBox.Show(tr);
                    dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('45','46')");
                    ...
                    */
                    ...
                    //MessageBox.Show(a_type);
                    //MessageBox.Show(a_ref);
                    ...
                    string whatever = str2.Substring(str2.IndexOf("E123") + 5, 3);
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
           ...
           // string inout = cmb_type.SelectedValue.Equals("45") ? "whno" : "towhno";
            total();
            cmb_salctr_Leave(null, null);
            try
            {
                ...
                //    return;
                //}
                if ( Convert.ToDouble(txt_nqty.Text) <=0 )
                {
                    MessageBox.Show("يجب ان تكون الكمية المنتجة اكبر او تساوي 1","",MessageBoxButtons.OK,MessageBoxIcon.Error);
                    txt_nqty.Focus();
                    return;
                }
                if (string.IsNullOrEmpty(txt_code.Text))
                {
                    MessageBox.Show("يجب ادخال رقم الصنف المركب", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    txt_code.Focus();
                    return;
                }
                ...
                if (isnew && string.IsNullOrEmpty(txt_ref.Text))
                {
                    ...
                    daml.SqlCon_Open();
                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("INSERT INTO sto_hdr(branch,trtype,ref,trmdate, trhdate,text,remarks,entries,amnttl,costttl,usrid,whno,towhno,supno,brsupp)"
                                   + " VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + ref_max + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "','" + txt_remark.Text + "'," + (dataGridView1.RowCount + 1) + "," + txt_total.Text + "," + txt_cost.Text + ",'" + txt_user.Text + "','" + cmb_frwh.SelectedValue +"','"+cmb_towh.SelectedValue + "',"+txt_nqty.Text+","+txt_code.Text+")", false);
                    daml.SqlCon_Close();
                    //daml.SqlCon_Open();
                ...
                {
                    daml.SqlCon_Open();
                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("update sto_hdr set trmdate='" + mdate + "', trhdate='" + hdate + "',text='" + txt_desc.Text + "',remarks='" + txt_remark.Text + "',supno='" + txt_nqty.Text + "',brsupp='"+txt_code.Text+"',entries=" + (dataGridView1.RowCount + 1) + ",lastupdt=getdate(),amnttl=" + txt_total.Text + ",costttl=" + txt_cost.Text + ",usrid='" + txt_user.Text + "',whno='" + cmb_frwh.SelectedValue + "',towhno='" + cmb_towh.SelectedValue + "' where branch='" + BL.CLS_Session.brno + "' and trtype='" + cmb_type.SelectedValue + "' and ref=" + txt_ref.Text + "", false);
    ...
    */
                    if (!row.IsNewRow && row.Cells[0].Value != null)
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO sto_dtl(branch,trtype,ref,trmdate, trhdate, itemno, qty, lcost, pack, pkqty,folio,whno,barcode,towhno,lprice) VALUES(@a1,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12,@a14,@a15,@a16,@a17)", con))
                        {
                         ...
                         //   cmd.Parameters.AddWithValue("@a2", cmb_salctr.SelectedValue);
                          ...
                          //  cmd.Parameters.AddWithValue("@a13", Convert.ToDouble(row.Cells[6].Value));
                            cmd.Parameters.AddWithValue("@a14", (cmb_type.SelectedValue.Equals("45") ? cmb_frwh.SelectedValue : ""));
                            cmd.Parameters.AddWithValue("@a15", row.Cells[8].Value);
                            cmd.Parameters.AddWithValue("@a16", (cmb_type.SelectedValue.Equals("46") ? cmb_towh.SelectedValue : ""));
                            cmd.Parameters.AddWithValue("@a17", Math.Round(Convert.ToDouble(row.Cells[5].Value),2));
                ...
                using (SqlCommand cmd = new SqlCommand("INSERT INTO sto_dtl(branch,trtype,ref,trmdate, trhdate, itemno, qty, lcost, pack, pkqty,folio,towhno,barcode,whno,lprice) VALUES(@a1,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12,@a14,@a15,@a16,@a17)", con))
                {
                    cmd.Parameters.AddWithValue("@a1", BL.CLS_Session.brno);
                    //   cmd.Parameters.AddWithValue("@a2", cmb_salctr.SelectedValue);
                    cmd.Parameters.AddWithValue("@a3", cmb_type.SelectedValue);
                    ...
                    // cmd.Parameters.AddWithValue("@a5", (string.IsNullOrEmpty(row.Cells[3].Value.ToString())? 0 : Convert.ToDouble(row.Cells[3].Value)));
                    cmd.Parameters.AddWithValue("@a6", hdate);
                    cmd.Parameters.AddWithValue("@a7", txt_code.Text);
                    cmd.Parameters.AddWithValue("@a8", txt_nqty.Text);
                    cmd.Parameters.AddWithValue("@a9", (Convert.ToDouble(txt_total.Text) / Convert.ToDouble(txt_nqty.Text)));
                  //  cmd.Parameters.AddWithValue("@a9", Convert.ToInt32(txt_total.Text));
                    cmd.Parameters.AddWithValue("@a10", 1);
                    cmd.Parameters.AddWithValue("@a11", 1);
                    cmd.Parameters.AddWithValue("@a12", dataGridView1.Rows.Count + 1);
                    //  cmd.Parameters.AddWithValue("@a13", Convert.ToDouble(row.Cells[6].Value));
                    cmd.Parameters.AddWithValue("@a14",  (cmb_type.SelectedValue.Equals("45") ? cmb_towh.SelectedValue : ""));
                    cmd.Parameters.AddWithValue("@a15", txt_code.Text);
                     cmd.Parameters.AddWithValue("@a16", (cmb_type.SelectedValue.Equals("46") ? cmb_frwh.SelectedValue : ""));
                     cmd.Parameters.AddWithValue("@a17", Math.Round((Convert.ToDouble(txt_total.Text) / Convert.ToDouble(txt_nqty.Text)),2));
                    //  cmd.Parameters.AddWithValue("@c7", Convert.ToInt32(lblcashir.Text));
                    //if (row.Cells[7].Value != null)
                    // { cmd.Parameters.AddWithValue("@c8", row.Cells[7].Value); }
                    // cmd.Parameters.AddWithValue("@c9", comp_id);
        ...
        private void Fill_Data(string slctr,string atype, string aref)
        {
            try
            {
              ...
              //  hdate = txt_hdate.Text.Replace("-", "").Substring(4, 4) + txt_hdate.Text.Replace("-", "").Substring(2, 2) + txt_hdate.Text.Replace("-", "").Substring(0, 2);
                 ...
                 dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select aa.itemno itemno,bb.item_name item_name,aa.pack pack,aa.pkqty pkqty,aa.qty qty,aa.lcost lcost,(aa.qty*aa.pkqty*aa.lcost) total,aa.lcost cost,aa.barcode barcode,cc.nqty oqty  from sto_dtl aa,items bb,items_cmbin cc where cc.itemno_cmbin=bb.item_no and aa.itemno=cc.itemno_cmbin and aa.trtype='" + atype + "' and aa.ref=" + aref + " and aa.branch='" + BL.CLS_Session.brno + "' order by aa.folio");// and aa.a_type='" + atype + "' and aa.a_ref=" + aref + "");
              ...
              //  txt_net.Text = dthdr.Rows[0]["nettotal"].ToString();
               ...
               // txt_key.Text = dthdr.Rows[0][19].ToString();
                txt_code.Text = dthdr.Rows[0]["brsupp"].ToString();
                txt_nqty.Text = dthdr.Rows[0]["supno"].ToString();
        ...
        private void load_key()
        {
            ...
            try
            {
                DataTable dta = daml.SELECT_QUIRY_only_retrn_dt("select item_name,round(item_obalance,2) a_opnbal,item_no  from items where item_cost=2 and item_no='" + txt_code.Text + "'");
                if (dta.Rows.Count > 0)
                {
        ...
        private void fill_items()
        {
            try
            {
                DataTable dt = daml.SELECT_QUIRY_only_retrn_dt("select aa.itemno_cmbin itemno,bb.item_name item_name,bb.item_unit pack,1 pkqty,aa.nqty qty,bb.item_curcost lcost,(aa.nqty*bb.item_curcost) total,bb.item_curcost cost,aa.barcode barcode,aa.nqty oqty from items_cmbin aa, items bb where aa.itemno_cmbin=bb.item_no and itemno='" + txt_code.Text + "'");
                dataGridView1.DataSource = dt;
                ...
                foreach(DataGridViewRow row in dataGridView1.Rows)
                {
                  ...
                  //  r.Cells[2] = dcombo;
                    DataView dv1 = dtunits.DefaultView;
                  //  dv1.RowFilter = "unit_id in('" + dt222.Rows[0][5] + "','" + dt222.Rows[0][12] + "','" + dt222.Rows[0][15] + "','" + dt222.Rows[0][18] + "')";
                    dv1.RowFilter = "unit_id in('" + row.Cells[2].Value + "')";
                    DataTable dtNew = dv1.ToTable();
                    dcombo.DataSource = dtNew;
                    dcombo.DisplayMember = "unit_name";
                    dcombo.ValueMember = "unit_id";
                  ...
                  //  dataGridView1.CurrentRow.Cells[2] = dcombo;
                    ...
                    //  dataGridView1.CurrentRow.Cells[2].Value = dt222.Rows[0][5];
                    row.Cells[2] = dcombo;
                    row.Cells[2].Value = dtNew.Rows[0][0];
        ...
        private void txt_code_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.F8)
                {
                    // var selectedCell = dataGridView1.SelectedCells[0];
                    // do something with selectedCell...
                    Search_All f4 = new Search_All("2", "Sto2");
                    f4.ShowDialog();
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    DataTable dta = daml.SELECT_QUIRY_only_retrn_dt("select item_name,0 a_opnbal,item_no  from items where item_cost=2 and item_no='" + txt_code.Text + "'");
                    if (dta.Rows.Count > 0)
                    {
                        txt_name.Text = dta.Rows[0][0].ToString();
                        txt_code.Text = dta.Rows[0][2].ToString();
        ...
        private void txt_nqty_TextChanged(object sender, EventArgs e)
        {
            dtv.ValidateText_numric(txt_nqty);

            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                row.Cells[4].Value = Convert.ToDouble(row.Cells[9].Value) * Convert.ToDouble(txt_nqty.Text);
