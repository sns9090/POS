...
namespace POS.Sto
{
    public partial class Sto_Report_Br_Dtl : BaseForm
    {
      ...
      //  DataTable dtt;
       ...
       // string typeno = "";
            ...
            try
            {
               ...
               // MessageBox.Show(datval.convert_to_yyyyDDdd(BL.CLS_Session.end_dt));
                if (!chk_notequal.Checked)
                {
                    ...
                    */
                    string usrname = cmb_user.SelectedIndex != -1 ? " and bb.usrid = '" + cmb_user.SelectedValue.ToString() + "' " : " ";
                    string cond = checkBox1.Checked ? "bb.invhdate" : "bb.trmdate";
                 ...
                 //   string conslctr = cmb_salctr.SelectedIndex != -1 ? " and bb.slcenter = '" + cmb_salctr.SelectedValue.ToString() + "' " : " ";
                    //  string qstr = "select a_type a_type,a_ref a_ref,CONVERT(VARCHAR(10), CAST(a_mdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(a_hdate as date), 105) a_hdate,a_text a_text,a_amt a_amt,a_posted,a_type a_t from acc_hdr where jhsrc like 'Acc%' and a_brno='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + " and " + cond + " between '" + txt_fdate.Text.Replace("-", "") + "' and '" + txt_tdate.Text.Replace("-", "") + "'";
                    string qstr = "select bb.trtype a_type,bb.ref a_ref,CONVERT(VARCHAR(10), CAST(bb.trmdate as date), 105) a_mdate,(substring(bb.trhdate,7,2) + '-' + substring(bb.trhdate,5,2)+'-'+substring(bb.trhdate,1,4)) a_hdate,bb.text a_text,bb.amnttl a_amt,bb.posted,bb.trtype a_t,'' src,'' sl from  sto_hdr bb  where bb.branch='" + BL.CLS_Session.brno + "' and isbrtrx=1 " + condp + usrname + condft + " and " + cond + condfdt + "";


                    dt = daml.SELECT_QUIRY_only_retrn_dt(qstr + " and bb.trtype like '%" + cmb_type.SelectedValue + "%' order by bb.trmdate");
                    DataTable dtdtl2 = new DataTable();
                    DataTable dtdtlall = new DataTable();
                    sumc1=0;  sumd1=0 ;
                    foreach (DataRow dtr in dt.Rows)
                    {
                        string qstr2 = "select aa.trtype a_type,cast(aa.ref  as varchar) a_ref,CONVERT(VARCHAR(10), CAST(aa.trmdate as date), 105) a_mdate,(substring(aa.trhdate,7,2) + '-' + substring(aa.trhdate,5,2)+'-'+substring(aa.trhdate,1,4)) a_hdate,aa.itemno a_itemno,cc.item_name a_name,dd.unit_name a_unit,aa.pkqty a_shd,round(aa.qty,2) a_qty,round(aa.lcost,2) a_price,round(aa.discpc,2) a_desc,round(((aa.lcost * aa.qty) - ((aa.lcost * aa.qty)*discpc/100)),2) a_total,bb.posted a_p,aa.trtype a_t,'' src,'' sl from sto_dtl aa join sto_hdr bb on aa.branch=bb.branch and aa.trtype=bb.trtype and aa.ref=bb.ref  join items cc on  aa.itemno=cc.item_no join units dd on dd.unit_id= aa.pack where aa.trtype='" + dtr[0] + "' and aa.ref=" + dtr[1] + "";
                        ...
                        if (!string.IsNullOrEmpty(txt_equl.Text))
                        {
                            ...
                            qstr2 = qstr2 + " and bb.amnttl " + comboBox2.Text.Substring(0, 1) + txt_equl.Text + " or bb.amnttl " + comboBox2.Text.Substring(0, 1) + txt_equl.Text;
                        }
                     ...
                     //   dtdtl2 = daml.SELECT_QUIRY_only_retrn_dt("select aa.a_type a_type,cast(aa.a_ref  as varchar) a_ref,CONVERT(VARCHAR(10), CAST(aa.a_mdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(aa.a_hdate as date), 105) a_hdate,aa.a_key,cc.a_name a_name,aa.a_text a_text,round(aa.a_camt,2) a_camt,round(aa.a_damt,2) a_damt,bb.a_posted,aa.a_type a_t,bb.jhsrc src,aa.sl_no sl,aa.pu_no pu from acc_dtl aa join acc_hdr bb on aa.a_brno=bb.a_brno and aa.a_type=bb.a_type and aa.a_ref=bb.a_ref and aa.sl_no=bb.sl_no and aa.pu_no=bb.pu_no join accounts cc on aa.a_brno=cc.a_brno and aa.a_key=cc.a_key where aa.a_type='" + dtr[0] + "' and aa.a_ref='" + dtr[1] + "' and aa.sl_no='" + dtr[9] + "' and aa.pu_no='" + dtr[10] + "' and aa.a_brno='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + " and " + cond + condfdt + "");
                        dtdtl2 = daml.SELECT_QUIRY_only_retrn_dt(qstr2 + " order by aa.trmdate");
        ...
        private void SalesReport_Load(object sender, EventArgs e)
        {
            ...
            cmb_type.DataSource = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no  in('31','32','33','45','46','36')");
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F9)
            {
                ...
                {
                    Sto_ImpExp_Br f4 = new Sto_ImpExp_Br("", dataGridView1.CurrentRow.Cells["a_t"].Value.ToString(), dataGridView1.CurrentRow.Cells[1].Value.ToString());
                    f4.MdiParent = ParentForm;
                    if (BL.CLS_Session.formkey.Contains(f4.Tag.ToString()))
                        f4.Show();
                }
