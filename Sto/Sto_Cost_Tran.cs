...
namespace POS.Sto
{
    public partial class Sto_Cost_Tran : BaseForm
    {
        ...
        public Sto_Cost_Tran(string slctr, string atype, string aref)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                if (dt222.Rows.Count > 0)
                {
                    ...
                    if (dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[0])
                    {
                        ...
                        dataGridView1.CurrentRow.Cells[2].Value = dt222.Rows[0][5];// dcombo;
                    ...
                    //{
                        ...
                        dataGridView1.CurrentRow.Cells[3].Value = 1;// dataGridView1.CurrentRow.Cells[2].Value.ToString().Equals(dt222.Rows[0][18].ToString()) ? dt222.Rows[0][19].ToString() : dataGridView1.CurrentRow.Cells[2].Value.ToString().Equals(dt222.Rows[0][15].ToString()) ? dt222.Rows[0][16].ToString() : dataGridView1.CurrentRow.Cells[2].Value.ToString().Equals(dt222.Rows[0][12].ToString()) ? dt222.Rows[0][13].ToString() : "1.00";

                       // if (Convert.ToDecimal(dataGridView1.CurrentRow.Cells[4].Value) == 0)
                        dataGridView1.CurrentRow.Cells[4].Value = dt222.Rows[0]["item_cbalance"];
        ...
        private void Acc_Tran_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    ////else
                    ////{
                    dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('36')");
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvr.Cells[9].Value = (2 * Convert.ToDouble(dgvr.Cells[5].Value)) - Convert.ToDouble(dgvr.Cells[7].Value);
            }
            ...
            try
            {
                ...
                if (isnew && string.IsNullOrEmpty(txt_ref.Text))
                {
                    ...
                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("INSERT INTO sto_hdr(branch,trtype,ref,trmdate, trhdate,text,remarks,entries,amnttl,costttl,usrid,supno,whno,towhno,stkjvno) "
                        + " VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + ref_max + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "','" + txt_remark.Text + "'," + (dataGridView1.RowCount - 1) + "," + txt_total.Text + "," + txt_cost.Text + ",'" + txt_user.Text + "','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "','" + cmb_ftwh.SelectedValue + "',"+(BL.CLS_Session.is_dorymost.Equals("2")? jrdacc : 0 )+")", false);
                    daml.SqlCon_Close();
                    //daml.SqlCon_Open();
                    ...
                    if ((!row.IsNewRow && row.Cells[0].Value != null) && (!row.IsNewRow && row.Cells[1].Value != null) && (!row.IsNewRow && row.Cells[3].Value != null))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO sto_dtl(branch,trtype,ref,trmdate, trhdate, itemno, qty, lprice, pack, pkqty,folio,whno,towhno,barcode,lcost,expdate) VALUES(@a1,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12,@a14,@a15,@a16,@a17,@exp)", con))
                        {
                           ...
                           // cmd.Parameters.AddWithValue("@a17", Convert.ToDouble(row.Cells[5].Value) / Convert.ToDouble(row.Cells[3].Value));
                            cmd.Parameters.AddWithValue("@a17", row.Cells[9].Value);
               ...
               // dataGridView1.Rows.Clear();
                ...
                foreach (DataRow dtr in data.Rows)
                {
                  //  MessageBox.Show("The name of the active sheet is : " + dtr[0].ToString());
                    dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select '" + dtr[0] + "' itemno,bb.item_name item_name," + dtr[1] + " pack,1 pkqty," + dtr[2] + " qty," + dtr[3] + " lcost,(" + Convert.ToDouble(dtr[2]) * Convert.ToDouble(dtr[3]) + ") total," + dtr[0] + " cost,bb.item_barcode barcode,'' expdate  from items bb where bb.item_no='"+dtr[0]+"'");// and aa.a_type='" + atype + "' and aa.a_ref=" + aref + "");
                    foreach (DataRow dr in dtdtl.Rows)
                    {
