       ...
       // string typeno = "";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                */
                ...
                string condfwh = cmb_ftwh.SelectedIndex==-1? " " : " and whno='"+cmb_ftwh.SelectedValue+"' ";
               // string condtobr = cmb_to_br.SelectedIndex == -1 ? " " : " and tobrno='" + cmb_to_br.SelectedValue + "' ";
                string condtobrwh = cmb_ftwh_br.SelectedIndex == -1 ? " " : " and towhno='" + cmb_ftwh_br.SelectedValue + "' ";
               ...
               // string qstr = "select trtype a_type,ref a_ref,CONVERT(VARCHAR(10), CAST(trmdate as date), 105) a_mdate,CONVERT(VARCHAR(10), CAST(trhdate as date), 105) a_hdate,text a_text,amnttl a_amt,posted a_posted,trtype a_t from sto_hdr where  branch='" + BL.CLS_Session.brno + "' " + condp + usrname + condft + " and " + cond + " between '" + datval.convert_to_yyyyDDdd(txt_fdate.Text) + "' and '" + datval.convert_to_yyyyDDdd(txt_tdate.Text) + "'";
                string qstr = "select trtype a_type,cast (ref as varchar) a_ref,CONVERT(VARCHAR(10), CAST(trmdate as date), 105) a_mdate,(substring(trhdate,7,2) + '-' + substring(trhdate,5,2)+'-'+substring(trhdate,1,4)) a_hdate,text a_text,amnttl a_amt,posted a_posted,trtype a_t from sto_hdr where  branch='" + BL.CLS_Session.brno + "' and isbrtrx=0 " + condp + usrname + condft + " and " + cond + condfdt + condfwh  + condtobrwh + "";
                ...
                foreach (DataGridViewRow rw in dataGridView1.Rows)
                {
                   ...
                   // rw.Cells[0].Value = rw.Cells[0].Value.ToString().Equals("01") ? "ق عام" : rw.Cells[0].Value.ToString().Equals("02")? "س قبض" : "س صرف";
                    rw.Cells[0].Value = string.IsNullOrEmpty(rw.Cells[0].Value.ToString()) ? "" : datval.validate_trtypes(rw.Cells[0].Value.ToString());// datval.validate_trtypes(rw.Cells[0].Value.ToString());
                }
        ...
        private void SalesReport_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            cmb_type.DataSource = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('31','32','33','36')");
            ...
            dttowhbr = daml.SELECT_QUIRY_only_retrn_dt("select wh_no,wh_name,mkhzon_acc,cstcode from whouses where wh_brno + wh_no in(" + sl + ") and wh_brno='" + BL.CLS_Session.brno + "'");
        ...
        private void cmb_ftwh_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Delete)
            {
                cmb_ftwh.SelectedIndex = -1;
