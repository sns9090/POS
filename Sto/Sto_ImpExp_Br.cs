...
namespace POS.Sto
{
    public partial class Sto_ImpExp_Br : BaseForm
    {
        List<BL.CurrencyInfo> currencies = new List<BL.CurrencyInfo>();
        string inv_t, inv_r;
        int ref_max, ref_max2, jrdacc;
        double seso = 0;
        // DataTable dtg;
        // DataTable dt2,dtunits;
        DataTable dthdr, dtdtl, dt222, dtunits, dtsal, dtvat, dttrtyps, dtslctr, dttobr, dttowhbr, dtref, accdthdr, accdtdtl;
        string a_slctr, a_ref, a_type, strcash, strrcash, strrcrdt, strcrdt, strdsc, strtax, strcashr, stwhno, sttowhno, stmkhzon, stcstcode;
        ...
        public Sto_ImpExp_Br(string slctr, string atype, string aref)
        {
            InitializeComponent();
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Modifiers == Keys.Control && e.KeyCode == Keys.E && (btn_Add.Enabled != true && btn_Edit.Enabled != true) && cmb_type.SelectedValue.Equals("31"))
            {
                try
                {
                    Date_Enter dte = new Date_Enter(dataGridView1.CurrentRow.Cells["Column10"].Value.ToString());
                    dte.ShowDialog();

                    dataGridView1.CurrentRow.Cells["Column10"].Value = dte.txt_date.Text;
        ...
        private void dataGridView1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.CurrentCell == dataGridView1.CurrentRow.Cells[5] && (isnew || isupdate) && Convert.ToDouble(dataGridView1.CurrentRow.Cells[5].Value) == 0)
                {
                    ...
                    DataTable dtlp = daml.SELECT_QUIRY_only_retrn_dt("select isnull(case when lcost=0 then openlcost else lcost end,0) from whbins where br_no='" + BL.CLS_Session.brno + "' and item_no='" + dataGridView1.CurrentRow.Cells[0].Value.ToString().Trim() + "' and wh_no='" + cmb_ftwh.SelectedValue + "'");
        ...
        {
            if (dataGridView1.CurrentRow.Cells["Column10"].Value == null)
                dataGridView1.CurrentRow.Cells["Column10"].Value = "";
            try
            {
        ...
        private void Acc_Tran_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    //if (a_type.Equals("35"))
                    //{
                    //    dttrtyps = daml.SELECT_QUIRY_only_retrn_dt("select * from trtypes where tr_no in(" + tr + ") and tr_no in('35')");
                    ...
                    dttobr = daml.SELECT_QUIRY_only_retrn_dt("select * from branchs where br_no <>'"+BL.CLS_Session.brno+"'");



                    cmb_to_br.DataSource = dttobr;
                    cmb_to_br.DisplayMember = "br_name";
                    cmb_to_br.ValueMember = "br_no";
                    cmb_to_br.SelectedIndex = -1;
                    ...
                    */
                    ...
                    // MessageBox.Show(sl);
                    dtslctr = daml.SELECT_QUIRY_only_retrn_dt("select wh_no,wh_name,mkhzon_acc,cstcode from whouses where wh_brno + wh_no in(" + sl + ") and wh_brno='" + BL.CLS_Session.brno + "'");
                    ...
                    cmb_ftwh.Enabled = false;

                    cmb_to_br.Enabled = false;
                    cmb_ftwh_br.Enabled = false;
                    ...
                    string whatever = str2.Substring(str2.IndexOf("E121") + 5, 3);
        ...
        private void Add_btn_Click_1(object sender, EventArgs e)
        {
            ...
            cmb_type.ValueMember = "tr_no";
            cmb_type.SelectedIndex = -1;
           // cmb_type.Items.Remove("سند جرد مخزني");
           ...
           // txt_key.Text = BL.CLS_Session.brcash;
            ...
            cmb_ftwh.Enabled = true;
            cmb_to_br.Enabled = true;
            cmb_ftwh_br.Enabled = true;

            cmb_ftwh.SelectedIndex = 0;

           // cmb_type.Items.RemoveAt(2);
        ...
        private void btn_Undo_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                if (isupdate)
                {
                    ...
                    Fill_Data(cmb_ftwh.SelectedValue.ToString(), cmb_type.SelectedValue.ToString(), txt_ref.Text);
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if ((txt_key.Visible ==true && txt_key.Text == "") || (txt_key.Visible ==true && txt_name.Text == ""))
                {
                    MessageBox.Show("يرجى ادخال حساب فارق المخزون","",MessageBoxButtons.OK,MessageBoxIcon.Error);
                    txt_key.Focus();
                    return;
                }

                if (string.IsNullOrEmpty( txt_accno.Text))
                {
                    MessageBox.Show("يرجى ادخال حساب الفرع المحول له", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    txt_accno.Focus();
                    return;
                }

                if (string.IsNullOrEmpty(BL.CLS_Session.brstkout) && cmb_type.SelectedValue.Equals("32"))
                {
                    MessageBox.Show("يرجى تعريف حساب البضاعة الصادرة في ملف الفرع", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    txt_accno.Focus();
                    return;
                }

                if (string.IsNullOrEmpty(BL.CLS_Session.brstkin) && cmb_type.SelectedValue.Equals("31"))
                {
                    MessageBox.Show("يرجى تعريف حساب البضاعة الواردة في ملف الفرع", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    txt_accno.Focus();
                    return;
                }

                if (cmb_to_br.SelectedIndex==-1)
                {
                    MessageBox.Show("يرجى ادخال الفرع المحول له", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cmb_to_br.Focus();
                    return;
                }

                if (cmb_ftwh_br.SelectedIndex == -1 || string.IsNullOrEmpty(cmb_ftwh_br.SelectedValue.ToString()))
                {
                    MessageBox.Show("يرجى ادخال المخزن المحول له", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cmb_ftwh_br.Focus();
                    return;
                }
               ...
               // jrdacc = Convert.ToInt32(dt2.Rows[0][0]) + 1;
                ...
                if (isnew && string.IsNullOrEmpty(txt_ref.Text))
                {
                    ...
                    DataTable dt2 = daml.SELECT_QUIRY_only_retrn_dt("select isnull(max(a_ref)," + BL.CLS_Session.dtbranch.Rows[0]["jv_01"] + ") from acc_hdr where a_type='01' and a_brno='" + BL.CLS_Session.brno + "'");
                
                    ref_max = Convert.ToInt32(dt.Rows[0][0]) + 1;
                    jrdacc = Convert.ToInt32(dt2.Rows[0][0]) + 1;

                    daml.SqlCon_Open();
                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("INSERT INTO sto_hdr(branch,trtype,ref,trmdate, trhdate,text,remarks,entries,amnttl,costttl,usrid,supno,whno,towhno,stkjvno,tobrno,isbrtrx) "
                        + " VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + ref_max + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "','" + txt_remark.Text + "'," + (dataGridView1.RowCount - 1) + "," + txt_total.Text + "," + txt_cost.Text + ",'" + txt_user.Text + "','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "','" + cmb_ftwh_br.SelectedValue + "',"+(BL.CLS_Session.is_dorymost.Equals("2")? jrdacc : jrdacc )+",'"+cmb_to_br.SelectedValue+"',1)", false);
                   // daml.SqlCon_Close();
                    //daml.SqlCon_Open();

                    int exexcuted = daml.Insert_Update_Delete_retrn_int("INSERT INTO acc_hdr(a_brno,a_type,a_ref,a_mdate, a_hdate,a_text,a_amt,a_entries,jhsrc,usrid,mainkey) VALUES('"+BL.CLS_Session.brno+"','01'," + jrdacc + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "'," + txt_total.Text + "," + (dataGridView1.RowCount - 1) + " , 'StoB' ,'" + BL.CLS_Session.UserID + "', '" + txt_accno.Text + "')", false);
                     daml.SqlCon_Close();
                    if (exexcuteds > 0)
                    {
                ...
                {
                    daml.SqlCon_Open();
                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("update sto_hdr set trmdate='" + mdate + "', trhdate='" + hdate + "',text='" + txt_desc.Text + "',remarks='" + txt_remark.Text + "',supno='" + txt_custno.Text + "',entries=" + (dataGridView1.RowCount - 1) + ",lastupdt=getdate(),amnttl=" + txt_total.Text + ",costttl=" + txt_cost.Text + ",usrid='" + txt_user.Text + "',whno='" + cmb_ftwh.SelectedValue + "',towhno='" + cmb_ftwh_br.SelectedValue + "' where branch='" + BL.CLS_Session.brno + "' and trtype='" + cmb_type.SelectedValue + "' and ref=" + txt_ref.Text + "", false);
                   ...
                   // daml.SqlCon_Close();
                    ...
                    //  daml.SqlCon_Open();
                    int exexcuted = daml.Insert_Update_Delete_retrn_int("update acc_hdr set a_mdate='" + mdate + "', a_hdate='" + hdate + "',a_text='" + txt_desc.Text + "',a_amt=" + txt_total.Text + ",a_entries=" + (dataGridView1.RowCount - 1) + ",usrid='" + BL.CLS_Session.UserID + "',mainkey='"+txt_accno.Text+"' where a_ref=" + txt_jrdacc.Text + " and a_type='01' and a_brno='" + BL.CLS_Session.brno + "'", false);
                    exexcuted = exexcuted + daml.Insert_Update_Delete_retrn_int("delete from acc_dtl where a_ref=" + txt_jrdacc.Text + " and a_type='01' and a_brno='" + BL.CLS_Session.brno + "'", false);
                     daml.SqlCon_Close();
                    if (exexcuteds > 0)
                    {
                        //  txt_ref.Text = ref_max.ToString();
                ...
                daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,sl_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','01'," + (isnew ? jrdacc : Convert.ToInt32(txt_jrdacc.Text)) + ",'" + BL.CLS_Session.brstkout + "','" + txt_desc.Text + "'," + txt_total.Text + ",0,1,'" + mdate + "','" + hdate + "','C','','" + stcstcode + "')", false);

                daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','01'," + (isnew ? jrdacc : Convert.ToInt32(txt_jrdacc.Text)) + ",'" + txt_accno.Text + "','" + txt_desc.Text + "',0," + txt_total.Text + ",2,'" + mdate + "','" + hdate + "','D','','" + stcstcode + "')", false);
    ...
    */
                 //   if (!row.IsNewRow && row.Cells[0].Value != null)
                    if ((!row.IsNewRow && row.Cells[0].Value != null) && (!row.IsNewRow && row.Cells[1].Value != null) && (!row.IsNewRow && row.Cells[3].Value != null))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO sto_dtl(branch,trtype,ref,trmdate, trhdate, itemno, qty, lprice, pack, pkqty,folio,whno,towhno,barcode,lcost,expdate) VALUES(@a1,@a3,@a4,@a5,@a6,@a7,@a8,@a9,@a10,@a11,@a12,@a14,@a15,@a16,@a17,@exp)", con))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@a15", cmb_ftwh_br.SelectedValue);
                            ...
                            cmd.Parameters.AddWithValue("@exp", row.Cells["Column10"].Value.ToString().Replace("-", "").Length == 8 ? dtv.convert_to_yyyyDDdd(row.Cells["Column10"].Value.ToString()) : "");
        ...
        public void threadt(string s1, string s2,string s3)
        {
               try
            {
                if (cmb_type.SelectedValue.ToString().Equals("32"))
                {
                    daml.SqlCon_Open();
                ...
                else
                {
                    if (string.IsNullOrEmpty(txt_accno.Text))
                    {
                        MessageBox.Show("يرجى ادخال حساب الفرع المحول منه", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_accno.Focus();
                        return;
                    }
                    ...
                    DataTable dt22 = daml.SELECT_QUIRY_only_retrn_dt("select isnull(max(a_ref)," + BL.CLS_Session.dtbranch.Rows[0]["jv_01"] + ") from acc_hdr where a_type='01' and a_brno='" + BL.CLS_Session.brno + "'");

                    jrdacc = Convert.ToInt32(dt22.Rows[0][0]) + 1;
                    daml.SqlCon_Open();

                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("update sto_hdr set stkjvno= " + (BL.CLS_Session.is_dorymost.Equals("2") ? jrdacc : jrdacc) + " where branch='" + BL.CLS_Session.brno + "' and trtype='" + s1 + "' and ref=" + s2 + "", false);

                   // DataTable dt22=daml.

                  //  int exexcuteds2 = daml.Insert_Update_Delete_retrn_int("INSERT INTO sto_hdr(branch,trtype,ref,trmdate, trhdate,text,remarks,entries,amnttl,costttl,usrid,supno,whno,towhno,stkjvno,tobrno,isbrtrx
                  

                    daml.Insert_Update_Delete_retrn_int("INSERT INTO acc_hdr(a_brno,a_type,a_ref,a_mdate, a_hdate,a_text,a_amt,a_entries,jhsrc,usrid,mainkey) VALUES('" + BL.CLS_Session.brno + "','01'," + jrdacc + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "'," + txt_total.Text + "," + (dataGridView1.RowCount - 1) + " , 'StoB' ,'" + BL.CLS_Session.UserID + "', '" + txt_accno.Text + "')", false);

                    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,sl_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','01'," + jrdacc + ",'" + txt_accno.Text + "','" + txt_desc.Text + "'," + txt_total.Text + ",0,1,'" + mdate + "','" + hdate + "','C','','" + stcstcode + "')", false);

                    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cu_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','01'," + jrdacc + ",'" + BL.CLS_Session.brstkin + "','" + txt_desc.Text + "',0," + txt_total.Text + ",2,'" + mdate + "','" + hdate + "','D','','" + stcstcode + "')", false);


                   // daml.SqlCon_Open();
                    int exexcuted9 = daml.Insert_Update_Delete_retrn_int("update sto_hdr set posted=1 where branch='" + BL.CLS_Session.brno + "' and trtype='" + cmb_type.SelectedValue + "' and ref=" + txt_ref.Text + "", false);
                    // exexcuted = exexcuted + daml.Insert_Update_Delete("delete from acc_dtl where a_ref=" + txt_ref.Text + " and a_type='" + comboBox1.SelectedValue + "'", false);
                    daml.SqlCon_Close();
        ...
        private void save_estlam_br()
        {
           // cmb_salctr_Leave(null, null);
            try
            {
                ...
                if (isnew)
                {
                    DataTable dt = daml.SELECT_QUIRY_only_retrn_dt("select isnull(max(ref)," + BL.CLS_Session.dtbranch.Rows[0]["jv_31"] + ") from sto_hdr where trtype='31' and branch='" + cmb_to_br.SelectedValue + "'");
                    DataTable dt2 = daml.SELECT_QUIRY_only_retrn_dt("select isnull(max(a_ref)," + BL.CLS_Session.dtbranch.Rows[0]["jv_21"] + ") from acc_hdr where a_type='21' and a_brno='" + cmb_to_br.SelectedValue + "'");

                    ref_max2 = Convert.ToInt32(dt.Rows[0][0]) + 1;
                    jrdacc = Convert.ToInt32(dt2.Rows[0][0]) + 1;

                    daml.SqlCon_Open();
                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("INSERT INTO sto_hdr(branch,trtype,ref,trmdate, trhdate,text,remarks,entries,amnttl,costttl,usrid,supno,whno,towhno,stkjvno,tobrno,isbrtrx,brxref) "
                        + " VALUES('" + cmb_to_br.SelectedValue + "','31'," + ref_max2 + ",'" + mdate + "','" + hdate + "','سند استلام بضاعة من - " + cmb_ftwh.Text + "','" + txt_remark.Text + "'," + (dataGridView1.RowCount - 1) + "," + txt_total.Text + "," + txt_cost.Text + ",'" + txt_user.Text + "','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "','" + cmb_ftwh.SelectedValue + "'," + (BL.CLS_Session.is_dorymost.Equals("2") ? jrdacc : 0) + ",'" + cmb_to_br.SelectedValue + "',1," + ref_max + ")", false);
                    daml.SqlCon_Close();
                    //daml.SqlCon_Open();
                ...
                {
                    daml.SqlCon_Open();
                     dtref = daml.SELECT_QUIRY_only_retrn_dt("select ref from sto_hdr where isbrtrx=1 and brxref=" + txt_ref.Text + " and trtype='31' and branch='" + cmb_to_br.SelectedValue + "'");
                    int exexcuteds = daml.Insert_Update_Delete_retrn_int("update sto_hdr set trmdate='" + mdate + "', trhdate='" + hdate + "',text='سند استلام بضاعة من - " + cmb_ftwh.Text + "',remarks='" + txt_remark.Text + "',supno='" + txt_custno.Text + "',entries=" + (dataGridView1.RowCount - 1) + ",lastupdt=getdate(),amnttl=" + txt_total.Text + ",costttl=" + txt_cost.Text + ",usrid='" + txt_user.Text + "',whno='" + cmb_ftwh.SelectedValue + "',tobrno='" + cmb_to_br.SelectedValue + "',towhno='" + cmb_ftwh_br.SelectedValue + "' where branch='" + cmb_to_br.SelectedValue + "' and trtype='31' and isbrtrx=1 and brxref=" + txt_ref.Text + "", false);
                    int exexcuteds2 = daml.Insert_Update_Delete_retrn_int("delete from  sto_dtl where branch='" + cmb_to_br.SelectedValue + "' and trtype='31' and ref=" + dtref.Rows[0][0] + "", false);
    ...
    */
                    if (!row.IsNewRow && row.Cells[0].Value != null)
                    {
                        ...
                        {
                            cmd.Parameters.AddWithValue("@a1", cmb_to_br.SelectedValue);
                            //   cmd.Parameters.AddWithValue("@a2", cmb_salctr.SelectedValue);
                            cmd.Parameters.AddWithValue("@a3", "31");
                            cmd.Parameters.AddWithValue("@a4", isnew ? ref_max2 : Convert.ToInt32(dtref.Rows[0][0]));
                            // cmd.Parameters.AddWithValue("@a4", row.Cells[2].Value);
                            cmd.Parameters.AddWithValue("@a5", mdate);
                            // cmd.Parameters.AddWithValue("@a5", (string.IsNullOrEmpty(row.Cells[3].Value.ToString())? 0 : Convert.ToDouble(row.Cells[3].Value)));
                            cmd.Parameters.AddWithValue("@a6", hdate);
        ...
        private void btn_Edit_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmb_type.SelectedValue.ToString().Equals("31"))
                {
                    MessageBox.Show("لا يمكن تعديل هذه الحركة الا من الفرع الاخر", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    //txt_key.Focus();
                    return;
                }
        ...
        private void btn_Del_Click(object sender, EventArgs e)
        {
           // btn_Rfrsh_Click(sender, e);
            ////if (cmb_type.SelectedValue.ToString().Equals("31"))
            ////{
            ////    MessageBox.Show("لا يمكن حذف هذه الحركة الا من الفرع الاخر", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ////    //txt_key.Focus();
            ////    return;
            ////}
            ...
            if (btn_Post.Enabled == true)
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        daml.Insert_Update_Delete_Only("delete from acc_hdr where a_posted=0 and a_brno='" + BL.CLS_Session.brno + "'  and a_type='01' and a_ref=" + txt_jrdacc.Text + "", false);
                        // exexcuted = exexcuted + daml.Insert_Update_Delete("delete from acc_dtl where a_ref=" + txt_ref.Text + " and a_type='" + comboBox1.SelectedValue + "'", false);
                        if (BL.CLS_Session.is_dorymost.Equals("2"))
                            daml.Insert_Update_Delete_Only("delete from acc_hdr where a_brno='" + BL.CLS_Session.brno + "' and sl_no='' and a_type='21' and a_ref=" + txt_jrdacc.Text + "", false);
        ...
        private void btn_Find_Click(object sender, EventArgs e)
        {
            try
            {
                var sr = new Search_All("30-br", "Sto");
                sr.ShowDialog();
        ...
        private void Fill_Data(string slctr, string atype, string aref)
        {
            try
            {
                ...
                dthdr = daml.SELECT_QUIRY_only_retrn_dt("select * from sto_hdr where trtype='" + atype + "' and ref=" + aref + " and branch='" + BL.CLS_Session.brno + "' and isbrtrx=1");
                dtdtl = daml.SELECT_QUIRY_only_retrn_dt("select aa.itemno itemno,bb.item_name item_name,aa.pack pack,aa.pkqty pkqty,aa.qty qty,aa.lprice lcost,(aa.qty*aa.lprice) total,aa.lcost cost,aa.barcode barcode,iif(rtrim(ltrim(expdate))<>'', convert(varchar(10) ,cast([expdate] as date) ,110),'') expdate  from sto_dtl aa,items bb where aa.itemno=bb.item_no and aa.trtype='" + atype + "' and aa.ref=" + aref + " and aa.branch='" + BL.CLS_Session.brno + "' order by aa.folio");// and aa.a_type='" + atype + "' and aa.a_ref=" + aref + "");

                if (Convert.ToBoolean(dthdr.Rows[0]["isbrtrx"]) == false)
                {
                    MessageBox.Show("لا يمكن فتح هذه الحركة من هذه الشاشة", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    //txt_key.Focus();
                    return;
                }
                cmb_type.SelectedValue = dthdr.Rows[0]["trtype"];
                cmb_ftwh.SelectedValue = atype.Equals("31") ? dthdr.Rows[0]["towhno"] : dthdr.Rows[0]["whno"];

                cmb_to_br.SelectedValue = dthdr.Rows[0]["tobrno"];
                cmb_to_br_Leave(null, null);
                cmb_ftwh_br.SelectedValue = dthdr.Rows[0]["towhno"];

                //cmb_towh.SelectedValue = dthdr.Rows[0]["towhno"];
                txt_ref.Text =isnew?"": dthdr.Rows[0]["ref"].ToString();
                txt_ref2.Text = dthdr.Rows[0]["brxref"].ToString();
                ...
                jrdacc =Convert.ToInt32(dthdr.Rows[0]["stkjvno"]);
              ...
              //  load_key();
                ...
                accdthdr = daml.SELECT_QUIRY_only_retrn_dt("select * from acc_hdr where a_type='01' and a_ref = " + jrdacc + " and a_brno='" + BL.CLS_Session.brno + "'");
                // accdtdtl = daml.SELECT_QUIRY_only_retrn_dt("select * from acc_hdr where a_type='01' and a_ref = " + txt_jrdacc.Text + "");
              //  MessageBox.Show(accdthdr.Rows[0]["mainkey"].ToString());
                txt_accno.Text=accdthdr.Rows[0]["mainkey"].ToString();
                load_key();
                // dcombo.Value = dt222.Rows[0][5];
                // dataGridView1.CurrentRow.Cells[2] = dcombo;
                /*
        ...
        {/*
          ...
          */
            ...
            //        int exexcuteds = daml.Insert_Update_Delete_retrn_int("update sto_hdr set stkjvno= " + (BL.CLS_Session.is_dorymost.Equals("2") ? jrdacc : jrdacc) + " where branch='" + BL.CLS_Session.brno + "' and trtype='" + cmb_type.SelectedValue + "' and ref=" + txt_ref.Text + "", false);
        ...
        private void comboBox1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (string.IsNullOrEmpty(txt_desc.Text))
                    txt_desc.Text = cmb_type.Text + " - " + cmb_ftwh.Text;
                cmb_ftwh.Focus();
        ...
        private void btn_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //  dt = dtdtl.Copy();
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sto.rpt.Sto_Tran_rpt.rdlc";
                ...
                parameters[5] = new ReportParameter("wh_name", cmb_ftwh.Text);

                parameters[6] = new ReportParameter("desc", txt_desc.Text);
               // parameters[7] = new ReportParameter("tax_id", BL.CLS_Session.tax_no);
                parameters[7] = new ReportParameter("total", txt_total.Text);
                parameters[8] = new ReportParameter("descount", txt_des.Text);
                parameters[9] = new ReportParameter("tax", "0");
                parameters[10] = new ReportParameter("nettotal", txt_total.Text);

                BL.ToWord toWord = new BL.ToWord(Convert.ToDecimal(txt_total.Text), currencies[Convert.ToInt32(BL.CLS_Session.cur)]);

                //  MessageBox.Show(toWord.ConvertToArabic());
                parameters[11] = new ReportParameter("a_toword", toWord.ConvertToArabic());
        ...
        {
            DataTable dt = daml.SELECT_QUIRY_only_retrn_dt("select a_name from accounts where a_key='" + txt_accno.Text + "' and a_brno='" + BL.CLS_Session.brno + "'");
            // dataGridView1.DataSource = daml.SELECT_QUIRY(qstr2);

            if (dt.Rows.Count > 0)
                txt_accname.Text = dt.Rows[0][0].ToString();
            else
                txt_accname.Text = "";
            /*
        ...
        private void txt_key_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F8 && dataGridView1.ReadOnly==false)
            {
                ...
                try
                {
                    ...
                    f4.ShowDialog();
                    txt_accno.Text = f4.dataGridView1.CurrentRow.Cells[0].Value.ToString();
                    txt_accname.Text = f4.dataGridView1.CurrentRow.Cells[1].Value.ToString();
                }
                catch { }
        ...
        private void save_acc1()
        {
            ...
            try
            {
                ...
                {
                    daml.SqlCon_Open();
                   // int exexcuted = daml.Insert_Update_Delete_retrn_int("INSERT INTO acc_hdr(a_brno,a_type,a_ref,a_mdate, a_hdate,a_text,a_amt,a_entries,usrid,jhsrc,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + ref_max + ",'" + mdate + "','" + hdate + "','" + txt_desc.Text + "'," + (Convert.ToDouble(txt_total.Text) + Convert.ToDouble(0)) + "," + (dataGridView1.RowCount - 1) + ",'" + txt_user.Text + "','Sal','" + cmb_ftwh.SelectedValue + "')", false);
                    if (BL.CLS_Session.is_dorymost.Equals("2"))
                        daml.Insert_Update_Delete_retrn_int("INSERT INTO acc_hdr(a_brno,a_type,a_ref,a_mdate, a_hdate,a_text,a_amt,a_entries,usrid,jhsrc,sl_no) VALUES('" + BL.CLS_Session.brno + "','21'," + jrdacc + ",'" + mdate + "','" + hdate + "','قيمة بضاعة جرد مستمر'," + (Convert.ToDouble(txt_cost.Text)) + "," + 2 + ",'" + txt_user.Text + "','Grd1','')", false);
                    daml.SqlCon_Close(); 
                }
                else
                {
                  ...
                  //  int exexcuted = daml.Insert_Update_Delete_retrn_int("update acc_hdr set a_mdate='" + mdate + "', a_hdate='" + hdate + "',a_text='" + txt_desc.Text + "',a_amt=" + txt_total.Text + ",a_entries=" + (dataGridView1.RowCount - 1) + ",usrid='" + txt_user.Text + "' where a_ref=" + txt_ref.Text + " and a_type='" + cmb_type.SelectedValue + "' and a_brno='" + BL.CLS_Session.brno + "'", false);
                    //exexcuted = exexcuted + daml.Insert_Update_Delete_retrn_int("delete from acc_dtl where a_ref=" + txt_ref.Text + " and a_type='" + cmb_type.SelectedValue + "' and a_brno='" + BL.CLS_Session.brno + "' and sl_no='" + cmb_ftwh.SelectedValue + "'", false);
                    //daml.SqlCon_Close();
                    if (BL.CLS_Session.is_dorymost.Equals("2"))
                        daml.Insert_Update_Delete_retrn_int("update acc_hdr set lastupdt=getdate(),a_mdate='" + mdate + "', a_hdate='" + hdate + "',a_amt=" + txt_cost.Text + ",usrid='" + txt_user.Text + "' where a_ref=" + txt_jrdacc.Text + " and a_type='21' and a_brno='" + BL.CLS_Session.brno + "' and sl_no=''", false);
                     //   daml.Insert_Update_Delete_retrn_int("delete from acc_dtl where a_ref=" + txt_ref.Text + " and a_type='" + cmb_type.SelectedValue + "' and a_brno='" + BL.CLS_Session.brno + "' and sl_no='" + cmb_salctr.SelectedValue + "'", false);
                    if (BL.CLS_Session.is_dorymost.Equals("2"))
                        daml.Insert_Update_Delete_retrn_int("delete from acc_dtl where a_ref=" + txt_jrdacc.Text + " and a_type='21' and a_brno='" + BL.CLS_Session.brno + "' and sl_no=''", false);
                    daml.SqlCon_Close();
                }
                ...
                switch (cmb_type.SelectedValue.ToString())
                {
                   // case "05":
                    ...
                    case "31":
                        // strkey = txt_temp.Text;
                        //daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strrcrdt + "','" + txt_desc.Text + "',0," + (Convert.ToDouble(txt_net.Text) + Convert.ToDouble(txt_des.Text) - Convert.ToDouble(0)) + ",1,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "')", false);
                        //daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cu_no,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strkey + "','" + txt_desc.Text + "'," + txt_net.Text + "," + 0 + ",3,'" + mdate + "','" + hdate + "','C','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "')", false);

                        //if (Convert.ToDouble(txt_des.Text) > 0)
                        //{
                        //    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strdsc + "','خصومات مبيعات'," + txt_des.Text + "," + 0 + ",3,'" + mdate + "','" + hdate + "','C','" + cmb_ftwh.SelectedValue + "')", false);
                        //}
                        if (BL.CLS_Session.is_dorymost.Equals("2"))
                        {
                            daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt, a_camt, a_folio,a_mdate,a_hdate,jddbcr,sl_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','21'," + (isnew ? jrdacc : Convert.ToInt32(txt_jrdacc.Text)) + ",'" + txt_key.Text + "','" + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)).ToString() + " قيمة البضاعة استلام مخزني '," + 0 + "," + Convert.ToDouble(txt_total.Text) + ",1,'" + mdate + "','" + hdate + "','C','','" + stcstcode + "')", false);
                            daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_damt, a_camt, a_folio,a_mdate,a_hdate,jddbcr,sl_no,cc_no) VALUES('" + BL.CLS_Session.brno + "','21'," + (isnew ? jrdacc : Convert.ToInt32(txt_jrdacc.Text)) + ",'" + stmkhzon + "','" + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)).ToString() + " قيمة البضاعة استلام مخزني '," + Convert.ToDouble(txt_total.Text) + ",0,2,'" + mdate + "','" + hdate + "','D','','" + stcstcode + "')", false);
                        }
                    ...
                    //////case "04":
                    //////    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strcash + "','" + txt_desc.Text + "'," + txt_total.Text + ",0,1,'" + mdate + "','" + hdate + "','C','" + cmb_ftwh.SelectedValue + "')", false);
                    //////    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cu_no,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strkey + "','" + txt_desc.Text + "',0," + txt_net.Text + ",3,'" + mdate + "','" + hdate + "','D','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "')", false);
                    ...
                    //////case "14":
                    //////    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strrcash + "','" + txt_desc.Text + "',0," + (Convert.ToDouble(txt_net.Text) + Convert.ToDouble(txt_des.Text) - Convert.ToDouble(0)) + ",1,'" + mdate + "','" + hdate + "','D','" + cmb_ftwh.SelectedValue + "')", false);
                    //////    daml.Insert_Update_Delete_Only("INSERT INTO acc_dtl(a_brno,a_type, a_ref, a_key, a_text, a_camt, a_damt, a_folio,a_mdate,a_hdate,jddbcr,cu_no,sl_no) VALUES('" + BL.CLS_Session.brno + "','" + cmb_type.SelectedValue + "'," + (isnew ? ref_max : Convert.ToInt32(txt_ref.Text)) + ",'" + strkey + "','" + txt_desc.Text + "'," + txt_net.Text + "," + 0 + ",3,'" + mdate + "','" + hdate + "','C','" + txt_custno.Text + "','" + cmb_ftwh.SelectedValue + "')", false);
        ...
        private void cmb_salctr_Leave(object sender, EventArgs e)
        {
            ...
            DataRow[] dr = dtslctr.Select("wh_no = '" + cmb_ftwh.SelectedValue + "'");
            stmkhzon = dr[0][2].ToString();
            stcstcode = dr[0]["cstcode"].ToString();
            stwhno = cmb_ftwh.SelectedValue.ToString(); // dr[0][14].ToString();
        }
        ...
        private void cmb_towh_Enter(object sender, EventArgs e)
        {
            try
            {
                ...
                //dv1 = dtslctr.DefaultView;
                //// dv1.RowFilter = " pack_id = 1";//or ProductId = 2 or ProductID = 3";
                //dv1.RowFilter = "wh_no not in(" + cmb_ftwh.SelectedValue + ")";
                //DataTable dtNew = dv1.ToTable();
                //// MessageBox.Show(dtNew.Rows[0][1].ToString());
                //cmb_towh.DataSource = dtNew;
        ...
        private void cmb_to_br_Leave(object sender, EventArgs e)
        {
            dttowhbr = daml.SELECT_QUIRY_only_retrn_dt("select * from whouses where wh_brno = '" + cmb_to_br.SelectedValue + "'");

            cmb_ftwh_br.DataSource = dttowhbr;
            cmb_ftwh_br.DisplayMember = "wh_name";
            cmb_ftwh_br.ValueMember = "wh_no";
            cmb_ftwh_br.SelectedIndex = -1;
        }

        private void cmb_to_br_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                cmb_ftwh_br.Focus();
        ...
        private void cmb_ftwh_br_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                txt_accno.Focus();
        ...
        private void txt_accno_Leave(object sender, EventArgs e)
        {
            load_key();
        }

        private void Sto_ImpExp_Br_Shown(object sender, EventArgs e)
        {
