...
namespace POS.Sto
{
    public partial class Sto_Whbins_Brb : BaseForm
    {
        ...
        private void whamount_Load(object sender, EventArgs e)
        {
           ...
           // daml.SqlCon_Close();
            ...
            ////{
                ...
                // MessageBox.Show(sl);
                dtslctr = daml.SELECT_QUIRY_only_retrn_dt("select br_no,br_name from branchs where br_no in(" + sl + ")");



                cmb_wh.DataSource = dtslctr;
                cmb_wh.DisplayMember = "br_name";
                cmb_wh.ValueMember = "br_no";
                cmb_wh.SelectedIndex = -1;
                ...
                foreach (DataRow dtrr in dtslctr.Rows)
                {
                    ...
                    col.Width = 100;
                    col.DataPropertyName = "val" + dtrr[0].ToString();
                    strqadd = strqadd + " ,sum(case when b.br_no='" + dtrr[0].ToString() + "' then b.openbal + b.qty else 0.00 end) as val" + dtrr[0].ToString();
                    //  col4.HeaderText = "Column4";
                    //  col4.Name = "Column4";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
          ...
          //  string query = "select a.item_no a1, a.item_name b1, b.qty + b.openbal c1,b.lcost d1, (b.qty + b.openbal) * b.lcost e1 from items a join whbins b on a.item_no=b.item_no and a.item_no + a.item_name like '%" + textBox2.Text + "%' " + conwh + conall +" ";
            string query ="";
          //  if(!chk_allbr.Checked)
            //    query = "select a.item_no a1, a.item_name a2,cast(a.item_unit as nvarchar) a3,round(a.item_curcost,2) a4,a.item_price a5 " + strqadd + " from items a WITH (NOLOCK) join whbins b WITH (NOLOCK) on a.item_no=b.item_no " + conit + consup + conwh + conall + congrp + congrsp + condopnbalonly + conaball + " group by a.item_no,a.item_name,a.item_unit,a.item_curcost,a.item_price ";
           // else
                query = "select a.item_no a1, a.item_name a2,cast(a.item_unit as nvarchar) a3,round(a.item_curcost,2) a4,a.item_price a5 " + strqadd + " from items a WITH (NOLOCK) join whbins b WITH (NOLOCK) on a.item_no=b.item_no " + conit + consup + conwh + conall + congrp + congrsp + condopnbalonly + conaball + " group by a.item_no,a.item_name,a.item_unit,a.item_curcost,a.item_price ";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // dt = daml.SELECT_QUIRY_only_retrn_dt("select b.item_no d1,a.item_name d2,(b.qty + b.openbal) d3 , c.wh_name d4 from items a inner join whbins b on a.item_no = b.item_no inner join whouses c on c.wh_no= b.wh_no and c.wh_brno=b.br_no");
              ///  if(!chk_allbr.Checked)
              ...
              ////  else
                dt = daml.SELECT_QUIRY_only_retrn_dt("select b.item_no c1,a.item_name c2,e.unit_name c3,round(a.item_curcost,2) c4,a.item_price c5,(b.qty + b.openbal) c6 , c.br_name c7 from items a inner join whbins b on a.item_no = b.item_no inner join branchs c on  c.br_no=b.br_no inner join units e on e.unit_id=a.item_unit");
                //////DataTable dt = new DataTable();
                //////dt = dt2.Copy();
                ...
                rf.reportViewer1.LocalReport.ReportEmbeddedResource = "POS.Sto.rpt.Sto_Report_Wh_rpt.rdlc";
        ...
        {
            //chk_allbr.Enabled = false;
            ////InitializeComponent();
            //dataGridView1.Visible = false;
            //dataGridView2.Visible = true;
            //whamount_Load( sender,  e);
            //if (chk_allbr.Checked)
            //{
